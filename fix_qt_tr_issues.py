        ...
        for pattern in QT_WIDGET_PATTERNS:
            ...
            if matches:
                for match in matches:
                    ...
                    if isinstance(match, tuple) or (
                        pattern == QT_WIDGET_PATTERNS[-1] and "," in match
                    ):
...
def scan_directory(
    directory: Path, extensions: List[str] = None, dry_run: bool = True
) -> Dict[str, int]:
    """
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't modify files, just show issues"
    )
