API Reference
=============

Complete API documentation for all GOES_VFI modules.

.. toctree::
   :maxdepth: 2
   :caption: API Documentation


Pipeline
--------

.. toctree::
   :maxdepth: 1

   pipeline_cache
   pipeline_encode
   pipeline_ffmpeg_builder
   pipeline_image_cropper
   pipeline_image_loader
   pipeline_image_processing_interfaces
   pipeline_image_saver
   pipeline_interpolate
   pipeline_loader
   pipeline_raw_encoder
   pipeline_run_ffmpeg
   pipeline_run_vfi
   pipeline_sanchez_processor
   pipeline_tiler


Integrity Check
---------------

.. toctree::
   :maxdepth: 1

   integrity_check_background_worker
   integrity_check_cache_db
   integrity_check_combined_tab
   integrity_check_dark_mode_style
   integrity_check_date_range_selector
   integrity_check_enhanced_gui_tab
   integrity_check_enhanced_gui_tab_components_dialogs
   integrity_check_enhanced_gui_tab_components_models
   integrity_check_enhanced_gui_tab_improved
   integrity_check_enhanced_imagery_tab
   integrity_check_enhanced_timeline
   integrity_check_enhanced_view_model
   integrity_check_goes_imagery
   integrity_check_goes_imagery_tab
   integrity_check_gui_tab
   integrity_check_optimized_timeline_tab
   integrity_check_reconcile_manager
   integrity_check_reconciler
   integrity_check_remote_base
   integrity_check_remote_cdn_store
   integrity_check_remote_composite_store
   integrity_check_remote_s3_store
   integrity_check_remote_store
   integrity_check_render_netcdf
   integrity_check_results_organization
   integrity_check_sample_processor
   integrity_check_satellite_integrity_tab_group
   integrity_check_shared_components
   integrity_check_thread_cache_db
   integrity_check_time_index
   integrity_check_time_index_refactored
   integrity_check_timeline_visualization
   integrity_check_view_model
   integrity_check_visual_date_picker
   integrity_check_visualization_manager


GUI Components
--------------

.. toctree::
   :maxdepth: 1

   gui
   gui_components_crop_manager
   gui_components_model_manager
   gui_components_preview_manager
   gui_components_processing_manager
   gui_components_settings_manager
   gui_enhancements_integration
   gui_integration_patch
   gui_tabs_ffmpeg_settings_tab
   gui_tabs_ffmpeg_settings_tab_enhanced
   gui_tabs_main_tab
   gui_tabs_main_tab_components_enums
   gui_tabs_main_tab_components_types
   gui_tabs_main_tab_components_utils
   gui_tabs_main_tab_components_widgets
   gui_tabs_main_tab_enhanced
   gui_tabs_model_library_tab
   gui_tabs_operation_history_tab
   gui_tabs_resource_limits_tab


Utilities
---------

.. toctree::
   :maxdepth: 1

   utils_config
   utils_date_utils
   utils_gui_helpers
   utils_log
   utils_memory_manager
   utils_rife_analyzer
   utils_security


Sanchez Integration
-------------------

.. toctree::
   :maxdepth: 1

   sanchez_health_check
   sanchez_runner


Exceptions
----------

.. toctree::
   :maxdepth: 1

   exceptions


Other
-----

.. toctree::
   :maxdepth: 1

   date_sorter_gui_tab
   date_sorter_sorter
   date_sorter_view_model
   file_sorter_gui_tab
   file_sorter_sorter
   file_sorter_sorter_refactored
   file_sorter_view_model
   view_models_main_window_view_model
   view_models_processing_view_model


Indices and Tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
