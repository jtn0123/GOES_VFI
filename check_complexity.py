...
FILES_TO_CHECK = [
    ...
    (
        ...
        r"extract_timestamp_from_directory_name",
    ),
    ("goesvfi/integrity_check/time_index_refactored.py", r"to_s3_key"),
    (
        "goesvfi/integrity_check/time_index_refactored.py",
        r"scan_directory_for_timestamps",
    ),
    ...
    for file_path, function_pattern in FILES_TO_CHECK:
        ...
        if issues:
            for function_name, complexity, line_num in issues:
                print(
                    f"  - Line {line_num}: Function '{function_name}' has complexity of {complexity}"
                )
        else:
            print(
                f"  - No complexity issues found! Functions matching '{function_pattern}' are under the threshold."
            )
