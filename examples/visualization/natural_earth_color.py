...
def correct_atmosphere(r, g, b):
    ...
    if g_max > 0: g_corr = g_corr / g_max
    if b_max > 0: b_corr = b_corr / b_max
...
def make_realistic_green(r, nir):
    ...
    valid_mask = (nir + r) > 0
    ndvi[valid_mask] = (nir[valid_mask] - r[valid_mask]) / (nir[valid_mask] + r[valid_mask])
...
def apply_earth_color_balance(r, g, b):
    ...
    r_balanced[cloud_mask] = r_balanced[cloud_mask] * (1-cloud_strength) + cloud_strength
    g_balanced[cloud_mask] = g_balanced[cloud_mask] * (1-cloud_strength) + cloud_strength
    b_balanced[cloud_mask] = b_balanced[cloud_mask] * (1-cloud_strength) + cloud_strength
...
def create_natural_earth_color(band_files, output_path, target_shape=None, minimal=False):
    """
...
def main():
    parser = argparse.ArgumentParser(description="Create natural Earth color imagery from GOES ABI data")
    ...
    parser.add_argument("--resolution", type=int, nargs=2, default=[2000, 2000],
