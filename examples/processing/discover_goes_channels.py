...
ABI_BANDS = {
    1: {"name": "Blue", "wavelength": "0.47 μm", "type": "Visible", "resolution": "1 km", "primary_use": "Daytime aerosol detection, smoke, haze"},
    2: {"name": "Red", "wavelength": "0.64 μm", "type": "Visible", "resolution": "0.5 km", "primary_use": "Daytime cloud and surface features"},
    3: {"name": "Veggie", "wavelength": "0.86 μm", "type": "Near-IR", "resolution": "1 km", "primary_use": "Daytime vegetation, burn scars, aerosol"},
    4: {"name": "Cirrus", "wavelength": "1.37 μm", "type": "Near-IR", "resolution": "2 km", "primary_use": "Daytime cirrus cloud detection"},
    5: {"name": "Snow/Ice", "wavelength": "1.6 μm", "type": "Near-IR", "resolution": "2 km", "primary_use": "Daytime cloud phase, snow detection"},
    6: {"name": "Cloud Particle", "wavelength": "2.2 μm", "type": "Near-IR", "resolution": "2 km", "primary_use": "Daytime land/cloud properties, particle size"},
    7: {"name": "Shortwave Window", "wavelength": "3.9 μm", "type": "IR", "resolution": "2 km", "primary_use": "Fire detection, night fog, winds"},
    8: {"name": "Upper-Level Water Vapor", "wavelength": "6.2 μm", "type": "IR", "resolution": "2 km", "primary_use": "High-level atmospheric water vapor, winds"},
    9: {"name": "Mid-Level Water Vapor", "wavelength": "6.9 μm", "type": "IR", "resolution": "2 km", "primary_use": "Mid-level atmospheric water vapor, winds"},
    10: {"name": "Lower-level Water Vapor", "wavelength": "7.3 μm", "type": "IR", "resolution": "2 km", "primary_use": "Lower-level water vapor, winds"},
    11: {"name": "Cloud-Top Phase", "wavelength": "8.4 μm", "type": "IR", "resolution": "2 km", "primary_use": "Total water, cloud phase, dust"},
    12: {"name": "Ozone", "wavelength": "9.6 μm", "type": "IR", "resolution": "2 km", "primary_use": "Ozone detection, turbulence"},
    13: {"name": "Clean IR Longwave", "wavelength": "10.3 μm", "type": "IR", "resolution": "2 km", "primary_use": "Surface and cloud features, cloud top"},
    14: {"name": "IR Longwave", "wavelength": "11.2 μm", "type": "IR", "resolution": "2 km", "primary_use": "Clouds, nighttime fog, volcanic ash"},
    15: {"name": "Dirty IR Longwave", "wavelength": "12.3 μm", "type": "IR", "resolution": "2 km", "primary_use": "Volcanic ash, dust, sea surface temperature"},
    16: {"name": "CO2 IR Longwave", "wavelength": "13.3 μm", "type": "IR", "resolution": "2 km", "primary_use": "Air temperature, cloud heights"}
}
...
def discover_available_channels(date_str, hour, satellite="GOES16", sector="F"):
    ...
    for band in range(1, 17):
        ...
        if files:
            print(f"  Band {band} ({ABI_BANDS[band]['name']}): {len(files)} files available")
...
def print_channel_info(channel_data):
    ...
    for ch in sorted(channel_data, key=lambda x: x['band']):
        if 'error' in ch:
            ...
            value_range = f"{ch.get('min_value', 'N/A'):.3f} - {ch.get('max_value', 'N/A'):.3f}"
    ...
    print(tabulate(
        basic_table, 
        headers=["Band", "Name", "Wavelength", "Type", "Value Type", "Value Range", "Resolution", "Primary Use"],
        tablefmt="grid"
    ))
    ...
    for ch in sorted(channel_data, key=lambda x: x['band']):
        ...
        print(f"  Value type: {ch.get('value_type', 'Unknown')} ({ch.get('units', 'Unknown')})")
        print(f"  Value range: {ch.get('min_value', 'N/A'):.3f} - {ch.get('max_value', 'N/A'):.3f} (mean: {ch.get('mean_value', 'N/A'):.3f})")
...
def main():
    parser = argparse.ArgumentParser(description="Discover and analyze available GOES ABI channels")
