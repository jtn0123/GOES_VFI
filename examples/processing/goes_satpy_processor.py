...
try:
    ...
    print("Warning: Satpy is not installed. Some features will be limited.")
    print("To install satpy, run: pip install satpy")
...
# Output directories
DOWNLOAD_DIR = Path(os.path.expanduser("~/Downloads/goes_satpy"))
...
def build_s3_cmip_wildcard(satellite, sector, band, year, doy, hour):
    ...
    for code in sector_codes:
        ...
        pattern = f"OR_ABI-L2-CMIP{code}-M6C{band:02d}_G{satellite_num}"\
...
def find_s3_files(bucket, prefix, patterns):
    """Find files in S3 bucket matching wildcard patterns."""
    print(f"Looking for files in {bucket}/{prefix} with patterns: {patterns}")
    ...
    try:
        ...
        for page in pages:
            ...
            for obj in page["Contents"]:
                key = obj["Key"]
                if any(re.search(pattern.replace("*", ".*"), key) for pattern in patterns):
...
def process_true_color_with_satpy(band_files, output_path):
    """Process true color image using Satpy."""
    if not SATPY_AVAILABLE:
        print("  Satpy is not available. Cannot process with Satpy.")
        return False, None
    
    print(f"Processing true color image with Satpy from {len(band_files)} files")
    
    try:
        # Create a Scene with the CMIP files
        filenames = [str(f) for f in band_files]
        scn = Scene(reader="abi_l2_cmip", filenames=filenames)
        
        # Available products
        available_composites = scn.available_composite_names()
        available_datasets = scn.available_dataset_names()
        
        print(f"  Available composites: {available_composites}")
        print(f"  Available datasets: {available_datasets}")
        
        # Load the true_color composite
        if "true_color" in available_composites:
            scn.load(["true_color"])
            
            # Convert to image and save
            img = to_image(scn["true_color"])
            img.save(output_path)
            print(f"  Saved true color image to {output_path}")
            
            return True, output_path
        else:
            print("  'true_color' composite not available from these files")
            
            # Try manual composition
            if set(["C01", "C02", "C03"]).issubset(set(available_datasets)):
                print("  Loading individual bands for manual composition")
                scn.load(["C01", "C02", "C03"])
                
                # Create RGB composite
                rgb = scn.images["true_color"]
                img = to_image(rgb)
                img.save(output_path)
        ...
        print(f"  Error processing with Satpy: {e}")
        return False, None

def process_true_color_manually(file_paths, output_path):
    """Create true color image by manually combining bands 1, 2, and 3."""
    print(f"Manually processing true color image from {len(file_paths)} files")
    ...
    try:
        ...
        for i, file_path in enumerate(file_paths):
            ...
            with xr.open_dataset(file_path) as ds:
                ...
                print(f"  Loaded Band {band_num} from {os.path.basename(file_path)}, shape: {data.shape}")
        ...
        if len(set(shapes)) > 1:
            ...
            for i, data in enumerate(band_data):
                if data.shape[0] == min_height and data.shape[1] == min_width:
                    ...
                    img = Image.fromarray(np.clip(data, 0, 1).astype(np.float32), mode="F")
...
def get_goes_image(satellite, sector, product, year, doy, hour, output_dir=None):
    ...
    for band in required_bands:
        ...
        if sector_code == "M":
            ...
            prefixes = [
                build_s3_prefix(f"ABI-L2-CMIPM1", year, doy, hour),
                build_s3_prefix(f"ABI-L2-CMIPM2", year, doy, hour)
            ]
        else:
    ...
    if product.lower() in ["ir", "band13"]:
        # Process IR image with Sánchez colorization
        output_path = output_dir / f"{satellite}_{sector}_ir_{closest_timestamp}_sanchez.png"
        success, gray_path, color_path = process_ir_image(downloaded_files[0], output_path)
        ...
        output_path = output_dir / f"{satellite}_{sector}_truecolor_{closest_timestamp}.png"
        
        # Try Satpy first if available
        if SATPY_AVAILABLE:
            success, image_path = process_true_color_with_satpy(downloaded_files, output_path)
            if success:
        ...
        output_path = output_dir / f"{satellite}_{sector}_{band_name}_{closest_timestamp}.png"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download and process GOES satellite imagery with Satpy")
    ...
    parser.add_argument("--product", choices=["truecolor", "ir", "band13", "wv", "band09", "swir", "band07"],
                      default="truecolor", help="Product to download (default: truecolor)")
    parser.add_argument("--year", type=int, default=2023,
                      help="Year (default: 2023)")
    parser.add_argument("--doy", type=int, default=121,
                      help="Day of year (default: 121 for May 1)")
    parser.add_argument("--hour", type=int, default=19,
                      help="Hour of day UTC (default: 19)")
