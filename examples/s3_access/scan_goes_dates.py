...
def scan_range(args):
    ...
    if result["new_exists"] or result["old_exists"]:
        ...
        print(f"✓ {result['bucket']} - {result['product']} - {result['date']} - Hour {result['hour']}")
...
def main():
    parser = argparse.ArgumentParser(description="Scan for available GOES satellite data")
    parser.add_argument("--buckets", nargs="+", default=["noaa-goes16", "noaa-goes18"], 
                        help="S3 bucket names to check (default: noaa-goes16 noaa-goes18)")
    parser.add_argument("--products", nargs="+", default=["CMIPF", "CMIPC", "CMIPM"], 
                        help="Product types to check (default: CMIPF CMIPC CMIPM)")
    parser.add_argument("--start-date", default="2023-05-05", 
                        help="Start date in YYYY-MM-DD format (default: 2023-05-05)")
    parser.add_argument("--end-date", default="2023-05-05", 
                        help="End date in YYYY-MM-DD format (default: same as start-date)")
    parser.add_argument("--hours", nargs="+", type=int, default=list(range(0, 24, 4)),
                        help="Hours to check (default: 0 4 8 12 16 20)")
    parser.add_argument("--verbose", action="store_true", help="Print all checks, not just found paths")
    ...
    # Print summary
    print("\nScan Complete!")
    print(f"Found {len(found_paths)} valid data paths across {len(scan_args)} combinations.")
