...
S3_BUCKETS = {"GOES_16": "noaa-goes16", "GOES_18": "noaa-goes18"}

# Satellite codes
SATELLITE_CODES = {"GOES_16": "G16", "GOES_18": "G18"}
...
async def list_s3_objects(
    bucket: str, prefix: str, pattern: Optional[str] = None, limit: int = 10
) -> List[Dict[str, Any]]:
    """
...
async def download_s3_object(bucket: str, key: str, output_path: str) -> bool:
    ...
    logger.info("Downloading s3://%s/%s to %s", bucket, key, output_path)
    ...
    async with session.client("s3", config=s3_config) as s3:
        try:
            ...
            logger.error("Error downloading S3 object: %s", e)
            return False
...
async def test_scan_times(
    ...
    if not satellite_code:
        logger.error("Invalid satellite: %s", satellite)
        return

    bucket = S3_BUCKETS.get(satellite)
    if not bucket:
        logger.error("Invalid satellite for bucket: %s", satellite)
        return
    ...
    if product_type == "RadF":
        scan_minutes = RADF_MINUTES
        logger.info("Using RadF scan minutes: %s", scan_minutes)
    elif product_type == "RadC":
        scan_minutes = RADC_MINUTES
    ...
    if total_tests > 0:
        ...
        logger.info(
            f"Summary: Found data for {total_found} out of {total_tests} test times ({success_rate:.1f}% success rate)"
        )
    else:
...
async def test_exact_timestamps(
    date: datetime, satellite: str, product_type: str, band: int, download: bool = False
) -> None:
    """
    ...
    for minute in scan_minutes:
        ...
        if objects:
            ...
            for i, obj in enumerate(objects):
                ...
                logger.info("  %s. %s", i + 1, key)
                logger.info("     Last modified: %s", modified)
                ...
                if download:
                    ...
                    if success:
                        ...
                        logger.error("     Failed to download: %s", output_path)
        else:
            logger.info("❌ No objects found for %s", test_time.isoformat())
...
async def test_band_wildcards(
    ...
    logger.info("Found %s objects with prefix: %s", len(objects), prefix)
        ...
        if f"ABI-L1b-{product_type}-M6C" in key and f"_{satellite_code}_s" in key:
            try:
                ...
                if band_counts[band_num] == 1:
                    logger.info("Band %s: %s", band_num, key)
            except (ValueError, IndexError):
                logger.warning("Could not extract band number from key: %s", key)

    # Sort bands by number and print summary
    sorted_bands = sorted(band_counts.keys())
    logger.info("Found data for %s different bands:", len(sorted_bands))
    for band in sorted_bands:
        logger.info("  Band %s: %s files", band, band_counts[band])
...
async def main():
    ...
    logger.info("Testing with date: %s", date.isoformat())
    logger.info("Satellite: %s", args.satellite)
    logger.info("Product type: %s", args.product)
