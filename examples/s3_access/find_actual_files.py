...
def download_and_process(bucket, key, product_type):
    ...
    try:
        ...
        # Process the file
        print(f"  Processing file to create IR image...")
...
def main():
    ...
    print(f"Checking {bucket} for data at noon Pacific time ({TEST_DATE} {TEST_HOUR}:xx UTC)")
    print("-" * 70)
    ...
    for product in PRODUCT_TYPES:
        ...
        sorted_files = sorted(band_files, 
                             key=lambda x: abs(int(x.split("s")[1].split("_")[0][8:12]) - int(target_time)))
        ...
        if success:
            print(f"  Successfully downloaded and processed file")
        else:
            print(f"  Failed to download and process file")
