...
try:
    from PyQt6.QtCore import pyqtRemoveInputHook, QObject, QMetaObject
...
def patch_s3_store():
    ...
    try:
        ...
        async def logged_get_s3_client(self):
            ...
            logger.debug(f"S3Store._get_s3_client called, client exists: {self._s3_client is not None}")
            try:
                # Show actual parameters being used
                logger.debug(f"AWS profile: {self.aws_profile}, region: {self.aws_region}")
                logger.debug(f"Session kwargs: {self.session_kwargs}")
                
                result = await original_get_s3_client(self)
                
                logger.debug(f"S3Store._get_s3_client succeeded, returning client: {result}")
                if result is None:
        ...
        async def logged_exists(self, ts, satellite):
            ...
            logger.debug(f"S3Store.exists called for {satellite.name} at {ts.isoformat()}")
            try:
                ...
                logger.debug(f"S3Store.exists result for {satellite.name} at {ts.isoformat()}: {result}")
                return result
            except Exception as e:
                logger.error(f"Error in S3Store.exists for {satellite.name} at {ts.isoformat()}: {e}")
                logger.error(traceback.format_exc())
                raise
        ...
        async def logged_download(self, ts, satellite, dest_path):
            ...
            logger.debug(f"S3Store.download called for {satellite.name} at {ts.isoformat()} to {dest_path}")
            try:
                bucket, key = self._get_bucket_and_key(ts, satellite, exact_match=True)
                ...
                logger.debug(f"S3Store.download succeeded for {satellite.name} at {ts.isoformat()}")
                return result
            except Exception as e:
                logger.error(f"Error in S3Store.download for {satellite.name} at {ts.isoformat()}: {e}")
                logger.error(traceback.format_exc())
                raise
    ...
    try:
        ...
        def logged_browse_directory(self):
            ...
            try:
                ...
                logger.debug(f"Current directory: {current_dir}, exists: {current_dir.exists()}")
                ...
                logger.error(f"Error in EnhancedIntegrityCheckTab._browse_directory: {e}")
                logger.error(traceback.format_exc())
                raise
        ...
        def logged_start_scan(self):
            ...
            try:
                ...
                logger.error(f"Error in EnhancedIntegrityCheckTab._start_enhanced_scan: {e}")
                logger.error(traceback.format_exc())
                raise
        ...
        logger.info("EnhancedIntegrityCheckTab methods successfully patched with logging")
    except ImportError as e:
