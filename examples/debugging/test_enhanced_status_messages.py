...
self.assertIn("color: #66aaf", text) # Blue color
...
self.assertIn("Troubleshooting Tips:", text)
self.assertIn(
"Check if your files follow the correct GOES naming pattern", text
)
...
"Unexpected error fetching or downloading the missing files"
)
mock_set_text.assert_called_once()
...
self.assertIn("font - weight: bold", text)
self.assertIn(
"This may take a few moments depending on the date range", text
)
