...
from goesvfi.integrity_check.enhanced_gui_tab_improved import ImprovedEnhancedIntegrityCheckTab
...
class TestWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.view_model.base_directory = downloads_dir  # Set directory after initialization

        # Add attributes that might be missing for demo purposes
        # Import needed classes from the improved tab
        from goesvfi.integrity_check.enhanced_gui_tab_improved import FetchSource

        # Set missing attributes if needed
        if not hasattr(self.view_model, 'preferred_source'):
            self.view_model.preferred_source = FetchSource.AUTO

        # Set other missing attributes
        if not hasattr(self.view_model, 'can_start_scan'):
            self.view_model.can_start_scan = True

        if not hasattr(self.view_model, 'total_expected'):
            self.view_model.total_expected = 0

        if not hasattr(self.view_model, 'missing_count'):
            self.view_model.missing_count = 0

        if not hasattr(self.view_model, 'is_scanning'):
            self.view_model.is_scanning = False

        if not hasattr(self.view_model, 'is_downloading'):
            self.view_model.is_downloading = False

        if not hasattr(self.view_model, 'status_message'):
            self.view_model.status_message = "Ready"
        
        # Set demo data for testing
        self._set_demo_data()
        
        # Create the improved tab
        self.tab = ImprovedEnhancedIntegrityCheckTab(self.view_model)
        layout.addWidget(self.tab)
        
        # Connect to signals for demonstration
        self.view_model.status_updated.connect(self._log_status)
        self.view_model.scan_completed.connect(self._log_scan_completed)
    ...
    def _set_demo_data(self):
        ...
        # Generate some demo missing items for the table
        missing_items = []
        
        # Create timestamps from yesterday going back, one per hour
