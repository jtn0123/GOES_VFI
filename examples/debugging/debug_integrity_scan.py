...
try:
    from goesvfi.integrity_check.time_index import (
        SatellitePattern, TimeIndex, extract_timestamp_from_directory_name,
        scan_directory_for_timestamps, find_date_range_in_directory
    )
    from goesvfi.integrity_check.enhanced_view_model import (
        EnhancedIntegrityCheckViewModel, FetchSource, AsyncScanTask
    )
...
def test_timestamp_extraction(directory_name: str) -> None:
    ...
    if timestamp:
        ...
        logger.error(f"❌ Failed to extract timestamp from directory name")
    
    # Test through TimeIndex class
    time_index_timestamp = TimeIndex.ts_from_directory_name(directory_name)
    
    if time_index_timestamp:
        logger.info(f"✅ TimeIndex extracted timestamp: {time_index_timestamp.isoformat()}")
    else:
        logger.error(f"❌ TimeIndex failed to extract timestamp from directory name")
...
def test_directory_structure(directory_path: str or Path) -> None:
    ...
    for i, subdir in enumerate(subdirs[:10]):  # Show first 10 subdirectories
        ...
        if ts:
            ...
            logger.info(f"    ❌ No timestamp found in directory name")
...
def test_directory_scanning(directory_path: str or Path, satellite_pattern: SatellitePattern) -> None:
    ...
    logger.info(f"Testing directory scanning for {directory} with pattern {satellite_pattern.name}")
    
    # Test directory structure
    logger.info(f"Directory structure:")
    for item in directory.glob("*"):
        if item.is_dir():
            logger.info(f"  DIR: {item.name} - {len(list(item.glob('*.png')))} PNG files")
    
    # Test timestamp extraction from directory names
    logger.info(f"Testing timestamp extraction from directory names:")
    ...
    logger.info(f"Found {found_timestamps} timestamps in {len(subdirs)} directory names")
    
    # Test full directory scan
    logger.info(f"Testing full directory scan:")
    ...
    logger.info(f"Testing date range detection:")
    ...
    if start_date and end_date:
        ...
        logger.info(f"  ❌ No date range found")
...
async def run_reconcile_manager_scan(
    ...
    logger.info(f"Running ReconcileManager scan asynchronously")
    ...
    try:
        ...
        logger.info(f"ReconcileManager scan completed:")
...
def test_view_model_scan(directory_path: str or Path, satellite_pattern: SatellitePattern) -> None:
    ...
    logger.info(f"Testing view model scan for {directory} with pattern {satellite_pattern.name}")
    ...
    if start_date and end_date:
        ...
        logger.info(f"Set date range: {start_date.isoformat()} to {end_date.isoformat()}")
    else:
        # Use default range (yesterday)
        yesterday = datetime.now() - timedelta(days=1)
        view_model.start_date = yesterday.replace(hour=0, minute=0, second=0, microsecond=0)
        view_model.end_date = yesterday.replace(hour=23, minute=59, second=59, microsecond=0)
        logger.info(f"Using default date range: {view_model.start_date.isoformat()} to {view_model.end_date.isoformat()}")
    ...
    view_model.progress_updated.connect(
        lambda current, total, eta: progress_callback(current, total, f"ETA: {eta:.1f}s")
    )
    ...
    if view_model.missing_items:
        logger.info(f"First missing timestamp: {view_model.missing_items[0].timestamp.isoformat()}")
        logger.info(f"Last missing timestamp: {view_model.missing_items[-1].timestamp.isoformat()}")
    
    # Clean up
    view_model.cleanup()

def test_direct_async_scan(directory_path: str or Path, satellite_pattern: SatellitePattern) -> None:
    ...
    logger.info(f"Testing direct async scan for {directory} with pattern {satellite_pattern.name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--satellite", choices=["GOES_16", "GOES_18", "GOES_17", "GENERIC"], 
                        default="GOES_18", help="Satellite pattern to use")
    parser.add_argument("--test", choices=["extract", "structure", "scan", "model", "direct", "all"],
