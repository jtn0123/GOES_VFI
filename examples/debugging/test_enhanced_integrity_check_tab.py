...
AsyncSignalWaiter(self.tab.directory_selected)
...
AsyncSignalWaiter(self.tab.date_range_changed)

# Call the method under test
self.tab._auto_detect_date_range()

# Verify auto_detect_date_range was called with correct parameters
mock_auto_detect.assert_called_once_with(self.base_dir, self.mock_view_model.satellite)

# Verify date pickers were updated
self.assertEqual(self.tab.start_date_picker.date().toPyDate(), detected_start.date())
self.assertEqual(self.tab.end_date_picker.date().toPyDate(), detected_end.date())
