...
from goesvfi.integrity_check.visual_date_picker import (  # Define a simple Satellite enum for demonstration class Satellite(Enum):
    TimelinePickerWidget,
    VisualDateRangePicker,
)

GOES16 = auto() GOES18 = auto() class ImprovedUIDemo(QMainWindow): """Main demo window for the improved UI components.""" def __init__(self): super().__init__() self.setWindowTitle("Integrity Check UI Components Demo") self.resize(1200, 800) # Create central widget central_widget = QWidget() self.setCentralWidget(central_widget) # Create layout main_layout = QVBoxLayout(central_widget) # Create thread - safe cache for demo thread_safe_cache = ThreadLocalCacheDB() # Create view model downloads_dir = os.path.expanduser("~/Downloads") self.view_model = EnhancedIntegrityCheckViewModel(cache_db=thread_safe_cache) self.view_model.base_directory = downloads_dir # Set up attributes for demo - use private attributes directly self.view_model._can_start_scan = True self.view_model._total_expected = 0 self.view_model._missing_count = 0 self.view_model._is_scanning = False self.view_model._is_downloading = False self.view_model._status_message = "Ready" # Set demo data self._generate_demo_data() # Create tabs for different components tabs = QTabWidget() # Tab 1: Date Selection date_tab = self._create_date_selection_tab() tabs.addTab(date_tab, "Date Selection") # Tab 2: Timeline Visualization timeline_tab = self._create_timeline_tab() tabs.addTab(timeline_tab, "Timeline Visualization") # Tab 3: Results Organization results_tab = self._create_results_tab() tabs.addTab(results_tab, "Results Organization") # Add tabs to layout main_layout.addWidget(tabs) # Status bar self.statusBar().showMessage("Ready") def _generate_demo_data(self) -> None: """Generate demo data for the visualizations.""" # Create timestamps spanning the last 7 days, one per hour self.view_model.start_date = datetime.now() - timedelta(days=7) self.view_model.end_date = datetime.now() # Generate missing items missing_items = [] # Create timestamps from the last 7 days, one per hour base_time = datetime.now() - timedelta(days=7) end_time = datetime.now() total_hours = int((end_time - base_time).total_seconds() / 3600) self.view_model._total_expected = total_hours # Use private attribute for i in range(total_hours): timestamp = base_time + timedelta(hours=i) # Create missing timestamp with different statuses expected_filename = ( f"OR_ABI - L1b - RadF - M6C13_G16_{timestamp.strftime('%Y % j % H % M % S')}.nc" ) item = MissingTimestamp(timestamp, expected_filename) # Set status based on pattern if i % 10 == 0: # Downloaded item.is_downloaded = True item.local_path = f"/tmp / GOES / OR_ABI - L1b - RadF - M6C13_G16_{timestamp.strftime('%Y % j % H % M % S')}.nc" elif i % 10 == 1: # Downloading item.is_downloading = True elif i % 10 == 2: # Error item.download_error = ( "Connection timeout while downloading from S3 bucket"
elif i % 10 >= 7:
     pass
# Missing
pass
