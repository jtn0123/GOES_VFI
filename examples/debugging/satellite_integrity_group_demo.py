...
from PyQt6.QtWidgets import (
    ...
    QTabWidget, QSplitter, QPushButton, QLabel, QComboBox, QFrame
)
from PyQt6.QtCore import Qt, QSize

# Import dark mode styling
from goesvfi.integrity_check.optimized_dark_mode import apply_optimized_dark_palette, OPTIMIZED_DARK_STYLESHEET
...
class SatelliteIntegrityGroupDemo(QMainWindow):
    ...
    def __init__(self):
        ...
        self.satellite_integrity_tabs.dateRangeSelected.connect(self._handle_date_range_selected)
        self.satellite_integrity_tabs.timestampSelected.connect(self._handle_timestamp_selected)
        self.satellite_integrity_tabs.itemSelected.connect(self._handle_item_selected)
        self.satellite_integrity_tabs.downloadRequested.connect(self._handle_download_requested)
    ...
    def _generate_demo_data(self) -> None:
        ...
        for i in range(total_hours):
            ...
            expected_filename = f"OR_ABI-L1b-RadF-M6C13_G16_{timestamp.strftime('%Y%j%H%M%S')}.nc"
            ...
            if i % 10 == 0:
                ...
                # Error
                item.download_error = "Connection timeout while downloading from S3 bucket"
            elif i % 10 >= 7:
                # Missing
                pass
            elif i % 10 == 3:
                # Another error
                item.download_error = "404 Not Found: The specified key does not exist [Error 404]"
            elif i % 10 == 4:
                # Another error
            ...
            setattr(item, 'satellite', Satellite.GOES16 if i % 2 == 0 else Satellite.GOES18)
        ...
        self.view_model._total_expected = total_hours  # Make sure total_expected is set correctly
    ...
    def _handle_date_range_selected(self, start: datetime, end: datetime) -> None:
        ...
        """
        self.statusBar().showMessage(f"Date range selected: {start.strftime('%Y-%m-%d %H:%M')} to {end.strftime('%Y-%m-%d %H:%M')}")
    ...
    def _handle_timestamp_selected(self, timestamp: datetime) -> None:
        ...
        """
        self.statusBar().showMessage(f"Timestamp selected: {timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def _handle_item_selected(self, item: MissingTimestamp) -> None:
        ...
        """
        self.statusBar().showMessage(f"Item selected: {item.timestamp.strftime('%Y-%m-%d %H:%M:%S')} - {item.expected_filename}")
    ...
    def _simulate_scan(self) -> None:
        ...
        self.statusBar().showMessage("Scan simulation triggered (would normally scan for missing files)")
    
    def _simulate_download(self) -> None:
        """Simulate a download action from the integrity tab."""
        self.statusBar().showMessage("Download simulation triggered (would normally download missing files)")
    
    def _simulate_integrity_check(self) -> None:
        """Simulate an integrity check action."""
        self.statusBar().showMessage("Integrity check simulation triggered (would check file integrity)")
    
    def _simulate_visualization(self) -> None:
        """Simulate a visualization action from the imagery tab."""
        self.statusBar().showMessage("Visualization simulation triggered (would display selected data)")
