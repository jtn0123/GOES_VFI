...
def extract_channel_from_netcdf(netcdf_path: Path, channel_var: str = "Rad") -> np.ndarray:
    ...
    with xr.open_dataset(netcdf_path) as ds:
        ...
        if channel_var in ds.variables:
            ...
            metadata = {}
            for attr in ['band_id', 'band_wavelength', 'instrument_type', 'platform_ID']:
                if attr in ds.attrs:
...
def process_and_save_image(data: np.ndarray, output_path: Path, 
                           colormap: str = 'gray', invert: bool = True,
                           min_val: float = None, max_val: float = None,
                           scale_factor: float = 0.25) -> Path:
    """
...
def process_channel_data(ds, channel_num, temp_dir):
    ...
    if var_name in ds.variables:
        ...
        logger.info(f"Channel {channel_num}: Min={min_val}, Max={max_val}, Robust Min={robust_min}, Robust Max={robust_max}")
...
def test_download_and_process_channels():
    ...
    if not channel_tests:
        ...
        channel_tests = [
            ("ABI-L1b-RadF/2024/362/00/OR_ABI-L1b-RadF-M6C01_G18_s20243620000208_e20243620009516_c20243620009561.nc", 1)
        ]
