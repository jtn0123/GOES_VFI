...
def create_sanchez_lut():
    ...
    for i in range(256):
        ...
        # Interpolate
        lut[i] = np.round(
            np.array(colors[idx1]) * (1 - frac) + np.array(colors[idx2]) * frac
        ).astype(np.uint8)
...
def build_s3_cmip_wildcard(satellite, sector, band, year, doy, hour):
    ...
    for code in sector_codes:
        ...
        pattern = (
            f"OR_ABI-L2-CMIP{code}-M6C{band:02d}_G{satellite_num}"
    ...
    try:
        ...
        for page in pages:
            ...
            for obj in page["Contents"]:
                key = obj["Key"]
                if any(
                    re.search(pattern.replace("*", ".*"), key) for pattern in patterns
                ):
    ...
    try:
        ...
        for i, file_path in enumerate(file_paths):
            ...
            with xr.open_dataset(file_path) as ds:
                ...
                print(
                    f"  Loaded Band {band_num} from {os.path.basename(file_path)}, shape: {data.shape}"
                )
