        ...
        with xr.open_dataset(file_path) as ds:
            ...
            for var_name, var in ds.variables.items():
                ...
                if var_name == "CMI":
                    ...
                    print(f"    Mean: {var.values.mean()}, Median: {np.median(var.values)}")
                elif var_name == "CMI_C01_C02_C03":
                    print(f"    Shape: {var.shape}, Min: {var.values.min()}, Max: {var.values.max()}")
            
            # Print key attributes
            print("\nKey Attributes:")
            for attr_name in ["instrument_type", "platform_ID", "scene_id", "spatial_resolution"]:
            ...
            for var_name in ["CMI", "CMI_C01_C02_C03"]:
                if var_name in ds.variables:
                    ...
                    print(f"  {var_name} has scale_factor: {has_scale}, add_offset: {has_offset}")
                    if has_scale:
