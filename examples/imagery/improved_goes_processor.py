...
def build_s3_cmip_wildcard(satellite, sector, band, year, doy, hour):
    ...
    for code in sector_codes:
        ...
        pattern = (
            f"OR_ABI-L2-CMIP{code}-M6C{band:02d}_G{satellite[-2:]}"
...
def build_s3_mcmip_wildcard(satellite, sector, year, doy, hour):
    ...
    for code in sector_codes:
        ...
        pattern = (
            f"OR_ABI-L2-MCMIP{code}-M6_G{satellite[-2:]}"
            f"_s{year}{doy:03d}{hour:02d}*.nc"
        )
        patterns.append(pattern)
...
def build_s3_rrqpe_wildcard(satellite, sector, year, doy, hour):
    ...
    for code in sector_codes:
        ...
        pattern = (
            f"OR_ABI-L2-RRQPE{code}-M6_G{satellite[-2:]}"
            f"_s{year}{doy:03d}{hour:02d}*.nc"
        )
        patterns.append(pattern)
    ...
    try:
        ...
        for i, file_path in enumerate(file_paths):
            with xr.open_dataset(file_path) as ds:
                ...
                print(
                    f"  Loaded Band {i+1} from {os.path.basename(file_path)}, shape: {data.shape}"
                )
