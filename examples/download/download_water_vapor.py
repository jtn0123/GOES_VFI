...
def process_water_vapor_image(file_path, output_path, colormap="jet", min_temp=210, max_temp=280):
    """
...
def download_and_process_water_vapor(date_str, hour, satellite="GOES16", sector="F", 
                                  output_dir=None, colormap="jet"):
    """
    ...
    print(f"Downloading GOES water vapor imagery for {date_str} (DoY {doy}) {hour:02d}:00 UTC")
    ...
    if not files:
        print(f"  No water vapor files found")
        return None
...
def main():
    parser = argparse.ArgumentParser(description="Download and process GOES water vapor imagery")
