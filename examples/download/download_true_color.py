#!/usr/bin/env python3
"""
Download and process GOES true color imagery with optimal patterns.
This script demonstrates the correct patterns for accessing GOES imagery,
particularly focusing on proper handling of band numbers in filenames.
"""
import argparse
import os
import re
...
def find_common_timestamp(files_by_band):
    ...
    common_timestamps = set(
        timestamps_by_band[list(timestamps_by_band.keys())[0]].keys()
    )
...
def process_true_color(band_files, output_path):
    """
    Process true color image from separate band files.

    Key insights:
    1. Band 2 has 2x resolution compared to Bands 1 and 3
    2. Need to apply gamma correction for natural appearance
    3. Contrast enhancement improves visual appeal
    """
    print(f"Processing true color image from band files")
    ...
    try:
        ...
        for band, file_path in sorted(band_files.items()):
            with xr.open_dataset(file_path) as ds:
                ...
                print(
                    f"    Shape: {data.shape}, Range: {np.nanmin(data):.3f} - {np.nanmax(data):.3f}"
                )
        ...
        if len(set(tuple(shape) for shape in band_shapes)) > 1:
            ...
            for i, data in enumerate(band_data):
                if data.shape[0] == min_height and data.shape[1] == min_width:
                    ...
                    resized = img.resize(
                        (min_width, min_height), Image.Resampling.LANCZOS
                    )
...
def download_and_process_true_color(
    date_str, hour, satellite="GOES16", sector="F", output_dir=None
):
    """
    Main function to download and process GOES true color imagery.
    ...
    print(
        f"Downloading GOES true color imagery for {date_str} (DoY {doy}) {hour:02d}:00 UTC"
    )
    ...
    output_path = output_dir / f"{satellite}_{sector}_truecolor_{timestamp}.png"
    success = process_true_color(downloaded_files, output_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download and process GOES true color imagery"
    )
    parser.add_argument(
        "--date",
        type=str,
    ...
    if result:
        print(f"\nSuccess! True color image saved to:")
        print(f"  {result}")
    else:
