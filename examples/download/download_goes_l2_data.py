...
async def download_and_process_cmip(satellite, product_type, date_str, hour="12", minutes=None):
    ...
    if minutes is None:
        if product_type == "Full Disk":
            ...
            minutes = ["01", "06", "11", "16", "21", "26", "31", "36", "41", "46", "51", "56"]
    ...
    for minute in minutes:
        ...
        if not files:
            logger.warning(f"No files found for {satellite} {product_type} at {hour}:{minute}")
            continue
        ...
        if success:
            ...
            # 1. True color image
            truecolor_path = product_dir / f"{satellite_abbr}_{product_name}_{hour}{minute}_truecolor.png"
            success_tc = save_truecolor_image(nc_path, truecolor_path)
            
            # 2. IR image (Band 13)
            ir_path = product_dir / f"{satellite_abbr}_{product_name}_{hour}{minute}_ir13.png"
...
async def download_and_process_rainfall(satellite, product_type, date_str, hour="12", minutes=None):
    ...
    for minute in minutes:
        ...
        if not files:
            logger.warning(f"No rainfall files found for {satellite} {product_type} at {hour}:{minute}")
            continue
        ...
        if success:
            ...
            rain_path = product_dir / f"{satellite_abbr}_{product_name}_{hour}{minute}_rainfall.png"
...
async def download_mesoscale_regions(satellite, date_str, hour="12"):
    ...
    test_minutes = ["00", "01", "02", "03", "04", "05", "30", "31", "32", "33", "34", "35"]
    ...
    for file_key in m1_files[:3]:
        ...
        if success:
            ...
            if "_s" in base_filename:
                ...
                # Process the file to extract images
                truecolor_path = m1_dir / f"{satellite_abbr}_M1_{minute_sec}_truecolor.png"
    ...
    for file_key in m2_files[:3]:
        ...
        if success:
            ...
            if "_s" in base_filename:
                ...
                # Process the file to extract images
                truecolor_path = m2_dir / f"{satellite_abbr}_M2_{minute_sec}_truecolor.png"
    ...
    for satellite, products in results.items():
        ...
        for product_type, counts in products.items():
            if product_type == "CMIPM":
                ...
                logger.info(f"  Mesoscale ({product_type}): M1 success: {m1_success}, M2 success: {m2_success}")
                total_success += m1_success + m2_success
            else:
                success = counts.get("success", 0)
                failed = counts.get("failed", 0)
                logger.info(f"  {product_type}: {success}/{success + failed} files processed successfully")
                total_success += success
                total_failed += failed
        
        logger.info(f"  Total: {total_success}/{total_success + total_failed} files processed successfully")
    
    logger.info(f"\nAll downloads completed. Files saved to: {DOWNLOAD_DIR}")
    logger.info("Check the subdirectories for pre-processed PNG images extracted from the NetCDF files")
