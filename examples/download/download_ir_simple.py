#!/usr/bin/env python3
"""
Download and process GOES infrared (Band 13) imagery without colorization.
This script demonstrates proper handling of IR data with grayscale visualization.
"""
import boto3
...
def process_ir_image(file_path, output_path, min_temp=180, max_temp=320):
    """
    Process IR image from a CMIP file with grayscale visualization.
    ...
    try:
        with xr.open_dataset(file_path) as ds:
            ...
            Image.fromarray(gray_data, "L").save(output_path)
...
def download_and_process_ir(date_str, hour, satellite="GOES16", sector="F", output_dir=None):
    """
    ...
    if sector in ["M1", "M2"]:
        product = f"{base_product}M{sector[-1]}"
    elif sector == "M":
        product = f"{base_product}M"  # Generic mesoscale
    else:
        product = f"{base_product}{sector}"
    
    print(f"Downloading GOES IR (Band 13) imagery for {date_str} (DoY {doy}) {hour:02d}:00 UTC")
    ...
    # Process IR image with grayscale
    output_path = output_dir / f"{satellite}_{sector}_ir_{timestamp}.png"
    result = process_ir_image(nc_path, output_path)
    
    return result

def main():
    parser = argparse.ArgumentParser(description="Download and process GOES IR imagery")
    ...
    parser.add_argument("--sector", choices=["F", "C", "M1", "M2", "M"], default="F",
                      help="Sector code (F=Full Disk, C=CONUS, M/M1/M2=Mesoscale) (default: F)")
    ...
    # If specified M1 or M2 and failed, try generic M
    if not result and args.sector in ["M1", "M2"]:
        print("\nTrying generic Mesoscale (M) instead of specific sector...")
        result = download_and_process_ir(
            args.date,
            args.hour,
            args.satellite,
