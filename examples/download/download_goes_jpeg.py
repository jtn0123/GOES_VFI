...
def try_recent_timestamps(
    satellite: str, sector: str, image_type: str, resolution: str, hours_back: int = 24
) -> Optional[str]:
    """
    ...
    """
    print(
        f"Looking for {satellite} {SECTORS[sector]} {IMAGE_TYPES[image_type]} images..."
    )
    ...
    for h in range(1, hours_back + 1):
        for m in range(0, 60, 10):  # Every 10 minutes
            ...
            try:
                ...
                if response.status_code == 200:
                    print(
                        f"Found image from {timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download GOES satellite images from NOAA's CDN"
    )
    parser.add_argument(
        "--satellite",
        choices=SATELLITES,
