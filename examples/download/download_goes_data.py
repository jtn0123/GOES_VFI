...
PRODUCT_TYPES = {"RadF": "Full Disk", "RadC": "CONUS", "RadM": "Mesoscale"}
...
async def download_mesoscale(
satellite,
bands,
date_str,
hour="12",
minutes=["00", "01", "02", "03", "15", "30", "31", "32", "45"],
):
...
all_files = await list_s3_files(bucket_name, prefix)
matching_files = [
f for f in all_files if band_str in f and f.endswith(".nc")
]

if matching_files:
     pass
logger.info(
f"Found {len(matching_files)} {band_str} files in {prefix}"
)
...
logger.info(
f"File {os.path.basename(file_key)} appears to be a {region} file based on timestamp {timestamp_match}"
)
...
logger.info(
f" {product_name} ({product_type}): {success}/{total} files downloaded successfully"
)
total_success += success
total_failed += failed

logger.info(
f" Total: {total_success}/{total_success + total_failed} files downloaded successfully"
)
