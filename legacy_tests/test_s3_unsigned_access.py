            ...
            for bucket in buckets:
                try:
                    ...
                    if "Contents" in response and response["Contents"]:
                        logger.info(
                            f"Success! Found {len(response['Contents'])} objects in {bucket}"
                        )
                        for obj in response["Contents"]:
                            logger.info(f"  - {obj['Key']} ({obj['Size']} bytes)")
                    else:
                        logger.warning(
                            f"No objects found in {bucket} with prefix {prefix}"
                        )
...
async def main():
    ...
    try:
        ...
        print(
            f"aioboto3 unsigned access: {'SUCCESS' if aioboto3_success else 'FAILED'}"
        )
