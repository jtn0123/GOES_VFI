...
from PyQt6.QtGui import QKeyEvent, QPalette, QColor, QFont
...
class TestAccessibility:
    ...
    def test_screen_reader_compatibility(self, qtbot, window):
        ...
        # Check main buttons
        buttons_to_check = [
            (window.main_tab.in_dir_button, "Input Directory", "Select directory containing input images"),
            (window.main_tab.out_file_button, "Output File", "Choose location for output video file"),
            (window.main_tab.start_button, "Start Processing", "Begin video interpolation process"),
            (window.main_tab.crop_button, "Crop Selection", "Define region of interest for processing"),
            (window.main_tab.clear_crop_button, "Clear Crop", "Remove current crop selection"),
        ]
        ...
        # Check input fields
        input_fields = [
            (window.main_tab.in_dir_edit, "Input Directory Path", "Path to directory containing input images"),
            (window.main_tab.out_file_edit, "Output File Path", "Path for output video file"),
        ]
        ...
        # Check combo boxes
        combos = [
            (window.main_tab.encoder_combo, "Encoder Selection", "Choose video encoding method"),
            (window.main_tab.sanchez_res_combo, "Enhancement Resolution", "Select enhancement resolution"),
        ]
    ...
    def test_tooltip_accuracy(self, qtbot, window):
        ...
        QToolTip.showText(window.main_tab.in_dir_button.mapToGlobal(window.main_tab.in_dir_button.rect().center()),
        ...
        for i in range(1, len(expected_order)):
            ...
            if current_focus != expected_order[i]:
                ...
                if not found:
                    ...
                    print(f"Tab order: Expected {expected_order[i].__class__.__name__}, "
