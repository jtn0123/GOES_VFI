...
class TestPerformanceUIOptimizedV2:
    ...
    def shared_monitoring_components(self):
        ...
        class ThreadPoolMonitor:
            ...
            def submit_task(self, task_func, task_id=None):
            ...
            def _run_task(self, task_func, task_id, submit_time):
                def wrapper():
            ...
            def get_throughput_stats(self):
                ...
                avg_wait_time = sum(self.queue_wait_times.values()) / len(self.queue_wait_times) if self.queue_wait_times else 0
                
                return {
                    'avg_task_time': avg_task_time,
                    'avg_wait_time': avg_wait_time,
                    'tasks_per_second': 1 / avg_task_time if avg_task_time > 0 else 0,
                    'efficiency': (avg_task_time / (avg_task_time + avg_wait_time)) if (avg_task_time + avg_wait_time) > 0 else 1.0
                }
    ...
    def test_ui_responsiveness_comprehensive_datasets(self, qtbot, main_window, shared_monitoring_components) -> None:
        ...
        for num_items, batch_size, description in dataset_scenarios:
            ...
            p99_frame_time = perf_monitor.get_percentile(99)
            performance_score = perf_monitor.get_performance_score()
            
            assert avg_fps > 20, f"FPS too low ({avg_fps:.1f}) for: {description}"
            assert p95_frame_time < 200, f"95th percentile frame time too high ({p95_frame_time:.1f}ms) for: {description}"
            ...
            scroll_patterns = [
                ...
                ("incremental", lambda: [file_list.verticalScrollBar().setValue(file_list.verticalScrollBar().value() + 100)]),
    ...
    def test_non_blocking_comprehensive_operations(self, qtbot, main_window, shared_monitoring_components) -> None:
        ...
        for operation_type, iterations, delay, description in operation_scenarios:
            ...
            class HeavyWorker(QThread):
                ...
                def run(self):
                    ...
                    for i in range(self.iterations):
                        ...
                        self.progress.emit(i + 1, f"Processing {operation_type} {i + 1}/{self.iterations}", self.metrics)
            
            # Connect worker
            worker = HeavyWorker(iterations, delay)
            progress_updates = []
            
            def update_progress(value, message, metrics):
            ...
            assert avg_response_time < 0.15, f"Average response time too high ({avg_response_time:.3f}s) for: {description}"
    ...
    def test_startup_and_initialization_comprehensive(self, qtbot, shared_monitoring_components) -> None:
        ...
        for scenario_name, debug_mode, description in startup_scenarios:
            ...
            assert len(responsiveness_results) > 0 and responsiveness_results[0], f"Not responsive after startup for: {description}"
    ...
    def test_animation_and_visual_performance(self, qtbot, main_window, shared_monitoring_components) -> None:
        ...
        for target_sequence, description in animation_scenarios:
            ...
            if frame_metrics:
                ...
                assert frame_consistency < 30, f"Frame time inconsistency too high ({frame_consistency:.1f}ms) for: {description}"
    ...
    def test_thread_pool_comprehensive_management(self, qtbot, main_window, shared_monitoring_components) -> None:
        ...
        for max_threads, task_count, task_duration, description in thread_pool_scenarios:
            ...
            # Performance benchmarks
            assert total_time < theoretical_min_time * 2, f"Execution too slow ({total_time:.2f}s vs theoretical {theoretical_min_time:.2f}s) for: {description}"
            assert efficiency > 0.3, f"Thread pool efficiency too low ({efficiency:.2f}) for: {description}"
            
            if throughput_stats:
                assert throughput_stats['efficiency'] > 0.5, f"Thread efficiency too low ({throughput_stats['efficiency']:.2f}) for: {description}"
                assert throughput_stats['tasks_per_second'] > 5, f"Throughput too low ({throughput_stats['tasks_per_second']:.2f} tasks/s) for: {description}"
    ...
    def test_lazy_loading_comprehensive_performance(self, qtbot, main_window, shared_monitoring_components) -> None:
        ...
        component_scenarios = [
            ...
            ("medium_widget", lambda: self._create_medium_widget(), 50, "Medium complexity component"),
            ("heavy_widget", lambda: self._create_heavy_widget(), 10, "Heavy component"),
            ("data_model", lambda: self._create_heavy_model(), 5, "Data model component"),
        ]
        ...
        for component_name, factory_func, access_count, description in component_scenarios:
            ...
            assert first_load_time > avg_cache_time * 10, f"Load time not significantly higher than cache time for: {description}"
    ...
    def test_ui_freeze_comprehensive_prevention(self, qtbot, main_window, shared_monitoring_components) -> None:
        ...
        for strategy_name, strategy_func, description in processing_strategies:
            # Create detector
            ...
            # Performance benchmarks
            assert freeze_stats['total_freezes'] <= 2, f"Too many freezes ({freeze_stats['total_freezes']}) for: {description}"
            ...
            if freeze_stats['total_freezes'] > 0:
                assert freeze_stats['max_duration'] < 1.0, f"Freeze too long ({freeze_stats['max_duration']:.3f}s) for: {description}"
                assert freeze_stats['avg_duration'] < 0.5, f"Average freeze too long ({freeze_stats['avg_duration']:.3f}s) for: {description}"
            
            # Strategy should complete in reasonable time
            max_strategy_time = 10.0  # 10 seconds max
            assert strategy_duration < max_strategy_time, f"Strategy took too long ({strategy_duration:.2f}s) for: {description}"
    ...
    def _test_chunked_processing(self, qtbot):
        ...
        for i in range(0, total_items, chunk_size):
            # Process chunk
            chunk_data = []
            for j in range(min(chunk_size, total_items - i)):
                # Simulate work
                chunk_data.append(sum(range(100)))
            
            # Yield control to UI
            QApplication.processEvents()

    def _test_event_driven_processing(self, qtbot):
    ...
    def _test_background_processing(self, qtbot):
    ...
    def _test_yielding_processing(self, qtbot):
