...
class TestSettingsAdvancedOptimizedV2:
    ...
    def test_profile_management_comprehensive(self, qtbot, main_window, temp_settings_workspace) -> None:
        ...
        for scenario in profile_test_scenarios:
            ...
            assert loaded_settings["encoder"] == scenario["settings"]["encoder"], f"Encoder mismatch for {scenario['name']}"
    ...
    def test_window_state_comprehensive_persistence(self, qtbot, main_window, mock_settings_components) -> None:
        ...
        for x, y, width, height, maximized, fullscreen, description in window_states:
            ...
            # Restore state
            restored_state = state_mgr.restore_window_state()
        ...
        restored_state = state_mgr.restore_window_state(fallback_geometry=(300, 200, 1000, 700))
    ...
    def test_recent_items_comprehensive_management(self, qtbot, main_window) -> None:
        ...
        class RecentItemsManager:
            ...
            def add_item(self, category, item, metadata=None):
            ...
            def _cleanup_tracking(self, current_items):
    ...
    def test_settings_migration_comprehensive(self, qtbot, main_window, temp_settings_workspace) -> None:
        ...
        class SettingsMigrator:
            ...
            def validate_migration(self, old_settings, new_settings):
                ...
                for key in critical_keys:
                    if key in old_settings:
                        ...
                        if not found:
                            raise ValueError(f"Critical setting '{key}' lost in migration")
                return True
            
            def _find_key_in_nested(self, data, key):
    ...
    def test_settings_import_export_comprehensive(self, qtbot, main_window, temp_settings_workspace, mock_settings_components) -> None:
        ...
        class SettingsPortability:
            ...
            def validate_import_file(self, filepath):
                ...
                try:
                    ...
                    return {"valid": False, "error": f"File parsing error: {str(e)}"}
        ...
        for template, format_type, expected_sections in export_scenarios:
            ...
            assert export_result["success"], f"Export failed for {template}/{format_type}: {export_result.get('reason', 'unknown')}"
        ...
        for test_case in import_test_cases:
            ...
            if test_case["should_succeed"]:
                assert import_result["success"], f"Import should have succeeded for {test_case['filename']}: {import_result.get('error', 'unknown')}"
    ...
    def test_auto_save_and_preferences_comprehensive(self, qtbot, main_window, mock_settings_components) -> None:
        ...
        class AutoSaveManager:
            ...
            def mark_changed(self, setting_name, old_value, new_value):
        ...
        class PreferencesReset:
            ...
            def reset_to_defaults(self, categories=None, confirm=True):
                ...
                if confirm:
                    category_text = "all preferences" if len(categories) == len(self.default_values) else ", ".join(categories)
        ...
        def fps_changed(value):
            auto_save.mark_changed("fps", 30, value)
        
        def encoder_changed(text):
        ...
        initial_fps = window.main_tab.fps_spinbox.value()
