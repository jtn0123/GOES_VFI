...
from PyQt6.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QMainWindow, QSplitter,
    QPushButton, QLabel, QHBoxLayout, QGridLayout
)
from PyQt6.QtCore import QTimer, Qt, QSize, QBuffer, QIODevice, QByteArray
...
def create_mock_image(width=600, height=500, channel=13, product="FULL_DISK", is_enhanced=False):
    ...
    draw.text((10, height-25), f"{product} • {timestamp}", 
        ...
        if 7 <= channel <= 10:  # Water vapor
            ...
            for y in range(height):
                for x in range(width):
                    dist = np.sqrt(((x - earth_center[0]) / earth_radius)**2 + 
...
def apply_mocks():
    ...
    patches = [
        patch('goesvfi.integrity_check.sample_processor.SampleProcessor', MockSampleProcessor),
        patch('goesvfi.integrity_check.enhanced_imagery_tab.SampleProcessor', MockSampleProcessor),
    ]
