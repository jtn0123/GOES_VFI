...
def create_mock_image(width=600, height=500, channel=13, enhanced=False):
    ...
    for y in range(height):
        ...
        if channel <= 6:  # Visible bands - blue/white gradient
            draw.line([(0, y), (width, y)], fill=(150 + color_value//2, 150 + color_value//2, 220 - color_value))
            ...
            if enhanced:
                ...
                if y < height / 3:
                    ...
                    draw.line([(0, y), (width, y)], fill=(0, 100 + color_value//2, 100 + color_value))
            else:
...
def create_fallback_preview(channel=13, error_msg="No data found for the selected date/time"):
    ...
    draw.text((width//2 - 95, 40), "STATUS: PREVIEW UNAVAILABLE", fill=(255, 255, 255), font=header_font)
    ...
    draw.text((20, y_pos), f"Channel: {channel_name} | Product: FULL_DISK", fill=(255, 255, 255), font=body_font)
    y_pos += 20
    draw.text((20, y_pos), f"Date/Time: {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC", 
    ...
    draw.text((20, y_pos), "You can still proceed with processing. The system will use fallback imagery samples.", 
              fill=(100, 200, 255), font=body_font)
    
    # Draw action button at bottom
    button_y = height - 35
    draw.rectangle([(width//2 - 130, button_y), (width//2 + 130, button_y + 25)], 
                   fill=(0, 120, 120), outline=(255, 255, 255))
    draw.text((width//2 - 120, button_y + 5), "Click 'Process Full Image' to continue", 
             fill=(255, 255, 255), font=body_font)
...
def apply_mocks():
    ...
    # Apply all patches
    patches = [
        patch('goesvfi.integrity_check.sample_processor.SampleProcessor', mock_sample_processor_class),
        patch('goesvfi.integrity_check.enhanced_imagery_tab.SampleProcessor', mock_sample_processor_class),
    ]
...
def main():
    ...
    try:
        ...
        timeout_btn.clicked.connect(lambda: show_fallback_preview(
            "Connection timeout after 30 seconds when connecting to NOAA server"
        ))
        
        nodata_btn = QPushButton("No Data Found")
        nodata_btn.clicked.connect(lambda: show_fallback_preview(
            "No data found for the selected date/time"
        ))
        
        corrupt_btn = QPushButton("Invalid Data")
        corrupt_btn.clicked.connect(lambda: show_fallback_preview(
        ...
        QTimer.singleShot(100, lambda: show_fallback_preview("No data found for the selected date/time"))
