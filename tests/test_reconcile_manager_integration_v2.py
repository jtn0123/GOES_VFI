...
class TestReconcileManagerIntegrationV2:
    ...
    def _configure_manager_methods(self, manager):
        ...
        async def mock_fetch_missing_files(missing_timestamps, satellite, destination_dir, progress_callback=None, item_progress_callback=None):
            results = {}
        ...
        async def mock_reconcile(directory, satellite, start_time, end_time, interval_minutes, progress_callback=None, file_callback=None):
    ...
    @pytest.mark.asyncio
    async def test_scan_directory_with_mock_files(self, mock_reconcile_manager, mock_satellite, test_timestamps):
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("missing_count,expected_results", [
        (2, 2),
        (4, 4),
        (0, 0),
    ])
    async def test_fetch_missing_files_scenarios(self, mock_reconcile_manager, mock_satellite, missing_count, expected_results):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_full_process(self, mock_reconcile_manager, mock_satellite, test_timestamps):
    ...
    @pytest.mark.asyncio
    async def test_cache_db_integration(self, mock_reconcile_manager, mock_satellite):
        """Test cache database integration."""
        test_timestamp = datetime(2023, 1, 1, 12, 0, 0)
        
        # Test timestamp existence check
        exists = await mock_reconcile_manager.cache_db.timestamp_exists(
        ...
        # Test cache update
        await mock_reconcile_manager.cache_db.update_timestamp(
            timestamp=test_timestamp,
            satellite=mock_satellite
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("store_type,is_recent", [
        ("cdn", True),
        ("s3", False),
    ])
    async def test_store_selection_logic(self, mock_reconcile_manager, mock_satellite, store_type, is_recent):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_scenarios(self, mock_reconcile_manager, mock_satellite):
        ...
        with pytest.raises(Exception, match="S3 download failed"):
            await mock_reconcile_manager.s3_store.download(
                test_timestamp, 
                mock_satellite, 
                Path("/mock/dest")
            )
    ...
    @pytest.mark.asyncio
    async def test_progress_callback_accuracy(self, mock_reconcile_manager, mock_satellite):
        """Test accuracy of progress callback reporting."""
        missing_count = 4
        missing_timestamps = [
            datetime(2023, 1, 1, 12, i * 15, 0)
            for i in range(missing_count)
        ]
        ...
        def detailed_progress_callback(current, total, message):
    ...
    @pytest.mark.asyncio
    async def test_file_callback_reliability(self, mock_reconcile_manager, mock_satellite):
    ...
    @pytest.mark.asyncio
    async def test_local_path_generation(self, mock_reconcile_manager, mock_satellite):
    ...
    @pytest.mark.asyncio
    async def test_integration_workflow_validation(self, mock_reconcile_manager, mock_satellite, test_timestamps):
