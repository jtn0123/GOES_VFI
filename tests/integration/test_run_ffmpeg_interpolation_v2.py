...
class TestRunFFmpegInterpolationV2:
    ...
    def test_ffmpeg_interpolation_successful_execution(self, tmp_path, input_factory, mock_ffmpeg_runner, default_ffmpeg_params):
        ...
        def create_output(cmd, desc, monitor_memory=False):
    ...
    @pytest.mark.parametrize("fps,num_frames,crop_rect", [
        (24, 1, (0, 0, 8, 8)),
        (30, 2, (1, 1, 6, 6)),
        (60, 3, (2, 2, 4, 4)),
    ])
    def test_ffmpeg_interpolation_parameter_variations(self, tmp_path, input_factory, mock_ffmpeg_runner, default_ffmpeg_params, fps, num_frames, crop_rect):
    ...
    def test_ffmpeg_interpolation_with_unsharp_filter(self, tmp_path, input_factory, mock_ffmpeg_runner, default_ffmpeg_params):
    ...
    @pytest.mark.parametrize("crf_value,bitrate_params", [
    ...
    def test_ffmpeg_interpolation_quality_presets(self, tmp_path, input_factory, mock_ffmpeg_runner, default_ffmpeg_params, crf_value, bitrate_params):
    ...
    def test_ffmpeg_interpolation_debug_mode(self, tmp_path, input_factory, mock_ffmpeg_runner, default_ffmpeg_params):
    ...
    def test_ffmpeg_interpolation_memory_monitoring(self, tmp_path, input_factory, mock_ffmpeg_runner, default_ffmpeg_params):
        """Test FFmpeg interpolation with memory monitoring enabled."""
        fake_run, captured_commands = mock_ffmpeg_runner
        input_dir = input_factory(tmp_path, count=2)
        output_file = tmp_path / "memory_monitored.mp4"

        def create_output_with_monitoring(cmd, desc, monitor_memory=False):
            fake_run(cmd, desc, monitor_memory)
            output_file.write_text("video_content")
    ...
    def test_ffmpeg_interpolation_error_handling(self, tmp_path, input_factory, default_ffmpeg_params):
        """Test FFmpeg interpolation error handling scenarios."""
        input_dir = input_factory(tmp_path, count=2)
        output_file = tmp_path / "error_output.mp4"

        # Mock FFmpeg command failure
        def failing_run(cmd, desc, monitor_memory=False):
    ...
    def test_ffmpeg_interpolation_input_validation(self, tmp_path, mock_ffmpeg_runner, default_ffmpeg_params):
    ...
    def test_ffmpeg_interpolation_complex_filter_chain(self, tmp_path, input_factory, mock_ffmpeg_runner, default_ffmpeg_params):
    ...
    def test_ffmpeg_interpolation_pixel_format_variations(self, tmp_path, input_factory, mock_ffmpeg_runner, default_ffmpeg_params, pix_fmt):
