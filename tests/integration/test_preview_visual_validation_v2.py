...
class TestPreviewVisualValidationV2:
    ...
    def mock_main_window_setup(self, shared_app):
        ...
        def create_mock_window():
            with patch("goesvfi.utils.settings.sections.BasicSettings.apply_values") as mock_apply:
                def mock_apply_values(target_object, values):
    ...
    def test_preview_image_visibility_scenarios(self, shared_app, test_image_factory, mock_main_window_setup, image_config):
    ...
    def _simulate_preview_display(self, mock_window, test_image):
        ...
        is_size_valid = (
            min_size[0] <= preview_size[0] <= max_size[0] and
            min_size[1] <= preview_size[1] <= max_size[1]
        )
    ...
    def test_preview_size_validation_comprehensive(self, shared_app, test_image_factory, mock_main_window_setup):
        ...
        for test_case in size_test_cases:
            test_image = test_image_factory(
                filename=f"size_test_{test_case['size'][0]}x{test_case['size'][1]}.png",
                size=test_case["size"]
            )
    ...
    def test_preview_color_accuracy_validation(self, shared_app, test_image_factory, mock_main_window_setup):
        ...
        for test_case in color_test_cases:
            test_image = test_image_factory(
                filename=f"color_{test_case['name']}.png",
    ...
    def test_preview_scaling_behavior(self, shared_app, test_image_factory, mock_main_window_setup):
    ...
    def test_preview_memory_efficiency(self, shared_app, test_image_factory, mock_main_window_setup):
    ...
    def test_preview_error_handling(self, shared_app, test_image_factory, mock_main_window_setup, error_scenario):
        ...
        with patch("PIL.Image.open") as mock_pil_open:
            if error_scenario == "corrupted_image":
                mock_pil_open.side_effect = IOError("Cannot identify image file")
    ...
    def test_preview_performance_monitoring(self, shared_app, test_image_factory, mock_main_window_setup):
    ...
    def test_preview_concurrent_loading(self, shared_app, test_image_factory, mock_main_window_setup):
        """Test concurrent preview loading behavior."""
        mock_window = mock_main_window_setup()
        
        # Create multiple test images
        test_images = [
            test_image_factory(filename=f"concurrent_{i}.png", size=(400, 300))
            for i in range(3)
        ]
    ...
    def test_preview_widget_integration(self, shared_app, mock_main_window_setup):
