...
from goesvfi.integrity_check.enhanced_view_model import EnhancedIntegrityCheckViewModel, FetchSource
...
class TestIntegrityTabsIntegration(PyQtAsyncTestCase):
    ...
    async def test_directory_selection_propagation(self):
        ...
        self.assertEqual(timeline_dir, test_dir, "Timeline tab did not receive correct directory")
        self.assertEqual(results_dir, test_dir, "Results tab did not receive correct directory")
        
        # Verify directory was updated in view model
        self.assertEqual(str(self.view_model.base_directory), test_dir, 
    ...
    async def test_date_range_auto_detection(self):
        ...
        with patch('goesvfi.integrity_check.time_index.TimeIndex.find_date_range_in_directory', 
            ...
            self.assertEqual(start.date(), start_date.date(), 
                            "Start date not correctly auto-detected")
            self.assertEqual(end.date(), end_date.date(), 
                            "End date not correctly auto-detected")
            
            # Verify the date range was updated in the view model
            self.assertEqual(self.view_model.start_date.date(), start_date.date(),
                            "View model start date not updated")
            self.assertEqual(self.view_model.end_date.date(), end_date.date(),
    ...
    async def test_data_propagation_after_scan(self):
        ...
        self.assertEqual(len(items), len(missing_items),
        ...
        self.assertEqual(self.view_model.missing_items, missing_items,
    ...
    async def test_date_range_synchronization(self):
        ...
        self.assertEqual(received_start, start_date, "Start date not correctly propagated")
        self.assertEqual(received_end, end_date, "End date not correctly propagated")
        
        # Verify the view model was updated
        self.assertEqual(self.view_model.start_date, start_date, 
                         "View model start date not updated")
        self.assertEqual(self.view_model.end_date, end_date, 
                         "View model end date not updated")
