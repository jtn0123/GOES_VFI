...
class TestPreviewWorkflowIntegrationOptimizedV2:
    ...
    def integration_test_suite() -> dict[str, Any]:
        ...
        class WorkflowTestManager:
            ...
            def _setup_preview_workflow(self, main_window, test_data):
                ...
                return [
                    ...
                    (
                        "verify_sanchez",
                        lambda: self._verify_sanchez_state(main_window, test_data.get("sanchez", False)),
                    ),
                ]
            ...
            def _verify_initial_state(self, main_window) -> str:
            ...
            def _verify_crop_buttons_updated(self, main_window) -> str:
            ...
            def _verify_cache_cleared(self, main_window) -> str:
                main_window.sanchez_preview_cache.clear.assert_called()
                return "Cache cleared"

            def _verify_preview_request(self, main_window) -> str:
                main_window.request_previews_update.emit.assert_called()
                return "Preview request verified"

            def _toggle_sanchez(self, main_window, enabled) -> str:
                main_window.main_tab.sanchez_false_colour_checkbox.setChecked(enabled)
                return f"Sanchez toggled to {enabled}"

            def _verify_sanchez_state(self, main_window, expected_state) -> str:
            ...
            def _verify_crop_saved(self, main_window) -> str:
                if main_window.current_crop_rect:
        ...
        class ComponentIntegrationManager:
            ...
            def _test_file_picker_integration(self, main_window, test_data) -> str:
            ...
            def test_integration(self, integration_type, main_window, test_data):
                ...
                if integration_type not in self.integration_scenarios:
                    msg = f"Unknown integration type: {integration_type}"
                    raise ValueError(msg)
    ...
    def test_complete_preview_workflow_comprehensive(
        self, shared_qt_app, integration_test_suite, temp_image_workspace
    ) -> None:
        ...
        for scenario in workflow_scenarios:
            ...
            workflow_results = workflow_manager.execute_workflow(
                scenario["workflow_type"], main_window, scenario["test_data"]
            )
    ...
    def test_component_integration_comprehensive(
        self, shared_qt_app, integration_test_suite, temp_image_workspace
    ) -> None:
        ...
        for scenario in integration_scenarios:
            ...
            try:
                ...
                integration_result = integration_manager.test_integration(
                    scenario["integration_type"], main_window, scenario["test_data"]
                )
    ...
    def test_state_management_and_persistence_comprehensive(
        self, shared_qt_app, integration_test_suite, temp_image_workspace
    ) -> None:
            ...
            for transition_type, transition_value in scenario["transitions"]:
                try:
                    if transition_type == "set_directory":
                        ...
                        main_window.settings.value.return_value = str(
                            workspace["image_directories"]["preview_workflow"]
                        )
    ...
    def test_error_handling_and_edge_cases_comprehensive(
        self, shared_qt_app, integration_test_suite, temp_image_workspace
    ) -> None:
            ...
            for action_type, action_value in scenario["test_actions"]:
                try:
                    if action_type == "set_invalid_directory":
                        ...
                        for dir_path in workspace["image_directories"].values():
    ...
    def _verify_state_consistency(self, main_window, scenario_name) -> bool:
        ...
        assert hasattr(main_window.main_tab, "sanchez_false_colour_checkbox"), (
            f"Missing sanchez checkbox in {scenario_name}"
        )
