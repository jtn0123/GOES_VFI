...
class TestVideoProcessingPipelineOptimizedV2:
    ...
    def pipeline_mock_suite(self):
        ...
        class SanchezMockManager:
            ...
            def _create_success_mock(self, output_path):
                def mock_colourise(*args, **kwargs):
            ...
            def _create_failure_mock(self, output_path):
                def mock_colourise(*args, **kwargs):
                    raise RuntimeError("Sanchez enhancement failed")
                return mock_colourise
            
            def _create_file_error_mock(self, output_path):
                def mock_colourise(*args, **kwargs):
                    raise FileNotFoundError("Input file not found for Sanchez")
                return mock_colourise
            
            def _create_permission_error_mock(self, output_path):
                def mock_colourise(*args, **kwargs):
                    raise PermissionError("Permission denied writing Sanchez output")
                return mock_colourise
    ...
    def test_complete_pipeline_comprehensive_scenarios(self, comprehensive_test_images, pipeline_mock_suite, tmp_path) -> None:
        ...
        for scenario in pipeline_scenarios:
            dataset_dir, image_files = test_datasets[scenario["dataset"]]
            ...
            with (
                ...
                if scenario["config"]["sanchez"]:
                    mock_factory = mock_suite["sanchez_manager"].get_mock_factory(
                        scenario["mocks"]["sanchez"]
                    )
                    mock_sanchez.side_effect = lambda *args, **kwargs: mock_factory(
                        tmp_path / "sanchez_output.png"
                    )(*args, **kwargs)
    ...
    def test_pipeline_error_handling_comprehensive(self, comprehensive_test_images, pipeline_mock_suite, tmp_path) -> None:
        ...
        for scenario in error_scenarios:
            ...
            with (
                ...
                if scenario["config"]["sanchez"]:
                    ...
                    mock_sanchez.side_effect = lambda *args, **kwargs: mock_factory(
                        tmp_path / "sanchez_error_output.png"
                    )(*args, **kwargs)
                
                # Expect error to be raised
                with pytest.raises((RuntimeError, subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired, PermissionError)):
                    result_gen = run_vfi(
                        folder=dataset_dir,
                        output_mp4_path=output_file,
                        **scenario["config"]
    ...
    def test_pipeline_performance_and_optimization_comprehensive(self, comprehensive_test_images, pipeline_mock_suite, tmp_path) -> None:
        ...
        for scenario in performance_scenarios:
            ...
            if scenario["test_type"] == "memory":
                ...
                output_file = tmp_path / f"memory_test_output.mp4"
                ...
                output_file = tmp_path / f"timing_test_output.mp4"
                ...
                output_file = tmp_path / f"cleanup_test_output.mp4"
                ...
                def run_concurrent_pipeline(pipeline_id):
    ...
    def test_pipeline_integration_with_different_formats_and_settings(self, comprehensive_test_images, pipeline_mock_suite, tmp_path) -> None:
        ...
        for scenario in integration_scenarios:
            dataset_dir, image_files = test_datasets[scenario["dataset"]]
            output_file = tmp_path / f"integration_{scenario['name'].replace(' ', '_').lower()}.{scenario['output_format']}"
