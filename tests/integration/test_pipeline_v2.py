...
class TestPipelineOptimizedV2:
    ...
    def comprehensive_mock_suite(self):
        ...
        class SanchezMockManager:
            ...
            def _create_failure_mock(self, output_path):
                def mock_colourise(*args, **kwargs):
                    raise RuntimeError("Sanchez processing failed")
                return mock_colourise
            ...
            def _create_permission_error_mock(self, output_path):
                def mock_colourise(*args, **kwargs):
                    raise PermissionError("Permission denied writing output file")
                return mock_colourise
    ...
    def test_pipeline_comprehensive_scenarios(self, temp_workspace, comprehensive_mock_suite, pipeline_test_constants) -> None:
        """Test comprehensive pipeline scenarios with different configurations."""
        workspace = temp_workspace
        mock_suite = comprehensive_mock_suite
        constants = pipeline_test_constants
        ...
        for scenario in pipeline_scenarios:
            with (
                ...
                mock_detector = mock_suite["rife_manager"].create_mock_detector(
                    scenario["rife_capabilities"]
                )
                ...
                if scenario["sanchez"]:
                    ...
                    mock_colourise.side_effect = lambda *args, **kwargs: mock_factory(
                        workspace["output_dir"] / "sanchez_output.png"
                    )(*args, **kwargs)
    ...
    def test_pipeline_error_handling_comprehensive(self, temp_workspace, comprehensive_mock_suite, pipeline_test_constants) -> None:
        ...
        for scenario in error_scenarios:
            with (
                ...
                if scenario["should_raise"]:
                    with pytest.raises((RuntimeError, subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired)):
                        result_gen = run_vfi(
                            folder=workspace["input_dir"],
                            output_mp4_path=workspace["output_file"],
                            fps=30,
        ...
        for scenario in processing_scenarios:
            if scenario["test_type"] == "loader":
                ...
                loaded_images = loader.load_images_from_directory(
                    workspace["input_dir"],
                    supported_formats=scenario["image_formats"]
                )
    ...
    def test_pipeline_performance_and_optimization(self, temp_workspace, comprehensive_mock_suite) -> None:
        ...
        for scenario in performance_scenarios:
            if scenario["test_type"] == "memory":
                ...
                initial_temp_files = len(list(workspace["output_dir"].glob("*"))) + len(list(workspace["input_dir"].glob("*")))
                ...
                final_temp_files = len(list(workspace["output_dir"].glob("*"))) + len(list(workspace["input_dir"].glob("*")))
