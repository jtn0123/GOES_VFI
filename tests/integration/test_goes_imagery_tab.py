...
from goesvfi.integrity_check.goes_imagery_tab import (
    GOESImageryTab, ImageSelectionPanel, ImageViewPanel
)
...
class TestImageSelectionPanel(unittest.TestCase):
    ...
    def test_initial_state(self):
        ...
        self.assertEqual(
            self.panel.resolution_combo.currentData(),
            "2.7k"
        )
        self.assertEqual(
            self.panel.processing_combo.currentData(),
            ProcessingMode.BASIC
        )
        self.assertEqual(
            self.panel.size_combo.currentData(),
            "1200"
        )
