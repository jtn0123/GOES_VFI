...
class TestPreviewCropWorkflowV2:
    ...
    def test_complete_preview_crop_workflow(self, shared_app, mock_main_window, test_image_factory, temp_dir_factory, workflow_config):
    ...
    def test_preview_update_workflow(self, shared_app, mock_main_window, test_image_factory):
        ...
        try:
            ...
            for test_image in test_images:
                ...
                with patch("PIL.Image.open") as mock_open:
                    ...
                    preview_size = (200, 150)  # Thumbnail size
    ...
    def test_crop_selection_workflow(self, shared_app, mock_main_window, test_image_factory):
        ...
        for scenario in crop_scenarios:
            crop_result = self._execute_crop_selection_workflow(
                main_window, test_image, scenario
            )
    ...
    def _execute_crop_selection_workflow(self, main_window, test_image, crop_scenario):
        ...
        try:
            ...
            is_valid = (
                crop_region["width"] >= min_size and
                crop_region["height"] >= min_size and
                crop_region["width"] <= max_size and
                crop_region["height"] <= max_size
            )
    ...
    def test_workflow_error_handling(self, shared_app, mock_main_window, test_image_factory):
        ...
        error_scenarios = [
            ...
            "crop_application_failure"
        ]
        
        for scenario in error_scenarios:
            error_result = self._test_workflow_error_scenario(
                main_window, test_image, scenario
            )
    ...
    def _test_workflow_error_scenario(self, main_window, test_image, scenario):
        ...
        try:
            if scenario == "image_load_failure":
                with patch("PIL.Image.open", side_effect=IOError("Image load failed")):
    ...
    def test_workflow_performance_monitoring(self, shared_app, mock_main_window, test_image_factory):
        """Test workflow performance monitoring and optimization."""
        # Create test images for performance testing
        test_images = [
            test_image_factory(f"perf_{i:03d}.png", size=(800, 600))
            for i in range(10)
        ]
    ...
    def test_workflow_state_persistence(self, shared_app, mock_main_window, test_image_factory):
        """Test workflow state persistence and restoration."""
        test_image = test_image_factory("state_test.png")
