...
async def test_s3_patterns(timestamp, satellite_pattern, dest_dir):
    ...
    for product_type in product_types:
        ...
        if not nearest_times:
            ...
            results[product_type] = {"success": False, "error": "No valid scan times found"}
            continue
        ...
        for ts in nearest_times:
            ...
            for band in bands:
                ...
                try:
                    ...
                    key = TimeIndex.to_s3_key(ts, satellite_pattern, product_type=product_type, band=band)
                    
                    LOGGER.info(f"Trying to access s3://{bucket}/{key} -> {dest_path}")
                    
                    # Check if file exists
                    exists = await s3_store.exists(ts, satellite_pattern, product_type=product_type, band=band)
                    attempt["exists"] = exists
                    
                    if exists:
                        # Download the file
                        LOGGER.info(f"File exists! Downloading...")
                        result_path = await s3_store.download(ts, satellite_pattern, dest_path, 
                        ...
                        if result_path.exists():
                            ...
                            LOGGER.info(f"✓ Successfully downloaded file: {result_path} ({file_size} bytes)")
                            
                            # Success for this product type
                            product_result["success"] = True
                        else:
                            attempt["error"] = "File reported as downloaded but doesn't exist"
...
async def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for satellite, results in all_results.items():
            ...
            LOGGER.info(f"{satellite}: {len(successful_products)}/{len(results)} successful")
            LOGGER.info(f"  Successful: {', '.join(successful_products) if successful_products else 'None'}")
            LOGGER.info(f"  Failed: {', '.join(failed_products) if failed_products else 'None'}")
