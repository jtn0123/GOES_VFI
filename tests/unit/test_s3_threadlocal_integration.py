...
from goesvfi.integrity_check.time_index import (
    ...
    RADF_MINUTES, RADC_MINUTES, RADM_MINUTES
)
...
class TestS3ThreadLocalIntegration(unittest.TestCase):
    ...
    def test_concurrent_downloads_with_threadlocal_cache(self):
        ...
        date_ranges = [
            ...
            (self.old_date - timedelta(days=1), self.old_date - timedelta(days=1) + timedelta(minutes=50)),  # S3 range
            (self.recent_date - timedelta(days=1), self.recent_date - timedelta(days=1) + timedelta(minutes=50))  # CDN range
        ]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for start_date, end_date in date_ranges:
                futures.append(executor.submit(
                    self._run_reconcile_in_thread, start_date, end_date
                ))
        ...
        self.assertGreater(len(self.thread_id_to_db), 1, 
        ...
        self.assertGreater(total_timestamps, 0, 
                        f"Expected timestamps, got {total_timestamps}")
    ...
    def test_different_product_types_with_threadlocal_cache(self):
        ...
        # Verify multiple thread IDs were used
        self.assertGreaterEqual(len(self.thread_id_to_db), len(product_types), 
    ...
    def test_threadlocal_cache_stress_test(self):
        ...
        self.assertGreaterEqual(len(self.thread_id_to_db), 2, 
        ...
        for thread_id, timestamps in self.thread_id_to_db.items():
            self.assertGreater(len(timestamps), 0, 
                              f"Thread {thread_id} processed 0 timestamps")
        ...
        self.assertGreaterEqual(len(all_timestamps), 50, 
...
for name in dir(TestS3ThreadLocalIntegration):
    if name.startswith('_get_') and asyncio.iscoroutinefunction(getattr(TestS3ThreadLocalIntegration, name)):
        setattr(TestS3ThreadLocalIntegration, name, async_test(getattr(TestS3ThreadLocalIntegration, name)))
