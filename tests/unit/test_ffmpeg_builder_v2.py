    ...
    def ffmpeg_test_components(self):
        ...
        class CommandBuilderTestManager:
            ...
            def _test_two_pass_builds(self, temp_workspace):
                ...
                assert "-c:v" in cmd_pass1 and "libx265" in cmd_pass1
                assert "-b:v" in cmd_pass1 and "1000k" in cmd_pass1
                assert "pass=1" in " ".join(cmd_pass1)
                assert "-f" in cmd_pass1 and "null" in cmd_pass1
                ...
                assert "-c:v" in cmd_pass2 and "libx265" in cmd_pass2
                assert "-b:v" in cmd_pass2 and "1000k" in cmd_pass2
            ...
            def _test_hardware_builds(self, temp_workspace):
                ...
                for config_name in ["hardware_hevc", "hardware_h264"]:
                    ...
                    codec_index = cmd.index("-c:v") + 1
                    assert cmd[codec_index] == config["expected_codec"]
                    assert "-b:v" in cmd and f"{config['bitrate']}k" in cmd
                    assert "-maxrate" in cmd and f"{config['bufsize']}k" in cmd
                    
                    if "tag" in config:
                        assert "-tag:v" in cmd and config["tag"] in cmd
        ...
        class CommandValidator:
            ...
            def validate_command(self, cmd: List[str], validation_types: List[str] = None) -> Dict[str, Any]:
        ...
        for scenario in builder_scenarios:
            try:
                ...
                scenario_results = test_manager.run_test_scenario(
                    scenario["test_type"], temp_workspace
                )
                ...
                if scenario["name"] == "Basic Software Encoder Builds":
                    ...
                    for encoder in scenario["expected_encoders"]:
                        found_encoder = any(
                            result.get("has_codec") for result in scenario_results.values()
                        )
    ...
    def test_ffmpeg_builder_specific_command_validation(self, ffmpeg_test_components, temp_workspace) -> None:
        ...
        validation_scenarios = [
            ...
            },
            {
                ...
                "expected_elements": ["ffmpeg", "-c:v", "hevc_videotoolbox", "-tag:v", "hvc1", "-b:v", "2000k", "-maxrate", "4000k"],
            },
            {
    ...
    def test_ffmpeg_builder_parameter_combinations(self, ffmpeg_test_components, temp_workspace) -> None:
        ...
        combination_scenarios = [
            ...
            },
            {
                ...
                "params": {"encoder": "Hardware HEVC (VideoToolbox)", "bitrate": 5000, "bufsize": 10000, "pix_fmt": "yuv420p"},
                "expected_quality": "high",
            },
            {
                "name": "Low Bitrate Hardware",
                "params": {"encoder": "Hardware H.264 (VideoToolbox)", "bitrate": 500, "bufsize": 1000, "pix_fmt": "yuv420p"},
                "expected_quality": "low",
            },
        ]
        ...
        for scenario in combination_scenarios:
            ...
            if "crf" in params:
                assert "-crf" in cmd and str(params["crf"]) in cmd, f"CRF {params['crf']} not found in {scenario['name']}"
            if "bitrate" in params:
                assert "-b:v" in cmd and f"{params['bitrate']}k" in cmd, f"Bitrate {params['bitrate']} not found in {scenario['name']}"
            if "bufsize" in params:
                assert "-maxrate" in cmd and f"{params['bufsize']}k" in cmd, f"Bufsize {params['bufsize']} not found in {scenario['name']}"
            
            assert "-pix_fmt" in cmd and params["pix_fmt"] in cmd, f"Pixel format {params['pix_fmt']} not found in {scenario['name']}"
            
            # Validate overall command structure
            validation_results = validator.validate_command(cmd, ["basic_structure", "codec_settings", "quality_settings"])
            
            assert validation_results["basic_structure"]["has_ffmpeg"], f"Command structure invalid for {scenario['name']}"
