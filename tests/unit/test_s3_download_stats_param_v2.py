...
class TestS3DownloadStatsParamOptimizedV2:
    ...
    def s3_stats_test_components(self):
        ...
        class S3DownloadStatsTestManager:
            ...
            def _test_basic_stats_update(self, scenario_name: str, **kwargs) -> Dict[str, Any]:
                ...
                if scenario_name == "single_update":
                    ...
                    for i, (success, error_type, download_time, file_size, error_message) in enumerate(self.test_scenarios):
                        self.reset_stats()
                    ...
                    for error_type, error_message, count in error_scenarios:
                        for i in range(count):
                            update_download_stats(
                                success=False,
                                error_type=error_type,
                                error_message=f"{error_message} {i}"
                            )
                            total_errors += 1
        ...
        return {
            "manager": S3DownloadStatsTestManager()
        }
    ...
    @pytest.mark.parametrize("success,error_type,download_time,file_size,error_message", [
        (True, None, 1.0, 100, None),
        (False, "timeout", 5.0, 0, "Connection timeout"),
        (True, None, 0.5, 200, None),
        (False, "network", 2.0, 0, "Network error"),
    ...
    def test_update_download_stats_scenarios(self, s3_stats_test_components, success, error_type, download_time, file_size, error_message) -> None:
