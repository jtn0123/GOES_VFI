...
class TestRealS3Patterns(unittest.TestCase):
    ...
    def test_radf_minutes_pattern(self) -> None:
        """Test if RadF minutes pattern matches expected schedule."""
        assert RADF_MINUTES == [0, 10, 20, 30, 40, 50]
        # Intervals should be 10 minutes apart
        for i in range(len(RADF_MINUTES) - 1):
            assert RADF_MINUTES[i + 1] - RADF_MINUTES[i] == 10

    def test_radc_minutes_pattern(self) -> None:
        """Test if RadC minutes pattern matches expected schedule."""
        assert RADC_MINUTES == [1, 6, 11, 16, 21, 26, 31, 36, 41, 46, 51, 56]
        # Intervals should be 5 minutes apart
        for i in range(len(RADC_MINUTES) - 1):
            assert RADC_MINUTES[i + 1] - RADC_MINUTES[i] == 5

    def test_radm_minutes_pattern(self) -> None:
        """Test if RadM minutes pattern matches expected schedule."""
        # Should include all minutes 0-59
        assert list(range(60)) == RADM_MINUTES
        assert len(RADM_MINUTES) == 60

    def test_to_s3_key_radf_format(self) -> None:
        ...
        assert key.startswith("ABI-L1b-RadF/2023/166/12/")
        ...
        if match:
            ...
            assert minute_str == "00"
        else:
            self.fail("Could not find timestamp pattern in key")

    def test_to_s3_key_radc_format(self) -> None:
        ...
        if match:
            minute_str = match.group(1)
            assert minute_str == "11"
        else:
            self.fail(f"Could not find timestamp pattern in key: {key}")

    def test_to_s3_key_band_format(self) -> None:
    ...
    def test_to_s3_key_wildcard_handling(self) -> None:
        ...
        assert "*" not in key_without_wildcards
    ...
    def test_filter_s3_keys_by_band(self) -> None:
        ...
        assert len(band13_keys) == 2
        for key in band13_keys:
            assert "C13_" in key

        # Filter for Band 1
        band1_keys = filter_s3_keys_by_band(keys, 1)
        assert len(band1_keys) == 1
        assert "C01_" in band1_keys[0]
    ...
    def test_real_file_pattern_examples(self) -> None:
        ...
        for file_pattern in real_patterns:
            if "C13_" in file_pattern:
                ...
                assert len(filtered) == 1, f"Failed to filter band 13 from {file_pattern}"
                assert filtered[0] == file_pattern

    def test_doy_handling_in_keys(self) -> None:
        ...
        assert "/060/" in key_3
        assert "s2024060" in key_3

    def test_nearest_valid_minute_selection(self) -> None:
        ...
        assert "s20231661232" in key_3

    def test_real_examples_from_logs(self) -> None:
