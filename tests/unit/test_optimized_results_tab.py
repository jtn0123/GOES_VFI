...
class TestOptimizedResultsTab(PyQtAsyncTestCase):
    ...
    async def test_set_items(self):
        ...
        self.assertEqual(total_expected_text, str(self.total_expected), 
    ...
    async def test_group_by_status(self):
        ...
        model = self.tab.tree_view.model
        self.assertEqual(model.rowCount(), 4,
                         "Tree view should show 4 status groups")
    ...
    async def test_group_by_satellite(self):
        ...
        self.assertTrue(model.rowCount() >= 2,
                        "Tree view should show at least 2 satellite groups")
    ...
    async def test_item_selection(self):
        ...
        self.assertEqual(received_item, test_item,
                        "Item signal not emitted with correct item")
        
        # The preview widget should be updated
        self.assertEqual(self.tab.preview_widget.current_item, test_item,
    ...
    async def test_view_request(self):
        ...
        # Verify the signal was emitted with the correct item
        self.assertEqual(received_item, test_item,
                        "View signal not emitted with correct item")
    ...
    async def test_summary_widget_update(self):
        ...
        missing_count = len(self.missing_items) - downloaded_count - errors_count - \
                        sum(1 for item in self.missing_items if item.is_downloading)
        
        # Verify counts match
        self.assertEqual(downloaded_text, str(downloaded_count),
                         "Downloaded count doesn't match expected value")
        self.assertEqual(errors_text, str(errors_count),
                         "Errors count doesn't match expected value")
        self.assertEqual(missing_text, str(missing_count),
    ...
    async def test_preview_widget_button_states(self):
        ...
        self.assertTrue(self.tab.preview_widget.download_btn.isEnabled(),
                       "Download button should be enabled for missing item")
        self.assertFalse(self.tab.preview_widget.view_btn.isEnabled(),
        ...
        self.assertFalse(self.tab.preview_widget.download_btn.isEnabled(),
                        "Download button should be disabled for downloaded item")
        self.assertTrue(self.tab.preview_widget.view_btn.isEnabled(),
