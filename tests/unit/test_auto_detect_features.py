...
class TestAutoDetectFeatures(PyQtAsyncTestCase):
    ...
    def test_auto_detect_date_range_goes16(self, mock_message_box):
        ...
        # Call the method
        with patch('goesvfi.integrity_check.enhanced_gui_tab.Qt.CursorShape.WaitCursor'):
    ...
    @patch('goesvfi.integrity_check.enhanced_gui_tab.QMessageBox')
    def test_combined_auto_detect_features(self, mock_message_box, mock_scan, mock_progress_dialog):
        ...
        self.assertEqual(mock_message_box.information.call_count, 1)  # Only from date range detection
    ...
    @patch('goesvfi.integrity_check.time_index.TimeIndex.scan_directory_for_timestamps')
    @patch('goesvfi.integrity_check.enhanced_gui_tab.QMessageBox')
    def test_auto_detect_satellite_with_error(self, mock_message_box, mock_scan, mock_progress_dialog):
    ...
    @patch('goesvfi.integrity_check.enhanced_gui_tab.QProgressDialog')
    @patch('goesvfi.integrity_check.time_index.TimeIndex.scan_directory_for_timestamps')
    def test_auto_detect_satellite_detail_logging(self, mock_scan, mock_progress_dialog):
        ...
        with patch('goesvfi.integrity_check.enhanced_gui_tab.LOGGER') as mock_logger:
            ...
            mock_logger.info.assert_any_call(f"Auto-detect satellite: Found {len(goes16_files)} GOES-16 files and {len(goes18_files)} GOES-18 files")
            
            # Verify the satellite selection was logged
            mock_logger.info.assert_any_call(f"Auto-detect satellite: Selected GOES-18 based on file count ({len(goes18_files)} vs {len(goes16_files)})")
            
            # Verify completion was logged
            mock_logger.info.assert_any_call(f"Auto-detect satellite: Completed successfully, selected GOES-18 (West)")
