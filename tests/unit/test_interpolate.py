...
def test_interpolate_pair_invokes_command_and_file_ops(tmp_path, dummy_img):
    ...
    with patch.object(interpolate_mod, "RifeCommandBuilder") as mock_cmd_builder_cls, \
         ...
         patch("goesvfi.pipeline.interpolate.pathlib.Path.exists", return_value=True) as mock_exists: # Give exists a name
        ...
        with patch('numpy.array', return_value=np.ones((4, 4, 3), dtype=np.uint8)) as mock_np_array:
            ...
            mock_run_patch.assert_called_once_with(expected_rife_cmd, check=True, capture_output=True, text=True)
...
def test_interpolate_pair_raises_on_subprocess_error(tmp_path, dummy_img):
    ...
    with patch.object(interpolate_mod, "RifeCommandBuilder") as mock_cmd_builder_cls, \
         ...
         patch("goesvfi.pipeline.interpolate.shutil.rmtree") as mock_rmtree, \
         patch("goesvfi.pipeline.interpolate.pathlib.Path.exists", return_value=True): # Keep exists mock simple
