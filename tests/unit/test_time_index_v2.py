...
class TestTimeIndexOptimizedV2:
    ...
    @pytest.mark.parametrize(
        "test_time,product_type,expected_intervals",
        [
            # RadF product (10-minute intervals)
            (
                datetime(2023, 6, 15, 12, 15, 0),
                "RadF",
                [datetime(2023, 6, 15, 12, 10, 0), datetime(2023, 6, 15, 12, 20, 0)],
            ),
            # RadC product (5-minute intervals) - assuming 11, 16 minute intervals
            (
                datetime(2023, 6, 15, 12, 15, 0),
                "RadC",
                [datetime(2023, 6, 15, 12, 11, 0), datetime(2023, 6, 15, 12, 16, 0)],
            ),
            # RadM product (1-minute intervals)
            (datetime(2023, 6, 15, 12, 15, 0), "RadM", [datetime(2023, 6, 15, 12, 15, 0)]),
            # Exact match on RadF interval
            (
                datetime(2023, 6, 15, 12, 20, 0),
                "RadF",
                [datetime(2023, 6, 15, 12, 20, 0), datetime(2023, 6, 15, 12, 30, 0)],
            ),
            # Edge case - start of hour
            (
                datetime(2023, 6, 15, 12, 0, 0),
                "RadF",
                [datetime(2023, 6, 15, 12, 0, 0), datetime(2023, 6, 15, 12, 10, 0)],
            ),
        ],
    )
    def test_nearest_interval_finding(self, test_time, product_type, expected_intervals) -> None:
        ...
        for actual, expected in zip(intervals, expected_intervals, strict=False):
