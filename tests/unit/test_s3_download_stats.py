...
class TestS3DownloadStats(unittest.TestCase):
    ...
    def test_update_download_stats_multiple_error_types(self):
        ...
        for error_type, counter_key in error_types.items():
            ...
            self.assertEqual(DOWNLOAD_STATS[counter_key], 1, 
    ...
    def test_network_diagnostics_on_repeated_failures(self, mock_get_info):
        ...
        update_download_stats(
            success=False,
            error_type='network',
            error_message="Network error 10"
        )
...
for name in dir(TestS3DownloadStatsIntegration):
    if name.startswith('test_') and asyncio.iscoroutinefunction(getattr(TestS3DownloadStatsIntegration, name)):
        setattr(TestS3DownloadStatsIntegration, name, async_test(getattr(TestS3DownloadStatsIntegration, name)))
