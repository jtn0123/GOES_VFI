...
def test_run_vfi_skip_model_writes_all_frames(
    mock_executor, mock_image_open, mock_popen_patch, mock_run_patch, tmp_path, mock_capability_detector
):
    ...
    expected_ffmpeg_cmd = [
        'ffmpeg', '-hide_banner', '-loglevel', 'verbose', '-stats', '-y',
        '-f', 'image2pipe', '-framerate', str(fps), # Effective FPS is just fps when skipping
        '-vcodec', 'png', '-i', '-', '-an', '-vcodec', 'libx264',
        '-preset', 'ultrafast', '-pix_fmt', 'yuv420p',
        '-vf', 'scale=trunc(iw/2)*2:trunc(ih/2)*2', str(raw_output)
    ]
    ...
    def wait_with_file_creation(*wait_args, **wait_kwargs):
        ...
        if raw_output and res == 0:
            try:
                ...
                with open(raw_output, "wb") as f: f.write(b"dummy ffmpeg output")
...
def test_run_vfi_ffmpeg_failure_raises(
    mock_exists, mock_executor, mock_image_open, mock_temp_dir, mock_popen_patch, mock_run_patch, tmp_path, mock_capability_detector
):
    # Setup mock executor
    mock_executor_instance = MagicMock()
    mock_executor.return_value.__enter__.return_value = mock_executor_instance
...
def test_run_vfi_sanchez_invocation(
    mock_executor, mock_image_open, mock_temp_dir, mock_popen_patch, mock_run_patch, mock_colourise_patch, tmp_path, mock_capability_detector
):
    ...
    mock_colourise_patch.side_effect = [mock_colourise_factory_0, mock_colourise_factory_1]
    ...
    mock_run_factory = create_mock_subprocess_run(
        expected_command=[
            str(rife_exe),
            '-0', ANY, # Use ANY for first image path since it might have a timestamp suffix
            '-1', ANY, # Use ANY for second image path since it might have a timestamp suffix
            '-o', ANY, # Actual output path uses a temp dir, match ANY
            '-m', 'rife-v4.6',
            '-n', '1',
            '-s', '0.5',
            '-g', '-1'
