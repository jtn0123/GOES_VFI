...
def test_stream_copy_success(mock_popen_patch, temp_paths):
    ...
    encode.encode_with_ffmpeg(
        ...
        crf=0, bitrate_kbps=0, bufsize_kb=0, pix_fmt="yuv420p"
    )
...
def test_2pass_x265_calls(mock_temp_file, mock_popen_patch, temp_paths):
    ...
    cmd_pass1 = [
        ...
        "-i", str(intermediate),
        "-c:v", "libx265", "-preset", "slower", "-b:v", f"{bitrate}k",
        "-x265-params", "pass=1", "-passlogfile", pass_log_prefix,
        "-f", "null", os.devnull
    ]
    cmd_pass2 = [
        "ffmpeg", "-hide_banner", "-loglevel", "info", "-stats", "-y",
        "-i", str(intermediate),
        "-c:v", "libx265", "-preset", "slower", "-b:v", f"{bitrate}k",
        "-x265-params", ANY, # Params string can be complex, use ANY
        "-passlogfile", pass_log_prefix, "-pix_fmt", pix_fmt, str(final)
    ]
    ...
    encode.encode_with_ffmpeg(
        ...
        crf=0, bitrate_kbps=bitrate, bufsize_kb=0, pix_fmt=pix_fmt
    )
...
@patch("goesvfi.pipeline.encode.subprocess.Popen") # Patch Popen directly
@pytest.mark.parametrize("encoder, expected_codec, use_crf", [
...
def test_single_pass_encoders(mock_popen_patch, temp_paths, encoder, expected_codec, use_crf):
    ...
    encode.encode_with_ffmpeg(
        ...
        final_output=final,
        encoder=encoder,
        crf=crf, bitrate_kbps=bitrate, bufsize_kb=bufsize, pix_fmt=pix_fmt
    )
