...
def get_timestamp_from_filename(filepath):
    ...
    if '_s' in filename:
        ...
        timestamp = base_date + timedelta(days=int(doy_str)-1, 
                                         hours=int(hour_str),
                                         minutes=int(min_str),
                                         seconds=int(sec_str))
...
def process_true_color(red_file, green_file, blue_file, resolution=None):
    ...
    logger.info(f"Channel shapes - Red: {red_shape}, Green: {green_shape}, Blue: {blue_shape}")
    
    # Find common resolution (minimum for each dimension)
    common_res = (min(s[0] for s in [red_shape, green_shape, blue_shape]),
    ...
    if green_data.shape != common_res:
        from skimage.transform import resize
        green_data = resize(green_data, common_res, anti_aliasing=True, preserve_range=True)
    
    if blue_data.shape != common_res:
        from skimage.transform import resize
        blue_data = resize(blue_data, common_res, anti_aliasing=True, preserve_range=True)
...
def main():
    ...
    parser.add_argument('--true-color', action='store_true', help='Create true color image')
    ...
    parser.add_argument('--resolution', type=str, help='Output resolution (full, half, 2.7k, or specific number)')
    ...
    if args.true_color and args.red and args.green and args.blue:
        ...
        print("Please provide either a single file to process or all three RGB files for true color.")
        parser.print_help()
        return
