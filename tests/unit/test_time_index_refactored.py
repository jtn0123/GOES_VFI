...
class TestExtractTimestampFromDirectoryName(unittest.TestCase):
    ...
    def test_try_extract_time_component(self):
        ...
        # Test with no match
        self.assertIsNone(_try_extract_time_component("test_dir", [patterns[0]]))

        # Test with valid match
        result = _try_extract_time_component("test_dir", [patterns[1]])
        self.assertEqual(result, (12, 30, 0))

        # Test with exception in group extraction
        self.assertIsNone(_try_extract_time_component("test_dir", [patterns[2]]))

    def test_try_primary_datetime_patterns(self):
    ...
    def test_extract_timestamp_from_directory_name(self):
        ...
        with patch('goesvfi.integrity_check.time_index_refactored.date_utils.parse_satellite_path', return_value=None):
            ...
            with patch('goesvfi.integrity_check.time_index_refactored._try_primary_datetime_patterns', return_value=None):
                with patch('goesvfi.integrity_check.time_index_refactored._try_satellite_specific_patterns') as mock_satellite:
            ...
            with patch('goesvfi.integrity_check.time_index_refactored._try_primary_datetime_patterns', return_value=None):
                with patch('goesvfi.integrity_check.time_index_refactored._try_satellite_specific_patterns', return_value=None):
...
class TestScanDirectoryForTimestamps(unittest.TestCase):
    ...
    def test_validate_directory_and_pattern(self):
        ...
        with patch('pathlib.Path.exists', return_value=True):
            with patch('pathlib.Path.is_dir', return_value=True):
                with patch('goesvfi.integrity_check.time_index_refactored.COMPILED_PATTERNS.get', return_value=None):
        ...
        with patch('pathlib.Path.exists', return_value=True):
            with patch('pathlib.Path.is_dir', return_value=True):
                ...
                with patch('goesvfi.integrity_check.time_index_refactored.COMPILED_PATTERNS.get', return_value=mock_pattern):
    ...
    def test_filter_timestamp_by_range(self):
        """Test the _filter_timestamp_by_range helper function."""
        timestamp = datetime(2023, 6, 15, 12, 30, 0)
        
        # Test with no range filters
        self.assertTrue(_filter_timestamp_by_range(timestamp, None, None))
        
        # Test with only start_time
        start_time = datetime(2023, 6, 15, 0, 0, 0)  # Earlier
        self.assertTrue(_filter_timestamp_by_range(timestamp, start_time, None))
        
        start_time = datetime(2023, 6, 16, 0, 0, 0)  # Later
        self.assertFalse(_filter_timestamp_by_range(timestamp, start_time, None))
        
        # Test with only end_time
        end_time = datetime(2023, 6, 16, 0, 0, 0)  # Later
        self.assertTrue(_filter_timestamp_by_range(timestamp, None, end_time))
        
        end_time = datetime(2023, 6, 14, 0, 0, 0)  # Earlier
        self.assertFalse(_filter_timestamp_by_range(timestamp, None, end_time))
        
        # Test with both start_time and end_time
        start_time = datetime(2023, 6, 14, 0, 0, 0)  # Earlier
        end_time = datetime(2023, 6, 16, 0, 0, 0)  # Later
        self.assertTrue(_filter_timestamp_by_range(timestamp, start_time, end_time))
        
        start_time = datetime(2023, 6, 16, 0, 0, 0)  # Later
        end_time = datetime(2023, 6, 17, 0, 0, 0)  # Later
        self.assertFalse(_filter_timestamp_by_range(timestamp, start_time, end_time))
    ...
    def test_scan_files_for_timestamps(self, mock_filter, mock_extract):
...
class TestToS3Key(unittest.TestCase):
    ...
    def test_to_s3_key(self, mock_date_to_doy, mock_detect_env):
        ...
        with patch('goesvfi.integrity_check.time_index_refactored._find_nearest_valid_scan_minute', return_value=30) as mock_find:
