...
class TestRealS3PatternsOptimizedV2:
    ...
    def s3_pattern_test_components(self):
        ...
        class S3PatternTestManager:
            ...
            def validate_key_structure(self, key: str, product_type: str, satellite: SatellitePattern, 
                                     band: int, timestamp: datetime) -> Dict[str, bool]:
            ...
            def _test_minute_schedules(self, scenario_name: str, **kwargs) -> Dict[str, Any]:
                ...
                if scenario_name == "schedule_validation":
                    ...
                    # RadF schedule
                    radf_valid = (
                        RADF_MINUTES == [0, 10, 20, 30, 40, 50] and
                        all(RADF_MINUTES[i+1] - RADF_MINUTES[i] == 10 for i in range(len(RADF_MINUTES)-1))
                    )
                    schedule_results["RadF"] = {
                    ...
                    # RadC schedule
                    radc_valid = (
                        RADC_MINUTES == [1, 6, 11, 16, 21, 26, 31, 36, 41, 46, 51, 56] and
                        all(RADC_MINUTES[i+1] - RADC_MINUTES[i] == 5 for i in range(len(RADC_MINUTES)-1))
                    )
                    schedule_results["RadC"] = {
                    ...
                    # RadM schedule
                    radm_valid = (
                        RADM_MINUTES == list(range(60)) and
                        len(RADM_MINUTES) == 60
                    )
                ...
                if scenario_name == "basic_formats":
                    ...
                    for product_type in self.test_configs["product_types"]:
                        for satellite in self.test_configs["satellites"]:
                            ...
                            validation = self.validate_key_structure(
                                key, product_type, satellite, 13, test_ts
                            )
                    ...
                    for product_type, test_ts, expected_minute in test_cases:
                        key = to_s3_key(test_ts, SatellitePattern.GOES_16, 
            ...
            def _test_band_handling(self, scenario_name: str, **kwargs) -> Dict[str, Any]:
                ...
                if scenario_name == "multiple_bands":
                    ...
                    for band in self.test_configs["bands"]:
                        key = to_s3_key(test_ts, SatellitePattern.GOES_18, 
                                       product_type="RadC", band=band)
                ...
                if scenario_name == "wildcard_generation":
                    ...
                    for exact_match in [True, False]:
                        ...
                        wildcard_results.append({
                            "exact_match": exact_match,
                            "has_wildcards": "*" in key,
                            "key": key
                        })
                    
                    # Verify wildcard behavior
                    assert wildcard_results[0]["exact_match"] and not wildcard_results[0]["has_wildcards"]
                    assert not wildcard_results[1]["exact_match"] and wildcard_results[1]["has_wildcards"]
            ...
            def _test_doy_handling(self, scenario_name: str, **kwargs) -> Dict[str, Any]:
                ...
                if scenario_name == "doy_formatting":
                    ...
                    for test_date, expected_doy in test_cases:
                        key = to_s3_key(test_date, SatellitePattern.GOES_18, 
                                       product_type="RadC", band=13)
                    ...
                    results["all_correct"] = all(
                        d["doy_in_path"] and d["doy_in_timestamp"] for d in doy_results
                    )
                    ...
                    for product_type in self.test_configs["product_types"]:
                        for satellite in self.test_configs["satellites"]:
                            for band in [1, 13]:  # Sample bands
                                ...
                                key = to_s3_key(test_ts, satellite, 
                                               product_type=product_type, band=band)
                                
                                validation = self.validate_key_structure(
                                    key, product_type, satellite, band, test_ts
                                )
    ...
    @pytest.mark.parametrize("product_type,expected_interval", [
        ("RadF", 10),
        ("RadC", 5),
        ("RadM", 1)  # Every minute
    ])
    ...
    def test_s3_patterns_integration_validation(self, s3_pattern_test_components) -> None:
        ...
        for product_type in ["RadF", "RadC", "RadM"]:
            key = to_s3_key(test_timestamp, SatellitePattern.GOES_18, 
                           product_type=product_type, band=13)
