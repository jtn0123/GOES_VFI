...
class TestRealS3PathOptimizedV2:
    ...
    def s3_path_test_components(self):
        ...
        class S3PathTestManager:
            ...
            def generate_filename(self, timestamp: datetime, satellite: SatellitePattern, 
                                product_type: str, band: int) -> str:
            ...
            def validate_s3_pattern(self, bucket: str, key: str, satellite: SatellitePattern, 
                                  product_type: str, timestamp: datetime) -> Dict[str, Any]:
            ...
            def _test_all_product_types(self, scenario_name: str, dest_dir: Path, **kwargs) -> Dict[str, Any]:
                ...
                if scenario_name == "product_type_patterns":
                    ...
                    results["all_valid"] = all(
                        pr["validation"]["all_valid"] for pr in product_results.values()
                    )
                ...
                if scenario_name == "various_timestamps":
                    ...
                    for timestamp in self.test_configs["timestamps"]:
                        ...
                        timestamp_results.append({
                            ...
                            "key_prefix": key_prefix,
                            "key_contains_date": (
                                str(year) in key_prefix and 
                                f"{day_of_year:03d}" in key_prefix and
                                f"{hour:02d}" in key_prefix
                            )
                        })
                ...
                if scenario_name == "comprehensive_validation":
                    ...
                    for satellite in self.test_configs["satellites"]:
                        for product_type in self.test_configs["product_types"]:
                            ...
                            validation = self.validate_s3_pattern(
                                bucket, full_key, satellite, product_type, timestamp
                            )
            ...
            def _test_comprehensive_validation(self, scenario_name: str, dest_dir: Path, **kwargs) -> Dict[str, Any]:
                ...
                if scenario_name == "full_matrix":
                    ...
                    for i, timestamp in enumerate(self.test_configs["timestamps"][:2]):
                        for j, satellite in enumerate(self.test_configs["satellites"]):
                            for k, product_type in enumerate(self.test_configs["product_types"][:2]):
    ...
    @pytest.mark.parametrize("product_type,schedule", [
        ("RadF", RADF_MINUTES),
        ("RadC", RADC_MINUTES),
        ("RadM", RADM_MINUTES)
    ])
