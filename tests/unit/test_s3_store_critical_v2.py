...
class TestS3StoreCriticalOptimizedV2:
    ...
    def s3_store_critical_test_components(self):
        ...
        class S3StoreCriticalTestManager:
            ...
            async def _test_initialization(self, scenario_name: str, **kwargs) -> Dict[str, Any]:
                ...
                if scenario_name == "context_manager":
                    ...
                    async_contexts = []
                    for store in stores:
                        async_contexts.append(store.__aenter__())
                    ...
                    exit_contexts = []
                    for store in stores:
                        exit_contexts.append(store.__aexit__(None, None, None))
            ...
            async def _test_download_operations(self, scenario_name: str, temp_dir, **kwargs) -> Dict[str, Any]:
                ...
                if scenario_name == "successful_download":
                    ...
                    mock_client = self.create_mock_s3_client(
                        head_object_response=self.s3_responses["success"]
                    )
                    
                    path_patches = self.create_path_patches(
                        path_exists=True,
                        file_size=9
                    )
                    ...
                    for i, file_size in enumerate(self.test_configs["file_sizes"]):
                        mock_client = self.create_mock_s3_client(
                            head_object_response={"ContentLength": file_size}
                        )
                ...
                if scenario_name == "high_throughput":
                    ...
                    with patch.object(store, "_get_s3_client", return_value=mock_client):
                        ...
                        for i in range(operation_count):
                            ...
                            task = store.check_file_exists(
                                ts,
                                self.test_configs["satellites"][i % len(self.test_configs["satellites"])]
                            )
                            tasks.append(task)
        ...
        return {
            "manager": S3StoreCriticalTestManager()
        }
    ...
    async def test_wildcard_matching_scenarios(self, s3_store_critical_test_components, temp_directory) -> None:
        ...
        wildcard_scenarios = [
            "wildcard_single_match",
            "wildcard_multiple_matches",
            "wildcard_no_matches"
        ]
    ...
    async def test_satellite_specific_operations(self, s3_store_critical_test_components, temp_directory, satellite) -> None:
        """Test operations with specific satellite patterns."""
        manager = s3_store_critical_test_components["manager"]
        store = manager.create_s3_store()
        
        mock_client = manager.create_mock_s3_client(
            head_object_response=manager.s3_responses["success"]
        )
        
        path_patches = manager.create_path_patches(
            path_exists=True,
            file_size=9
        )
    ...
    @pytest.mark.asyncio
    async def test_s3_store_critical_integration_validation(self, s3_store_critical_test_components, temp_directory) -> None:
