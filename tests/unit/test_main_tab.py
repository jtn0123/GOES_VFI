...
def main_tab(qtbot, mock_main_window_view_model):
    ...
    patch('goesvfi.utils.config.get_project_root', MagicMock(return_value=pathlib.Path("/mock/project/root"))).start()
    patch('goesvfi.utils.config.get_cache_dir', MagicMock(return_value=pathlib.Path("/mock/cache/dir"))).start()
