...
class TestReconcileManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_s3_store.download = AsyncMock(
            return_value=self.base_dir / "test.nc"
        )
    ...
    def test_is_recent(self):
        ...
        try:
            ...
            is_recent = loop.run_until_complete(
                self.manager._is_recent(self.old_date)
            )
    ...
    def test_fetch_missing_files(self, mock_render_png):
        ...
        try:
            ...
            self.mock_cdn_store.exists.assert_called_with(recent_timestamp, SatellitePattern.GOES_16)
            self.mock_s3_store.exists.assert_called_with(old_timestamp, SatellitePattern.GOES_16)
    ...
    @patch('goesvfi.integrity_check.reconcile_manager.ReconcileManager.fetch_missing_files')
    def test_reconcile(self, mock_fetch, mock_scan):
        ...
        try:
            ...
            # Mock scan_directory
            existing = {start_time, start_time.replace(minute=20), start_time.replace(minute=40)}
