...
def create_files(sorter_dirs):
    ...
    return source_dir, files_details_by_output, expected_structure, expected_copied_count
...
class TestFileSorter:
    ...
    def test_basic_sort(self, sorter_dirs, create_files):
        ...
        sorter.sort_files(root_dir=str(source_dir))
    ...
    def test_dry_run(self, sorter_dirs, create_files):
        ...
        assert not converted_dir.exists() or not any(converted_dir.iterdir()) # Check if empty
        assert sorter.files_copied == 0 # No files actually copied
        assert sorter.files_skipped == 0 # Should not skip in dry run before copy attempt
    ...
    def test_duplicate_skip(self, sorter_dirs, create_files):
        ...
        conflicting_output_filename = "imageA_20230115T100000Z.png" # Expected name for the first imageA
    ...
    def test_duplicate_overwrite(self, sorter_dirs, create_files):
        ...
        source_dir, files_details_by_output, expected_structure, expected_copied = create_files
        ...
        original_source_path = files_details_by_output[conflicting_output_filename]['path']
        ...
        assert abs(existing_file.stat().st_mtime - original_source_path.stat().st_mtime) < 2
    ...
    def test_duplicate_rename(self, sorter_dirs, create_files):
        ...
        rename_conflict_name_1 = "imageA_20230115T100000Z_1.png" # What it would rename to first
        ...
        expected_structure_with_rename[base_name_to_conflict] = sorted(
             [f for f in expected_structure[base_name_to_conflict] if f != conflicting_output_filename] +
             [conflicting_output_filename, rename_conflict_name_1, expected_renamed_file.name]
        )

        for base_name, expected_files in expected_structure_with_rename.items():
             base_dir = converted_dir / base_name
             assert base_dir.exists()
             actual_files = sorted([f.name for f in base_dir.iterdir()]) # Sort for comparison
             assert actual_files == sorted(list(set(expected_files))) # Use set to handle potential duplicates in logic, then sort
             total_files_found += len(actual_files)
    ...
    def test_invalid_date_format_string(self, sorter_dirs, create_files):
        ...
        sorter = FileSorter(dry_run=False, duplicate_mode=DuplicateMode.SKIP) # Using SKIP for variety
        sorter.sort_files(root_dir=str(source_dir))
