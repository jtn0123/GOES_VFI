...
class TestOptimizedTimelineTab(PyQtAsyncTestCase):
    ...
    async def test_set_data(self):
        ...
        self.assertEqual(self.tab.start_timestamp, self.start_date,
                        "Start timestamp not set correctly")
        self.assertEqual(self.tab.end_timestamp, self.end_date,
                        "End timestamp not set correctly")
        self.assertEqual(self.tab.missing_items, self.missing_items,
                        "Missing items not set correctly")
        self.assertEqual(self.tab.interval_minutes, self.interval_minutes,
    ...
    async def test_set_date_range(self):
        ...
        self.assertEqual(self.tab.start_timestamp, new_start,
                        "Start timestamp not updated correctly")
        self.assertEqual(self.tab.end_timestamp, new_end,
                        "End timestamp not updated correctly")
    ...
    async def test_set_directory(self):
        ...
        self.assertEqual(received_dir, test_dir,
    ...
    async def test_timestamp_selection(self):
        ...
        self.assertEqual(received_timestamp, test_timestamp,
                        "Timestamp signal not emitted with correct timestamp")
        
        # The selected timestamp should be stored
        self.assertEqual(self.tab.selected_timestamp, test_timestamp,
    ...
    async def test_view_switching(self):
        ...
        self.assertEqual(self.tab.stack.currentIndex(), 0,
                        "Initial view should be timeline")
        ...
        self.assertEqual(self.tab.stack.currentIndex(), 1,
                        "View not switched to calendar")
        
        # The view buttons should reflect the current state
        self.assertFalse(self.tab.view_timeline_btn.isChecked(),
                        "Timeline button should not be checked")
        self.assertTrue(self.tab.view_calendar_btn.isChecked(),
                        "Calendar button should be checked")
        ...
        self.assertEqual(self.tab.stack.currentIndex(), 0,
                        "View not switched back to timeline")
        
        # The view buttons should reflect the current state
        self.assertTrue(self.tab.view_timeline_btn.isChecked(),
                        "Timeline button should be checked")
        self.assertFalse(self.tab.view_calendar_btn.isChecked(),
    ...
    async def test_info_panel_update(self):
        ...
        self.assertIn("No item selected", self.tab.info_label.text(),
        ...
        self.assertNotIn("No item selected", self.tab.info_label.text(),
                        "Info panel should be updated with selection")
        
        # The timestamp should be mentioned in the text
        self.assertIn(selected_timestamp.strftime("%Y-%m-%d"), self.tab.info_label.text(),
