...
def run_test(test_path: str, debug_mode: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if summary_line_match:
             ...
             summary_details = re.findall(r"^(FAILED|ERROR|SKIPPED)\s+.*?::.*?::(.*?)\s+-?.*?$", summary_text, re.MULTILINE)
             # Format as "status test_name"
             summary_details = [f"{status} {name}" for status, name, _ in summary_details if status in ["FAILED", "ERROR"]]
        ...
        if not counts["passed"] and not counts["failed"] and not counts["error"] and (counts["skipped"] > 0 or "collected 0 items" in output):
            status = "SKIPPED"
...
def print_status(result: Dict[str, Any], verbose: bool = False, dump_logs: bool = False, debug_mode: bool = False):
    ...
    if dump_logs and (status == "FAILED" or status == "ERROR" or status == "CRASHED" or ("PASSED" in status and "teardown error" in status)):
        log_path = dump_log_to_file(path, output, debug_mode)
    
    # Print verbose output for failed, error, or crashed tests
    if verbose and (status in ["FAILED", "ERROR", "CRASHED"] or counts.get("failed", 0) > 0 or counts.get("error", 0) > 0):
...
def main():
    ...
    parser.add_argument("--tolerant", action="store_true", help="Always return success (0) even if tests fail or crash")
    parser.add_argument("--dump-logs", action="store_true", help="Dump output logs for crashed and failed tests to files")
    ...
    total_passed = len([r for r in all_results if r["status"] == "PASSED" or r["status"] == "PASSED (with teardown error)" or r["status"] == "PASSED (with crash)"])
