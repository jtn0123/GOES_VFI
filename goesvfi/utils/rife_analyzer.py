    ...
    def _detect_capabilities(self) -> None:
        ...
        if not success:
            if not help_text_str:
                 ...
                 logger.error(f"RIFE help command failed. Help text/error captured: \n{help_text_str}")
        # Optional: Log a warning if command succeeded but output looks bad
        elif not help_text_str or "Error:" in help_text_str or "Timeout" in help_text_str or "No output" in help_text_str:
             logger.warning(f"RIFE help command succeeded but output seems problematic: \n{help_text_str}")
        # --- End Refined Logging --- #
        ...
        version_match = re.search(r'version[:\s]+([0-9.]+)', help_text_str, re.IGNORECASE)
        if version_match:
        ...
        self._capabilities["tiling"] = any(arg in self._supported_args for arg in ["t", "tile"]) or "tile" in help_text_lower
        self._capabilities["uhd"] = any(arg in self._supported_args for arg in ["u", "uhd"]) or "uhd" in help_text_lower
        self._capabilities["tta_spatial"] = any(arg in self._supported_args for arg in ["x", "tta-spatial"]) or "spatial" in help_text_lower
        self._capabilities["tta_temporal"] = any(arg in self._supported_args for arg in ["z", "tta-temporal"]) or "temporal" in help_text_lower
        self._capabilities["thread_spec"] = any(arg in self._supported_args for arg in ["j", "thread"]) or "thread" in help_text_lower
        self._capabilities["batch_processing"] = any(arg in self._supported_args for arg in ["i", "input-pattern"]) or "batch" in help_text_lower
        self._capabilities["timestep"] = any(arg in self._supported_args for arg in ["s", "timestep"]) or "timestep" in help_text_lower
        self._capabilities["model_path"] = any(arg in self._supported_args for arg in ["m", "model"]) or "model" in help_text_lower
        self._capabilities["gpu_id"] = any(arg in self._supported_args for arg in ["g", "gpu"]) or "gpu" in help_text_lower
