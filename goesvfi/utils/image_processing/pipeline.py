...
class ConditionalPipeline(ProcessorBase):
    ...
    def __init__(self, condition_map: Dict[str, ProcessorBase], default_processor: Optional[ProcessorBase] = None):
...
class ParallelPipeline(ProcessorBase):
    ...
    def __init__(self, processors: List[ProcessorBase], combiner_func: Optional[callable] = None):
    ...
    def _default_combiner(
        self, results: List[ImageProcessingResult], input_data: Any, context: Optional[Dict[str, Any]]
    ) -> ImageProcessingResult:
        ...
        if not successful_results:
            ...
            return results[0] if results else ImageProcessingResult.failure_result(
