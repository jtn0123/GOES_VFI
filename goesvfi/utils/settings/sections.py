    ...
    def apply_values(self, target_object: Any, values: Dict[str, Any]) -> None:
        ...
        if model_combo is not None:
            self.accessor.safe_set_value(
                target_object, 'model_combo', values.get("rife_model_key", ""), 'setCurrentText'
            )
        ...
        self.accessor.set_checkbox_checked(main_tab, "rife_tta_temporal_checkbox", values.get("rife_tta_temporal", False))
    
    def get_setting_keys(self) -> List[str]:
        """Get list of settings keys handled by this section."""
        return [
            "fps", "mid_count", "max_workers", "encoder", "rife_model_key",
            "rife_tile_enable", "rife_tile_size", "rife_uhd_mode", "rife_thread_spec",
            "rife_tta_spatial", "rife_tta_temporal"
        ]
...
class SanchezSettings(SettingsSection):
    ...
    def apply_values(self, target_object: Any, values: Dict[str, Any]) -> None:
        ...
        self.accessor.set_checkbox_checked(
            main_tab, "sanchez_false_colour_checkbox", values.get("sanchez_false_colour", False)
        )
        ...
        if sanchez_res_combo is not None:
            self.accessor.safe_set_value(
                target_object, 'sanchez_res_km_combo', values.get("sanchez_res_km", ""), 'setCurrentText'
            )
...
class FFmpegSettings(SettingsSection):
    ...
    def apply_values(self, target_object: Any, values: Dict[str, Any]) -> None:
        ...
        self.accessor.set_checkbox_checked(
            ffmpeg_tab, "use_ffmpeg_interp_checkbox", values.get("ffmpeg_use_interp", False)
        )
        
        # Apply combo box settings
        self.accessor.set_combobox_text(
            ffmpeg_tab, "ffmpeg_filter_preset_combo", values.get("ffmpeg_filter_preset", "")
        )
        self.accessor.set_combobox_text(
    ...
    def get_setting_keys(self) -> List[str]:
        """Get list of settings keys handled by this section."""
        return [
            "ffmpeg_use_interp", "ffmpeg_filter_preset", "ffmpeg_mi_mode",
            "ffmpeg_mc_mode", "ffmpeg_me_mode", "ffmpeg_me_algo",
            "ffmpeg_scd_mode", "ffmpeg_search_param"
        ]
