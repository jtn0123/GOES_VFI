...
class AsyncFileManager(BaseManager):
    ...
    async def batch_write(
        ...
        async def write_single(path: Path, content: str | bytes) -> Path:
            await self.write_file(path, content, mode, encoding, create_parents=create_parents)
            return path
...
async def async_temporary_file(
    suffix: str = "", prefix: str = "goes_vfi_", temp_dir: Path | None = None, mode: str = "w+b"
    ...
    async with aiofiles.tempfile.NamedTemporaryFile(
        suffix=suffix, prefix=prefix, dir=str(temp_dir) if temp_dir else None, mode=mode, delete=False
    ) as temp_file:
...
def get_async_file_manager() -> AsyncFileManager:
    ...
    Returns:
        Global AsyncFileManager instance
    """
    global _async_file_manager  # noqa: PLW0603
