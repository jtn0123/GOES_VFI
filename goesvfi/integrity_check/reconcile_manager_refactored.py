...
class ReconcileManager:
        ...
        async with semaphore:
            ...
            try:
                ...
                if await store.exists(ts, satellite):
                    ...
                    await self._handle_successful_download(ts, satellite, downloaded_path, file_callback)
    ...
    async def fetch_missing_files(
        ...
        self._update_progress(progress_callback, 0, 4, "Step 1/4: Analyzing missing files...")
        LOGGER.info(f"Fetching {total} missing files")
        
        # Group timestamps by recency for efficient store handling
        recent_timestamps, old_timestamps = await self._group_timestamps_by_recency(missing_timestamps)
