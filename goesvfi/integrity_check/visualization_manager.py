...
from .goes_imagery import ChannelType, ProductType, ProcessingMode
...
class VisualizationManager:
    ...
    def __init__(self, base_dir: Optional[Union[str, Path]] = None, satellite: str = "G16") -> None:
        """
    ...
    def get_filename(self, channel: Union[ChannelType, int], is_map: bool = False, 
    ...
    def process_band_image(self, data: NDArray[np.float64],
                           channel: Union[ChannelType, int],
                           colorized: bool = False) -> Image.Image:
        """
        ...
        if channel_num <= 6:  # Reflective bands (visible, near-IR)
            ...
            data_norm = 1.0 - np.clip((data_clean - min_temp) / (max_temp - min_temp), 0, 1)
    ...
    def process_rgb_composite(self, red_data: NDArray[np.float64], green_data: NDArray[np.float64],
                              blue_data: NDArray[np.float64], composite_type: str) -> Image.Image:
        """
    ...
    def save_visualization(self, img: Image.Image, channel: Union[ChannelType, int],
                          timestamp: Optional[datetime] = None, colorized: bool = False) -> Path:
        """
    ...
    def list_available_visualizations(self, 
                                  ...
                                  date_range: Optional[Tuple[datetime, datetime]] = None) -> List[Path]:
        """
    ...
    def get_latest_visualization(self, channel: Union[int, str], colorized: bool = False) -> Optional[Path]:
        """
    ...
    def create_sample_visualization(self, data: NDArray[np.float64], 
                                 channel: Union[ChannelType, int], 
                                 size: Tuple[int, int] = (400, 400)) -> Tuple[Image.Image, Image.Image]:
        """
    ...
    def create_comparison_image(self, 
                             ...
                             titles: List[str], 
                             output_path: Optional[Path] = None) -> Optional[Union[Image.Image, Path]]:
        """
...
class ExtendedChannelType:
    ...
    AIRMASS_RGB = (103, "Airmass RGB", "RGB", "Air mass analysis using water vapor and ozone bands")
    FIRE_RGB = (104, "Fire Temperature RGB", "RGB", "Fire detection using thermal bands")
    CLOUD_PHASE_RGB = (105, "Day Cloud Phase RGB", "RGB", "Cloud type and phase analysis")
