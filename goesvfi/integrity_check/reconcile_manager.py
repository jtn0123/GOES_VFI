...
class ReconcileManager:
    ...
    def __init__(
        ...
        if isinstance(cache_db, CacheDB) and not isinstance(
    ...
    async def scan_directory(
        ...
        if interval_minutes <= 0:
            LOGGER.warning(
                f"Invalid interval minutes: {interval_minutes}, defaulting to 10"
            )
            interval_minutes = 10
        ...
        while current <= end_time and timestamp_count < max_iterations:
            ...
            if timestamp_count % 1000 == 0:
                LOGGER.debug(
                    f"Generated {timestamp_count} timestamps so far, current: {current}"
                )
                if progress_callback:
                    progress_callback(
                        0,
                        5,
                        f"Step 1/5: Generating timestamps ({timestamp_count} so far)...",
                    )

        if timestamp_count >= max_iterations:
            LOGGER.warning(
                f"Reached maximum timestamp limit ({max_iterations}), stopping generation"
            )

        total_files = len(expected_timestamps)
        LOGGER.debug(
            f"Generated {total_files} expected timestamps from {start_time} to {end_time}"
        )

        # Step 2: Checking cache for existing timestamps
        LOGGER.debug(
            f"Starting scan step 2/5: Checking cache for {total_files} timestamps..."
        )
        if progress_callback:
            progress_callback(
                1, 5, f"Step 2/5: Checking cache for {total_files} timestamps..."
            )
        ...
        LOGGER.debug(
            f"Starting scan step 3/5: Found {len(existing_timestamps)}/{total_files} in cache, "
            f"preparing filesystem check..."
        )
        if progress_callback:
            progress_callback(
                2,
                5,
                f"Step 3/5: Found {len(existing_timestamps)}/{total_files} in cache, "
                f"preparing filesystem check...",
            )
        ...
        LOGGER.debug(
            f"Starting scan step 4/5: Checking filesystem for {unchecked_count} timestamps..."
        )
        if progress_callback:
            progress_callback(
                3,
                5,
                f"Step 4/5: Checking filesystem for {unchecked_count} timestamps...",
            )
        ...
        for i, ts in enumerate(sorted(unchecked_timestamps)):
            ...
            if progress_callback:
                ...
                if (
                    i % 10 == 0 or i == unchecked_count - 1
                ):  # Update only every 10 items to reduce UI updates
                    progress_callback(
                        3,
                        5,
                        f"Step 4/5: Checking filesystem ({sub_progress}): {ts.isoformat()}",
                    )
        ...
        # Final step complete
        LOGGER.debug(
            f"Scan completed successfully: {len(missing_timestamps)} files missing out of "
            f"{total_files}"
        )
        if progress_callback:
            progress_callback(
                5,
                5,
                f"Scan complete: {len(missing_timestamps)} files missing out of {total_files}",
            )
    ...
    async def fetch_missing_files(
            ...
            async with semaphore:
                ...
                try:
                    ...
                    if progress_callback:
                        if is_recent:
                            progress_callback(
                                2,
                                4,
                                f"Step 3/4: {exists_message} ({idx+1}/{source_total})",
                            )
                        else:
                            progress_callback(
                                3,
                                4,
                                f"Step 4/4: {exists_message} ({idx+1}/{source_total})",
                            )
                    ...
                    if await store.exists(ts, satellite):
                        ...
                        if progress_callback:
                            if is_recent:
                                progress_callback(
                                    2,
                                    4,
                                    f"Step 3/4: {download_message} ({idx+1}/{source_total})",
                                )
                            else:
                                progress_callback(
                                    3,
                                    4,
                                    f"Step 4/4: {download_message} ({idx+1}/{source_total})",
                                )

                        # Download the file
                        downloaded_path = await store.download(
                            ts, satellite, local_path
                        )

                        # For S3 NetCDF files, render to PNG
                        if not is_recent and downloaded_path.suffix.lower() == ".nc":
                            render_message = (
                                f"Rendering NetCDF to PNG: {ts.isoformat()}"
                            )
                            if progress_callback:
                                progress_callback(
                                    3,
                                    4,
                                    f"Step 4/4: {render_message} ({idx+1}/{source_total})",
                                )
                        ...
                        if progress_callback:
                            if is_recent:
                                progress_callback(
                                    2,
                                    4,
                                    f"Step 3/4: {not_found_message} ({idx+1}/{source_total})",
                                )
                            else:
                                progress_callback(
                                    3,
                                    4,
                                    f"Step 4/4: {not_found_message} ({idx+1}/{source_total})",
                                )
                        ...
                        msg = (
                            f"File not found remotely for {ts.isoformat()} (DOY={doy})"
                        )
                        ...
                        LOGGER.warning(msg)
                        LOGGER.debug(
                            f"Note: NOAA GOES imagery is available at specific time intervals"
                        )
                        LOGGER.debug(
                        ...
                        LOGGER.debug(
                            f"Nearest standard timestamps: {nearest_intervals_str}"
                        )
                    ...
                    error_message = (
                        f"Error fetching from {source_name}: {ts.isoformat()}"
                    )
                    if progress_callback:
                        if is_recent:
                            progress_callback(
                                2,
                                4,
                                f"Step 3/4: {error_message} ({idx+1}/{source_total})",
                            )
                        else:
                            progress_callback(
                                3,
                                4,
                                f"Step 4/4: {error_message} ({idx+1}/{source_total})",
                            )
                    ...
                    LOGGER.debug(
                        f"Download error context - Satellite: {satellite}, Store: {store.__class__.__name__}, Remote exists check failed"
                    )
                    ...
                    error_message = (
                        f"Unexpected error from {source_name}: {ts.isoformat()}"
                    )
                    if progress_callback:
                        if is_recent:
                            progress_callback(
                    ...
                    LOGGER.error(
                        f"Stacktrace for {ts.isoformat()}: {traceback.format_exc()}"
                    )
        ...
        async with self.cdn_store:
            for i, ts in enumerate(sorted(recent_timestamps)):
                cdn_tasks.append(
                    fetch_single(
                        ts, self.cdn_store, True, "CDN", i, len(recent_timestamps)
                    )
                )
        ...
        async with self.s3_store:
            for i, ts in enumerate(sorted(old_timestamps)):
                s3_tasks.append(
                    fetch_single(ts, self.s3_store, False, "S3", i, len(old_timestamps))
                )
    ...
    async def reconcile(
        ...
        if progress_callback:
            if len(missing) > 0:
                progress_callback(
                    1,
                    2,
                    f"Phase 2/2: Preparing to download {len(missing)} missing files...",
                )
            else:
                progress_callback(
                    1,
                    2,
                    f"Phase 2/2: No missing files to download, completing the process...",
                )
        ...
        if progress_callback:
            progress_callback(
                2,
                2,
                f"Reconciliation complete: {existing_count} existing, {fetched} downloaded, {len(missing) - fetched} failed",
            )
