...
class ThreadLocalCacheDB:
    ...
    def _get_connection(self) -> CacheDB:
        ...
        if hasattr(self._local, "db"):
            ...
            return (
                self._local.db
                if isinstance(self._local.db, CacheDB)
                else CacheDB(db_path=self.db_path)
            )
    ...
    def close(self) -> None:
        ...
        with self._lock:
            LOGGER.debug(
                f"Closing all CacheDB connections ({len(self._connections)} connections)"
            )
            ...
            if current_thread_id in self._connections:
                try:
                    LOGGER.debug(
                        f"Closing connection for current thread ID: {current_thread_id}"
                    )
                    self._connections[current_thread_id].close()
                    del self._connections[current_thread_id]
                except Exception as e:
                    LOGGER.error(
                        f"Error closing connection for current thread ID {current_thread_id}: {e}"
                    )

            # Log information about other thread connections but don't try to close them
            other_threads = [
                tid for tid in self._connections.keys() if tid != current_thread_id
            ]
            if other_threads:
                LOGGER.debug(
                    f"Note: {len(other_threads)} connections from other threads will be abandoned"
                )
                for tid in other_threads:
                    LOGGER.debug(
                        f"Thread {tid} connection will be abandoned (cannot be closed from thread {current_thread_id})"
                    )
    ...
    def close_current_thread(self) -> None:
        ...
        with self._lock:
            if thread_id in self._connections:
                try:
                    LOGGER.debug(
                        f"Closing connection for current thread ID: {thread_id}"
                    )
                    self._connections[thread_id].close()
                    del self._connections[thread_id]
                except Exception as e:
                    LOGGER.error(
                        f"Error closing connection for thread ID {thread_id}: {e}"
                    )
    ...
    async def add_timestamp(
        ...
        return await self._get_connection().add_timestamp(
            timestamp=timestamp, satellite=satellite, file_path=file_path, found=found
        )
    ...
    async def timestamp_exists(
        ...
        return await self._get_connection().timestamp_exists(
            timestamp=timestamp, satellite=satellite
        )
