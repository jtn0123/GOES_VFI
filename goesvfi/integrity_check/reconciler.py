...
class Reconciler:
    ...
    def scan_date_range(
        ...
        """
        LOGGER.info(
            f"Starting scan for date range {start_date} to {end_date} "
        ...
        if not force_rescan:
            ...
            if cached_result:
                LOGGER.info(
                    f"Using cached scan results from {cached_result['scan_time']}"
                )
                ...
                missing_timestamps = [
                    item["timestamp"] for item in cached_result["missing"]
                ]
        ...
        LOGGER.info(
            f"Generating expected timestamps at {interval_minutes}-minute intervals"
        )
        ...
        for dt in missing_timestamps:
            ...
            missing_details.append(
                {"timestamp": dt, "expected_filename": expected_filename}
            )
    ...
    def analyze_missing_intervals(
        ...
        isolated_missing = [
            ts for ts in sorted_timestamps if ts not in all_gap_timestamps
        ]
