...
from .goes_imagery import ChannelType, ImageryMode, ProcessingMode, ProductType
...
class SampleProcessor:
    ...
    def __init__(
        ...
        self.s3_client = boto3.client(
            ...
            config=botocore.config.Config(  # Use botocore.config.Config instead of boto3.session.Config
                signature_version=botocore.UNSIGNED, retries={"max_attempts": 3}
            ),
        )

        # S3 bucket names
        self.s3_buckets = {"G16": "noaa-goes16", "G18": "noaa-goes18"}
    ...
    def download_sample_data(
        ...
        if date is None:
            ...
            logger.info(
                f"Using specified date: {date.strftime('%Y-%m-%d %H:%M:%S UTC')}"
            )
        ...
        logger.info(
            f"Search parameters - Year: {year}, Day of Year: {doy}, Hour: {hour}, Minute: {minute}"
        )
        ...
        if channel_num > 16:
            ...
            logger.warning(
                f"Composite channel {channel_num} not supported in sample processor - would require multiple base files"
            )
            return None
        ...
        if file_path is None:
            logger.info(
                f"SEARCH STRATEGY 1: Searching in original date with different hours"
            )
            file_path = self._search_day_hours(
                bucket, year, doy, channel_num, product_type, pattern
            )
        ...
        if file_path is None:
            ...
            logger.info(
                f"SEARCH STRATEGY 2: Searching in previous day {prev_date.strftime('%Y-%m-%d')}"
            )
            file_path = self._search_day_hours(
                bucket, prev_year, prev_doy, channel_num, product_type, pattern
            )
        ...
        if file_path is None:
            ...
            for fallback_date in fallback_dates:
                logger.info(
                    f"Trying fallback date: {fallback_date.strftime('%Y-%m-%d %H:%M:%S UTC')}"
                )
                ...
                file_path = self._search_day_hours(
                    bucket, fb_year, fb_doy, channel_num, product_type, pattern
                )
                if file_path:
                    logger.info(
                        f"Found data using fallback date: {fallback_date.strftime('%Y-%m-%d')}"
                    )
                    break
        ...
        if file_path is None and product_type != ProductType.FULL_DISK:
            logger.info(
                f"SEARCH STRATEGY 4: Trying with FULL_DISK product type as final fallback"
            )
            for fallback_date in fallback_dates:
                logger.info(
                    f"Trying FULL_DISK for date: {fallback_date.strftime('%Y-%m-%d %H:%M:%S UTC')}"
                )
                fb_year = fallback_date.year
                fb_doy = fallback_date.timetuple().tm_yday
                ...
                file_path = self._search_day_hours(
                    bucket, fb_year, fb_doy, channel_num, ProductType.FULL_DISK, pattern
                )
                if file_path:
                    logger.info(
                        f"Found FULL_DISK data using fallback date: {fallback_date.strftime('%Y-%m-%d')}"
                    )
                    break
        ...
        if file_path:
            ...
            logger.error(
                f"Failed to find any sample data for channel {channel_num} after all fallback attempts"
            )

        return file_path

    def _search_day_hours(
        self,
        bucket: str,
        year: int,
        doy: int,
        ...
        logger.info(
            f"Searching for data in {year}/{doy:03d} for channel {channel_num} in product {product_path}"
        )
        ...
        for try_hour in hours_to_try:
            ...
            try:
                ...
                if not matches:
                    logger.info(
                        f"No matching files found for pattern {pattern} in {current_prefix}"
                    )
                    continue
                ...
                for attempt in range(max_retries):
                    try:
                        self.s3_client.download_file(
                            Bucket=bucket, Key=file_key, Filename=str(output_path)
                        )
                        logger.info(
                            f"Successfully downloaded sample file to {output_path}"
                        )
                        ...
                        if attempt < max_retries - 1:
                            ...
                            logger.error(
                                f"All {max_retries} download attempts failed for {file_key}: {str(e)}"
                            )
                ...
                logger.warning(
                    f"Error during search/download from {current_prefix}: {str(e)}"
                )
                continue

        # If we got here, no data was found in any hour
        logger.warning(
            f"No suitable data found in {year}/{doy:03d} for channel {channel_num}"
        )
        return None
    ...
    def download_web_sample(
        ...
        logger.info(
            f"Attempting to download web sample for channel {channel_num} from NOAA CDN"
        )

        # Try multiple strategies to obtain imagery from web sources
        return self._try_all_web_sources(channel_num, product_type, size)

    def _try_all_web_sources(
        self, channel_num: int, product_type: ProductType, size: str = "600x600"
    ) -> Optional[Image.Image]:
        """
    ...
    def _try_noaa_cdn(
        ...
        if product_type == ProductType.FULL_DISK:
            ...
            logger.info(
                f"Unsupported product type for web sample: {product_type}, trying FULL_DISK instead"
            )
            web_path = "FD"
    ...
    def _try_rammb_slider(self, channel_num: int) -> Optional[Image.Image]:
        ...
        if channel_num <= 16:
            ...
            urls_to_try.append(
                f"{rammb_base}/goes-16/full_disk/band_{channel_num:02d}/latest/1200/1200.jpg"
            )
            urls_to_try.append(
                f"{rammb_base}/goes-18/full_disk/band_{channel_num:02d}/latest/1200/1200.jpg"
            )
        elif channel_num == 100:
            # True color
            urls_to_try.append(
                f"{rammb_base}/goes-16/full_disk/geocolor/latest/1200/1200.jpg"
            )
            urls_to_try.append(
                f"{rammb_base}/goes-18/full_disk/geocolor/latest/1200/1200.jpg"
            )
        elif channel_num == 103:
            # Airmass RGB
            urls_to_try.append(
                f"{rammb_base}/goes-16/full_disk/air_mass/latest/1200/1200.jpg"
            )
            urls_to_try.append(
                f"{rammb_base}/goes-18/full_disk/air_mass/latest/1200/1200.jpg"
            )
        elif channel_num == 104:
            # Fire Temperature
            urls_to_try.append(
                f"{rammb_base}/goes-16/full_disk/fire_temperature/latest/1200/1200.jpg"
            )
            urls_to_try.append(
                f"{rammb_base}/goes-18/full_disk/fire_temperature/latest/1200/1200.jpg"
            )
    ...
    def _try_archived_imagery(self, channel_num: int) -> Optional[Image.Image]:
        ...
        dates_to_try = ["20230501/18", "20230701/19", "20230901/20", "20240101/17"]
        ...
        if channel_num <= 16:
            for date_hour in dates_to_try:
                urls_to_try.append(
                    f"{archive_base}/ABI-L2-CMIPF/{date_hour}/OR_ABI-L2-CMIPF-M6C{channel_num:02d}_G16.jpg"
                )
            ...
            try:
                ...
                try:
                    ...
                    if img.width < 10 or img.height < 10:
                        logger.warning(
                            f"Image too small ({img.width}x{img.height}), might be invalid"
                        )
                        os.unlink(tmp_path)
                        continue
                    ...
                    # Resize if needed
                    if (
                        img.width > self.sample_size[0]
                        or img.height > self.sample_size[1]
                    ):
                        scale = min(
                            self.sample_size[0] / img.width,
                            self.sample_size[1] / img.height,
                        )
                    ...
                    logger.info(
                        f"Successfully downloaded image ({img.width}x{img.height}) from {url}"
                    )
                    return img
                ...
                logger.warning(
                    f"Connection error on attempt {attempt+1}/{max_retries} for {url}: {str(e)}"
                )
                if attempt < max_retries - 1:
                    time.sleep(2**attempt)  # Exponential backoff
                continue
                ...
                logger.warning(
                    f"Request error on attempt {attempt+1}/{max_retries} for {url}: {str(e)}"
                )
                if attempt < max_retries - 1:
