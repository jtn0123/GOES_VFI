...
class QuickDateDialog(QDialog):
    ...
    def _set_yesterday(self) -> None:
        ...
        self.calendar.setSelectedDate(QDate(yesterday.year, yesterday.month, yesterday.day))
        self.start_time_edit.setTime(QTime(0, 0))
        self.end_time_edit.setTime(QTime(23, 59))
    ...
    def _set_last_week(self) -> None:
        ...
        self.calendar.setSelectedDate(QDate(last_week.year, last_week.month, last_week.day))
        self.start_time_edit.setTime(QTime(0, 0))
        self.end_time_edit.setTime(QTime(23, 59))
    ...
    def _set_last_month(self) -> None:
        ...
        self.calendar.setSelectedDate(QDate(last_month.year, last_month.month, last_month.day))
        self.start_time_edit.setTime(QTime(0, 0))
        self.end_time_edit.setTime(QTime(23, 59))
...
class ImprovedEnhancedIntegrityCheckTab(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.directory_browse_button.setToolTip("Select directory to scan for GOES imagery")
        ...
        main_actions.addLayout(directory_layout, 3)  # Give directory selection more space
    ...
    def _show_context_menu(self, position: QPoint) -> None:
        ...
        if row >= 0 and row < len(self.view_model.missing_items):
            ...
            self.download_action.setEnabled(
                not getattr(item, 'is_downloaded', False) and 
                not getattr(item, 'is_downloading', False)
            )
            
            # Open folder
            self.open_folder_action.setEnabled(
                hasattr(item, 'local_path') and 
                bool(item.local_path) and 
                getattr(item, 'is_downloaded', False)
            )
    ...
    def _update_missing_items(self, items: List[MissingTimestamp]) -> None:
        ...
        if items:
            ...
            downloading = sum(1 for item in items if getattr(item, 'is_downloading', False))
