    ...
    def __init__(self, base_url: str, timeout: int = 30, verify_ssl: bool = True):
        ...
        try:
            ...
            if response.status_code not in [200, 301, 302]:
                LOGGER.warning(
                    "Base URL may not be accessible: %s, status: %d",
                    self.base_url,
    ...
    def download_file(
        ...
        try:
            ...
            if response.status_code != 200:
                LOGGER.error(
                    "File not found or not accessible: %s, status: %d",
                    url,
                    response.status_code,
                )
                return False
                ...
                with open(destination, "wb") as f:
                    ...
                    for chunk in response.iter_content(chunk_size=8192):
                        ...
                        if chunk:  # Filter out keep-alive chunks
                            ...
                            if (
                                progress_callback
                                and (now - last_progress_time).total_seconds() >= 0.25
                            ):
                                progress_callback(
                                    downloaded,
            ...
            if progress_callback:
                progress_callback(total_size, total_size, 0.0)  # Download complete
            ...
            with open(source_path, "rb") as src, open(destination, "wb") as dst:
                ...
                while True:
                    ...
                    if (
                        ...
                        progress_callback(
                            copied, total_size, 0.0  # No ETA calculation here
                        )
                        last_progress_time = now

            # Final progress update
            if progress_callback:
                progress_callback(total_size, total_size, 0.0)  # Copy complete
