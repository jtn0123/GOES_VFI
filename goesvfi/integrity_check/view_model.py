"""ViewModel for the Integrity Check tab in the GOES VFI application.
...
from typing import List, Dict, Optional, Set, Any, Callable, cast
...
class IntegrityCheckViewModel(QObject):
    ...
    def __init__(self, reconciler: Optional[Reconciler] = None):
        ...
        self._progress_eta = 0.0
        
        # Scan parameters
        self._start_date = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=1)
        self._end_date = datetime.now().replace(hour=23, minute=59, second=59, microsecond=0) - timedelta(days=1)
        self._selected_pattern = SatellitePattern.GENERIC
        self._interval_minutes = 0  # 0 = auto detect
        self._base_directory = Path(os.path.expanduser("~"))
        ...
        self._remote_base_url = "https://example.com/satellite-images"
    ...
    def base_directory(self, value: str) -> None:
        ...
        self._base_directory = Path(value)
    ...
    def can_start_scan(self) -> bool:
        ...
        return (
            self._status != ScanStatus.SCANNING and 
            self._status != ScanStatus.DOWNLOADING and
            self._base_directory.exists() and 
            self._base_directory.is_dir()
        )
        ...
        try:
            ...
            for index, item in enumerate(self._missing_timestamps):
                ...
                download_task.signals.finished.connect(
                    lambda success, msg, i=index, item=item: 
                    self._handle_download_completed(i, item, success, msg)
                )
            ...
            LOGGER.info(f"Started downloads for {len(self._missing_timestamps)} items")
    ...
    def _handle_scan_completed(self, result: Dict[str, Any]) -> None:
        ...
        self._total_expected = result.get("total_expected", 0)
        self._total_found = result.get("total_found", 0)
        self._detected_interval = result.get("interval", 0)
        ...
        if timestamp_details:
            for detail in timestamp_details:
                item = MissingTimestamp(
                    detail['timestamp'],
                    detail.get('expected_filename', 'unknown.png')
                )
    ...
    def _handle_download_completed(self, 
                                 item_index: int, 
                                 item: MissingTimestamp,
                                 success: bool, 
                                 message: str) -> None:
        ...
        if all_complete:
            ...
            self.status = ScanStatus.COMPLETED
            self.status_message = f"Downloads complete: {success_count}/{len(self._missing_timestamps)} successful"
