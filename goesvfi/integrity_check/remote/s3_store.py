...
from typing import Optional, Dict, Any, Tuple, List, Union, cast, TypeVar, Type, TypeVar
...
from goesvfi.integrity_check.time_index import TimeIndex, SatellitePattern, SATELLITE_CODES
...
def log_download_statistics() -> None:
    ...
    total_attempts = int(DOWNLOAD_STATS['total_attempts']) if isinstance(DOWNLOAD_STATS['total_attempts'], (int, float)) else 0
    ...
    start_time = float(DOWNLOAD_STATS['start_time']) if isinstance(DOWNLOAD_STATS['start_time'], (int, float)) else time.time()
    ...
    network_errors = int(DOWNLOAD_STATS['network_errors']) if isinstance(DOWNLOAD_STATS['network_errors'], (int, float)) else 0
    
    # Safely extract file size stats
    largest_file_size = int(DOWNLOAD_STATS['largest_file_size']) if isinstance(DOWNLOAD_STATS['largest_file_size'], (int, float)) else 0
    smallest_file_size = float(DOWNLOAD_STATS['smallest_file_size']) if isinstance(DOWNLOAD_STATS['smallest_file_size'], (int, float)) else float('inf')
...
def update_download_stats(success: bool, download_time: float = 0, file_size: int = 0, 
                         error_type: Optional[str] = None, error_message: Optional[str] = None,
                         satellite: Optional[str] = None, bucket: Optional[str] = None, 
...
class S3Store(RemoteStore):
    ...
    async def _get_s3_client(self) -> S3ClientType:
        ...
        retry_count_value = int(DOWNLOAD_STATS['retry_count']) if isinstance(DOWNLOAD_STATS['retry_count'], (int, float)) else 0
        ...
        while retry_count < max_retries:
            try:
                ...
                if self._s3_client is None:
                    LOGGER.debug(f"Creating new S3 client with unsigned access (attempt {retry_count+1}/{max_retries}, "
                               f"region: {self.aws_region}, delay: {delay:.2f}s)")
                    
                    # Apply delay for retries using exponential backoff
                    if delay > 0:
                        LOGGER.info(f"Applying exponential backoff delay of {delay:.2f}s before retry {retry_count+1}")
                    ...
                    LOGGER.debug(f"Created config with UNSIGNED signature version and timeouts (connect: 10s, read: {self.timeout}s)")
                    
                    # First, create the Session
                    session = aioboto3.Session(**self.session_kwargs)
                    LOGGER.debug(f"Created aioboto3 Session with kwargs: {self.session_kwargs}")
                    ...
                    try:
                        ...
                        try:
                            ...
                            LOGGER.debug(f"Attempting to create S3 client with timeout {client_timeout:.1f}s")
                            self._s3_client = await asyncio.wait_for(
                                client_context.__aenter__(),
                                timeout=client_timeout
                            )
                            LOGGER.debug(f"Entered client context, got S3 client: {self._s3_client}")
                            ...
                            LOGGER.warning(f"Timeout creating S3 client, attempt {retry_count}/{max_retries}")
                        ...
                        if "NoCredentialsError" in str(e) or "AccessDenied" in str(e):
                            ...
                            LOGGER.warning(f"Connection error creating S3 client, attempt {retry_count}/{max_retries}")
                            if retry_count >= max_retries:
                                # Add network diagnostics to help troubleshoot
                                network_info = get_system_network_info()
    ...
    async def __aexit__(self, exc_type: Optional[Type[ExcType]], exc_val: Optional[ExcType], exc_tb: Optional[Any]) -> None:
        """Context manager exit."""
        await self.close()
    
    def _get_bucket_and_key(self, ts: datetime, satellite: SatellitePattern, 
                        product_type: str = "RadC", band: int = 13, exact_match: bool = False) -> Tuple[str, str]:
        ...
        key = TimeIndex.to_s3_key(ts, satellite, product_type=product_type, 
                                 band=band, exact_match=exact_match)
        return bucket, key
    
    async def exists(self, ts: datetime, satellite: SatellitePattern, 
                 product_type: str = "RadC", band: int = 13) -> bool:
        ...
        bucket, key = self._get_bucket_and_key(ts, satellite, product_type=product_type, 
                                             band=band, exact_match=True)
    ...
    async def download(self, ts: datetime, satellite: SatellitePattern, dest_path: Path,
                  product_type: str = "RadC", band: int = 13) -> Path:
        ...
        LOGGER.info(f"Attempting to download S3 file for {satellite.name} at {ts.isoformat()}")
        ...
        LOGGER.debug(f"Timestamp details - ISO: {ts.isoformat()}, Year: {ts.year}, "
        ...
        try:
            try:
                ...
                if error_code == '404':
                    ...
                    LOGGER.info(f"Exact file not found (404), will try wildcard pattern")
            ...
            if has_exact_match:
                ...
                try:
                    ...
                    LOGGER.debug(f"System info: Python {sys.version}, Platform: {sys.platform}")
                    ...
                    LOGGER.info(f"Download complete: {dest_path} in {download_time:.2f} seconds")
                    ...
                    if dest_path.exists():
                        ...
                        LOGGER.info(f"File details: Size: {file_size} bytes, Download time: {download_time:.2f}s, Speed: {speed_str}")
                        download_success = True
                    else:
                        LOGGER.error(f"File download completed but file doesn't exist at {dest_path}")
                    
                    # Update download statistics
                    update_download_stats(success=download_success, download_time=download_time, file_size=file_size)
                    
                    # If this is a significant milestone (every 50 successful downloads), log full stats
                    successful_count = int(DOWNLOAD_STATS['successful']) if isinstance(DOWNLOAD_STATS['successful'], (int, float)) else 0
                    ...
                    LOGGER.error(f"Failed to download file after {download_time:.2f}s: {download_error}")
                    LOGGER.error(f"Download exception type: {type(download_error).__name__}")
                    ...
                    if isinstance(download_error, botocore.exceptions.ClientError):
                        ...
                        # Create error object using helper function
                        custom_msg = f"File for {satellite.name} at {ts.isoformat()} disappeared during download" if error_code in ('NoSuchKey', '404') else None
                        error = create_error_from_code(
                    ...
                    update_download_stats(
                        ...
                        download_time=download_time,
                        error_type=error_type,
                        error_message=f"{error_type}: {str(download_error)[:100]}..." if len(str(download_error)) > 100 else str(download_error)
                    )
                    
                    # Log and raise the error
                    LOGGER.error(f"Download error: {error.get_user_message()}")
                    LOGGER.error(f"Download technical details: {error.technical_details}")
                    
                    # If we've had many failures, log full statistics
                    failed_count = int(DOWNLOAD_STATS['failed']) if isinstance(DOWNLOAD_STATS['failed'], (int, float)) else 0
                ...
                try:
                    ...
                    LOGGER.info(f"Search complete: Examined {page_count} pages with {total_objects} total objects")
                    ...
                    if not matching_objects:
                        ...
                        error = ResourceNotFoundError(
                            message=error_msg,
                            technical_details=technical_details,
                            original_exception=FileNotFoundError(f"No matching files found in S3")
                        )
                        raise error
                    ...
                    LOGGER.info(f"Downloading best match s3://{bucket}/{best_match_key} to {dest_path}")
                    download_start = time.time()
                    download_success = False
                    file_size = 0
                    ...
                    try:
                        # Log system info and network state at download time
                        LOGGER.debug(f"System info: Python {sys.version}, Platform: {sys.platform}")
                        LOGGER.debug(f"Wildcard download starting at {datetime.now().isoformat()}")
                        
                        await s3.download_file(Bucket=bucket, Key=best_match_key, Filename=str(dest_path))
                        download_time = time.time() - download_start
                        
                        LOGGER.info(f"Wildcard download complete: {dest_path} in {download_time:.2f} seconds")
                        ...
                        if dest_path.exists():
                            ...
                            LOGGER.error(f"Download completed but file doesn't exist at {dest_path}")
                        ...
                        LOGGER.error(f"Failed to download wildcard match after {download_time:.2f}s: {download_error}")
                        ...
                        # Record network diagnostics
                        LOGGER.debug(f"Wildcard download failed at {datetime.now().isoformat()}")
                        ...
                        if isinstance(download_error, botocore.exceptions.ClientError):
                            ...
                            # Create error object using helper function
                            custom_msg = "File disappeared during download" if error_code in ('NoSuchKey', '404') else None
                        ...
                        update_download_stats(
                            ...
                            download_time=download_time,
                            error_type=error_type,
                            error_message=f"Wildcard {error_type}: {str(download_error)[:100]}..." if len(str(download_error)) > 100 else str(download_error)
                        )
                        
                        # Log and raise the error
                        LOGGER.error(f"Wildcard download error: {wildcard_error.get_user_message()}")
                        if wildcard_error.technical_details:
                            LOGGER.error(f"Technical details: {wildcard_error.technical_details}")
                    ...
                    LOGGER.error(f"S3 error during list objects operation: {error_code} - {error_message}")
                    ...
                    list_error = create_error_from_code(
                        ...
                        exception=e,
                        error_msg=(
                            f"No files found for {satellite.name} at {ts.isoformat()}" if error_code == '404' else
                            f"AWS S3 service is currently unavailable" if error_code in ('ServiceUnavailable', 'InternalError') else
                            f"Error listing {satellite.name} data files"
                        )
                    )
                    ...
                    if list_error.technical_details:
                        LOGGER.error(f"Technical details: {list_error.technical_details}")
            ...
            LOGGER.error(f"S3 client error during download: {error_code} - {error_message}")
            ...
            client_error = create_error_from_code(
                ...
                exception=e,
                error_msg=(
                    f"File not found for {satellite.name} at {ts.isoformat()}" if error_code in ('NoSuchBucket', 'NoSuchKey', '404') else
                    f"Timeout accessing {satellite.name} data" if "timeout" in str(e).lower() else
            ...
            error_code = (
                ...
                "access" if "permission" in str(e).lower() or "access" in str(e).lower() else
                "disk" if "disk" in str(e).lower() or "space" in str(e).lower() else
                "unknown"
            )
