...
from PyQt6.QtCore import (
    ...
    QSortFilterProxyModel, QTimer, QPoint
)
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            if col == 0:  # Timestamp
                ...
                if item.is_downloaded:
                    ...
                    if "unable to connect" in error_msg.lower() or "connection" in error_msg.lower():
        ...
        # For dark mode, we want to ensure text is visible against colored backgrounds
            ...
            if col == 3:  # Status column
                if item.is_downloaded or item.download_error or item.is_downloading:
                    return QColor(255, 255, 255)  # White text for better visibility on colored backgrounds
...
class AWSConfigDialog(QDialog):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.setWindowTitle("AWS S3 Configuration")
        ...
        info_label.setStyleSheet("QLabel { background-color: #e5f5e5; padding: 10px; border-radius: 5px; }")
...
class AdvancedOptionsDialog(QDialog):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.setWindowTitle("Advanced Integrity Check Options")
        self.setMinimumWidth(500)
        ...
        self.max_concurrent_spinbox.setToolTip("Maximum number of concurrent downloads. Higher values may improve speed but increase resource usage.")
        ...
        self.retry_spinbox.setToolTip("Number of times to retry failed downloads automatically")
        ...
        self.throttle_checkbox.setToolTip("Limit download speed to reduce impact on your network")
        ...
        self.throttle_spinbox.setToolTip("Maximum download speed per file")
        ...
        self.priority_combo.setToolTip("Process priority for download operations")
        ...
        image_group = QGroupBox("Image Processing Options")
        image_layout = QFormLayout()
        
        # Auto-enhance images
        self.auto_enhance_checkbox = QCheckBox()
        self.auto_enhance_checkbox.setToolTip("Automatically enhance downloaded images for better visibility")
        image_layout.addRow("Auto-enhance Images:", self.auto_enhance_checkbox)
        
        # Apply false color
        self.false_color_checkbox = QCheckBox()
        self.false_color_checkbox.setToolTip("Apply false coloring to IR images for better visualization")
        ...
        self.convert_netcdf_checkbox.setToolTip("Automatically convert NetCDF files to PNG after download")
        ...
        notification_group = QGroupBox("Notification Options")
        notification_layout = QFormLayout()
        
        # Desktop notifications
        self.desktop_notify_checkbox = QCheckBox()
        self.desktop_notify_checkbox.setToolTip("Show desktop notifications when operations complete")
        notification_layout.addRow("Desktop Notifications:", self.desktop_notify_checkbox)
        
        # Sound alerts
        self.sound_alerts_checkbox = QCheckBox()
        self.sound_alerts_checkbox.setToolTip("Play sound when operations complete or errors occur")
...
class BatchOperationsDialog(QDialog):
    ...
    def __init__(self, items: List[EnhancedMissingTimestamp], parent: Optional[QWidget] = None) -> None:
        ...
        # Summary label
        self.summary_label = QLabel("Selected operation will process X items")
...
class EnhancedIntegrityCheckTab(IntegrityCheckTab):
    ...
    def closeEvent(self, event: Optional[QCloseEvent]) -> None:
        ...
        try:
            ...
            if hasattr(enhanced_view_model, 'scan_task') and enhanced_view_model.scan_task is not None:
            ...
            if hasattr(enhanced_view_model, 'cleanup') and callable(enhanced_view_model.cleanup):
    ...
    def _setup_ui(self) -> None:
        ...
        auto_detect_satellite_button = QPushButton("Auto-Detect Satellite")
        auto_detect_satellite_button.setToolTip("Automatically detect satellite type from files in the selected directory")
        ...
        if style is not None:
            self.cdn_settings_button.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogDetailedView))
        ...
        style = self.style()
        if style is not None:
            self.s3_settings_button.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogDetailedView))
        ...
        if horizontal_header is not None:
            ...
            horizontal_header.setSectionResizeMode(5, QHeaderView.ResizeMode.Stretch)  # Path column
        ...
        self.download_button.setToolTip("Download all missing files identified in the scan")
        self.download_button.setMinimumWidth(180)
        actions_layout.addWidget(self.download_button)
        
        # Retry button for failed downloads
        self.retry_button = QPushButton("Retry Failed Downloads")
        self.retry_button.setEnabled(False)
        self.retry_button.setCursor(Qt.CursorShape.PointingHandCursor)
        self.retry_button.setToolTip("Retry downloading files that failed in the previous attempt")
        ...
        self.advanced_button.setToolTip("Show additional advanced options")
        ...
        self.export_button.setToolTip("Export a report of missing timestamps to a CSV file")
        actions_layout.addWidget(self.export_button)
        
        self.cache_button = QPushButton("Cache Info")
        self.cache_button.setCursor(Qt.CursorShape.PointingHandCursor)
        self.cache_button.setToolTip("Show information about the current cache database")
    ...
    def _update_status_display(self, message: str, status_type: str = "normal") -> None:
        ...
        if status_type == "normal":
            if "error" in message.lower() or "fail" in message.lower() or "unable" in message.lower():
                status_type = "error"
            elif "warn" in message.lower() or "caution" in message.lower():
                status_type = "warning"
            elif "success" in message.lower() or "complete" in message.lower() or "done" in message.lower():
                status_type = "success"
    ...
    def _update_button_states(self) -> None:
        ...
        self.scan_button.setEnabled(bool(has_directory and not self.view_model.is_scanning and not self.view_model.is_downloading))
        
        # Update cancel button state
        self.cancel_button.setEnabled(self.view_model.is_scanning or self.view_model.is_downloading)
        ...
        self.download_button.setEnabled(has_missing and scan_complete and not self.view_model.is_downloading)
        ...
        if hasattr(self, 'retry_button'):
            ...
            failed_items = [item for item in self.view_model.missing_items 
                           if not item.is_downloaded and item.download_error]
            has_failed = len(failed_items) > 0
            self.retry_button.setEnabled(has_failed and not self.view_model.is_scanning and not self.view_model.is_downloading)
    ...
    def _show_context_menu(self, position: QPoint) -> None:
        ...
        if item.download_error:
            ...
            action_error_details.triggered.connect(lambda: self._show_error_details(row))
    ...
    def _show_advanced_menu(self) -> None:
        ...
        menu.exec(self.advanced_button.mapToGlobal(self.advanced_button.rect().bottomLeft()))
    ...
    def _show_advanced_options(self) -> None:
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if hasattr(enhanced_view_model, 'set_max_concurrent_downloads'):
                enhanced_view_model.set_max_concurrent_downloads(self.advanced_options["max_concurrent"])
            
            # Apply auto-retry setting
            if hasattr(enhanced_view_model, 'set_retry_attempts'):
                enhanced_view_model.set_retry_attempts(self.advanced_options["retry_attempts"])
    ...
    def _show_batch_operations(self) -> None:
        ...
        enhanced_items = cast(List[EnhancedMissingTimestamp], self.view_model.missing_items)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if options["filter"] == "all":
                ...
                if selection_model is not None:
                    ...
                    selected_indexes = []
                items = [self.view_model.missing_items[index.row()] for index in selected_indexes]
            elif options["filter"] == "failed":
                items = [item for item in self.view_model.missing_items if not item.is_downloaded and item.download_error]
            elif options["filter"] == "missing":
                items = [item for item in self.view_model.missing_items if not item.is_downloaded]
            elif options["filter"] == "downloaded":
                items = [item for item in self.view_model.missing_items if item.is_downloaded]
    ...
    def _batch_export_items(self, items: List[MissingTimestamp]) -> None:
        ...
        try:
            ...
            with open(file_path, 'w', newline='') as csvfile:
                import csv
                fieldnames = ['timestamp', 'satellite', 'source', 'status', 'local_path', 'error']
    ...
    def _batch_delete_items(self, items: List[MissingTimestamp]) -> None:
        ...
        delete_items = [item for item in items if item.is_downloaded and item.local_path]
    ...
    def _show_network_diagnostics(self) -> None:
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Network Diagnostics")
        ...
        refresh_button.clicked.connect(lambda: self._update_network_diagnostics(text_browser))
    ...
    def _update_network_diagnostics(self, text_browser: QTextBrowser) -> None:
        ...
        html += f"<p><b>Timestamp:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>"
        
        # Try to get download statistics from the S3 store
        from goesvfi.integrity_check.remote.s3_store import DOWNLOAD_STATS, log_download_statistics
        ...
        if errors:
            ...
            for error in error_list:
                if isinstance(error, dict):
                    ...
                    html += f"<tr><td colspan='3'>Invalid error format: {str(error)}</td></tr>"
    ...
    def _show_visualization_options(self) -> None:
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Visualization Options")
        ...
        color_schemes = ["Default", "Enhanced", "False Color", "High Contrast", "Grayscale"]
        ...
        timestamp_formats = ["YYYY-MM-DD HH:MM:SS", "YYYY-MM-DD HH:MM", "MM/DD/YYYY HH:MM", "ISO Format"]
    ...
    def _save_configuration(self) -> None:
        ...
        try:
            ...
            config = {
                ...
                "fetch_source": self.auto_radio.isChecked() and "AUTO" or
                              self.cdn_radio.isChecked() and "CDN" or
                              self.s3_radio.isChecked() and "S3" or "LOCAL",
    ...
    def _auto_detect_date_range(self) -> None:
        ...
        if not self.view_model.base_directory or not self.view_model.base_directory.exists():
            QMessageBox.warning(
                self,
    ...
    def _browse_directory(self) -> None:
        ...
        try:
            ...
            try:
                LOGGER.debug(f"Opening directory dialog with starting directory: {current_dir}")
                ...
                if directory:
                    ...
                    if not path_obj.exists() or not path_obj.is_dir():
                        LOGGER.error(f"Selected path is not a valid directory: {directory}")
                        QMessageBox.warning(
                            self,
                            "Invalid Directory",
                    ...
                    try:
                        ...
                        LOGGER.error(f"Permission denied reading directory: {directory}")
                        QMessageBox.warning(
                            self,
                            "Permission Denied",
    ...
    def _start_enhanced_scan(self) -> None:
        ...
        try:
            ...
            # Log date range
            LOGGER.debug(f"Scan date range: {start_date.isoformat()} to {end_date.isoformat()}")
            ...
            LOGGER.debug(f"Scan parameters: interval={interval_minutes}min, force_rescan={force_rescan}, auto_download={auto_download}")
            ...
            LOGGER.debug(f"Satellite: {satellite.name}, Fetch source: {fetch_source.name}")
    ...
    def _retry_failed_downloads(self) -> None:
        ...
        failed_items = [item for item in enhanced_view_model.missing_items 
                       if not item.is_downloaded and item.download_error]
        
        if not failed_items:
            QMessageBox.information(self, "No Failed Downloads", 
                                   "There are no failed downloads to retry.")
            return
    ...
    def _auto_detect_satellite(self) -> None:
        ...
        try:
            ...
            self.setCursor(Qt.CursorShape.WaitCursor)
            QApplication.processEvents()
            self._update_status_display("Detecting satellite type from files...", "info")
            
            # Create progress dialog to show progress
            progress_dialog = QProgressDialog("Scanning files...", "Cancel", 0, 100, self)
            progress_dialog.setWindowTitle("Detecting Satellite Type")
            ...
            LOGGER.info(f"Auto-detect satellite: Starting scan of directory {directory}")
            ...
            LOGGER.info(f"Auto-detect satellite: Scanning for GOES-16 files in {directory}")
            
            # Log number of files in the directory
            try:
                png_files = list(directory.glob("**/*.png"))
                all_files = list(directory.glob("**/*"))
                LOGGER.info(f"Directory statistics: {len(png_files)} PNG files, {len(all_files)} total files")
                ...
                if len(all_files) > len(png_files):
                    non_png_samples = [f.name for f in all_files if f.suffix.lower() != '.png'][:5]
            ...
            try:
                ...
                goes16_files = TimeIndex.scan_directory_for_timestamps(directory, SatellitePattern.GOES_16)
                LOGGER.info(f"Auto-detect satellite: Found {len(goes16_files)} GOES-16 files")
                # Log the first few timestamps found, if any
                if goes16_files:
                    LOGGER.info(f"Sample GOES-16 timestamps: {[ts.isoformat() for ts in goes16_files[:5]]}")
            except Exception as scan_error:
                LOGGER.error(f"Auto-detect satellite: Error scanning for GOES-16 files: {scan_error}")
                LOGGER.error(traceback.format_exc())
                # Try to get more context about what went wrong
                try:
                    # If the error was in pattern matching, try to log filesystem state
                    LOGGER.info(f"Directory exists: {directory.exists()}, is directory: {directory.is_dir()}")
                    ...
                    try:
                        ...
                        LOGGER.info(f"Directory permissions: Read={is_readable}, Write={is_writable}, Execute={is_executable}")
                    except Exception as perm_error:
                        LOGGER.error(f"Error checking directory permissions: {perm_error}")
            ...
            LOGGER.info(f"Auto-detect satellite: Scanning for GOES-18 files in {directory}")
            
            goes18_files = []
            try:
                LOGGER.info("Beginning GOES-18 file scan with pattern matching")
                goes18_files = TimeIndex.scan_directory_for_timestamps(directory, SatellitePattern.GOES_18)
                LOGGER.info(f"Auto-detect satellite: Found {len(goes18_files)} GOES-18 files")
                # Log the first few timestamps found, if any
                if goes18_files:
                    LOGGER.info(f"Sample GOES-18 timestamps: {[ts.isoformat() for ts in goes18_files[:5]]}")
            except Exception as scan_error:
                LOGGER.error(f"Auto-detect satellite: Error scanning for GOES-18 files: {scan_error}")
                LOGGER.error(traceback.format_exc())
            ...
            if goes16_count == 0 and goes18_count == 0:
                ...
                try:
                    ...
                    LOGGER.error(f"Error gathering additional directory information: {info_error}")
            ...
            if goes16_count > goes18_count:
                # Select GOES-16
                LOGGER.info(f"Auto-detect satellite: Selected GOES-16 based on file count ({goes16_count} vs {goes18_count})")
                enhanced_view_model.satellite = SatellitePattern.GOES_16
                self.goes16_radio.setChecked(True)
                satellite_name = "GOES-16 (East)"
            else:
                # Select GOES-18
                LOGGER.info(f"Auto-detect satellite: Selected GOES-18 based on file count ({goes18_count} vs {goes16_count})")
                enhanced_view_model.satellite = SatellitePattern.GOES_18
                self.goes18_radio.setChecked(True)
            ...
            LOGGER.info(f"Auto-detect satellite: Completed successfully, selected {satellite_name}")
            
        except Exception as e:
            LOGGER.error(f"Auto-detect satellite: Critical error in satellite detection: {e}")
            import traceback
            LOGGER.error(traceback.format_exc())
        ...
        if is_step_operation or is_phase_operation:
            ...
            if is_step_operation:
                ...
                try:
                    ...
                    if "/" in step_parts:
                        current_step, total_steps = step_parts.split("/", 1)[0].split(" ")[-1], step_parts.split("/", 1)[1].split(" ")[0]
                ...
                self.progress_bar.setFormat(f"{progress_percent}% - {step_info} ({current}/{total})")
                ...
                try:
                    ...
                    if "/" in phase_parts:
                        current_phase, total_phases = phase_parts.split("/", 1)[0].split(" ")[-1], phase_parts.split("/", 1)[1].split(" ")[0]
                ...
                self.progress_bar.setFormat(f"{progress_percent}% - {phase_info} ({current}/{total})")
        else:
            # For regular operations or when ETA is available
            if eta > 0:
                eta_minutes = int(eta / 60)
                eta_seconds = int(eta % 60)
                self.progress_bar.setFormat(f"{progress_percent}% - ETA: {eta_minutes}m {eta_seconds}s ({current}/{total})")
            else:
    ...
    def _update_status(self, message: str) -> None:
        ...
        if "error" in message.lower() or "failed" in message.lower():
            ...
            formatted_message = f"<span style='color: #ff6666; font-weight: bold;'>{message}</span>"
            
            # For certain common errors, provide extra help based on detected error patterns
            if "unexpected error" in message.lower():
                error_details = "<br><span style='color: #ffaaaa;'><b>Troubleshooting Tips:</b><br>"
            ...
            formatted_message = f"<span style='color: #66ff66; font-weight: bold;'>{message}</span>"
        ...
        elif "scanning" in message.lower() or "downloading" in message.lower() or "step" in message.lower():
            # In-progress message - make it blue and bold
            formatted_message = f"<span style='color: #66aaff; font-weight: bold;'>{message}</span>"
        ...
        elif "detected" in message.lower() and ("satellite" in message.lower() or "goes" in message.lower()):
    ...
    def _update_download_progress(self, current: int, total: int) -> None:
        ...
        download_rate_text = ""
        if hasattr(self, '_last_progress_update_time') and hasattr(self, '_last_progress_count'):
            time_diff = time.time() - self._last_progress_update_time
            if time_diff > 0:
                items_diff = current - self._last_progress_count
                if items_diff > 0 and time_diff >= 1.0:  # Only calculate if at least 1 second passed
        ...
        self.progress_bar.setFormat(f"Downloading: {progress_percent}% ({current}/{total}){download_rate_text}")
            ...
            if success_count > 0 or failed_count > 0:
                ...
                if success_count > 0:
                    ...
                    if failed_count > 0:
                        status_text += ", "
                if failed_count > 0:
                    status_text += f"<span style='color: #ff6666;'>{failed_count} failed</span>"
                status_text += f"{download_rate_text}</span>"
        
        # Display detail about current downloads
        if hasattr(self.view_model, 'currently_downloading_items') and self.view_model.currently_downloading_items:
            status_text += (
                f"<br><span style='color: #aaccff; font-size: 90%;'>"
                f"Current files: "
            )
            ...
            for ts in self.view_model.currently_downloading_items:
                ...
                displayed_items += 1
                if displayed_items < min(3, len(self.view_model.currently_downloading_items)):
                    status_text += ", "
    ...
    def _show_detailed_error_dialog(self, title: str, message: str) -> None:
        ...
        tips_label = QLabel("<b>Troubleshooting Tips:</b>")
        ...
        if "AUTH-" in error_code or "access denied" in message.lower() or "permission" in message.lower():
            troubleshooting_html = """
            <ol>
        ...
        # Not found errors
        elif "NF-" in error_code or "not found" in message.lower() or "no such" in message.lower():
            troubleshooting_html = """
            <ol>
        ...
        # Connection errors
        elif "CONN-" in error_code or "NET-" in error_code or "connection" in message.lower() or "timeout" in message.lower():
            troubleshooting_html = """
            <ol>
        ...
        elif "RATE-" in error_code or "throttl" in message.lower() or "limit" in message.lower():
            troubleshooting_html = """
            <ol>
        ...
        # Server-side errors
        elif "AWS-" in error_code or "S3-" in error_code or "service" in message.lower():
            troubleshooting_html = """
            <ol>
        ...
        # SSL/TLS errors
        elif "SSL-" in error_code or "ssl" in message.lower() or "certificate" in message.lower():
            troubleshooting_html = """
            <ol>
        ...
        if "Error code:" in message or "Technical details:" in message:
            tech_details_label = QLabel("<b>Technical Information:</b>")
