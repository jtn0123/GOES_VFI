...
def run_vfi(
    ...
    **kwargs: Any
    ...
    crop_rect_tuple = kwargs.get("crop_rect")
    ...
    # --- Crop Setup ---
    if crop_rect_tuple:
        crop = cast(Tuple[int, int, int, int], crop_rect_tuple)
        LOGGER.info(f"Applying crop rectangle: {crop}")
    else:
        crop = None
    ...
    try:
        ...
        width, height = im0.size # Get original size first
        if crop:
            x, y, w_crop, h_crop = crop
            # Check if crop rectangle is valid for the first image
            if x + w_crop > width or y + h_crop > height:
                 raise ValueError(f"Crop rectangle {crop} exceeds dimensions ({width}x{height}) of first image {paths[0].name}")
            im0 = im0.crop((x, y, x + w_crop, y + h_crop))
        # Final target dimensions are post-crop
        width, height = im0.size
        LOGGER.info(f"Target frame dimensions (post-crop): {width}x{height}")
        ...
        try:
            with Image.open(p_path) as img:
                ...
                if crop:
                    ...
                    if x + w_crop > orig_w or y + h_crop > orig_h:
                        raise ValueError(f"Crop rectangle {crop} exceeds dimensions ({orig_w}x{orig_h}) of image {p_path.name}")
                    img_to_validate = img.crop((x, y, x + w_crop, y + h_crop))
                # Check final size after potential crop
                if img_to_validate.size != (width, height):
                    raise ValueError(f"Inconsistent dimensions for {p_path.name}! Got {img_to_validate.size} after crop, expected {(width, height)}.")
        except Exception as e:
    ...
    try:
        ...
        if skip_model:
            ...
            for idx, p_path in enumerate(paths[1:], start=1):
                try:
                    with Image.open(p_path) as img:
                        img_to_write = img.crop(crop) if crop else img
                        # Pre-validation ensures dimensions match
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                for idx, (p1_path, p2_path) in enumerate(zip(paths, paths[1:])):
                    ...
                    # --- Load, crop, convert, write INTERPOLATED frame --- 
                    ...
                    try:
                        with Image.open(interpolated_frame_path) as im_interp:
                            im_interp_to_write = im_interp.crop(crop) if crop else im_interp
                            # Re-add dimension check/resize for interpolated frame
                            if im_interp_to_write.size != (width, height):
                                LOGGER.warning(f"Resizing interpolated frame for pair {idx} from {im_interp_to_write.size} to {(width, height)}.")
                                im_interp_to_write = im_interp_to_write.resize((width, height), Image.Resampling.LANCZOS)
                    ...
                    try:
                        with Image.open(p2_path) as im_second:
                            im_second_to_write = im_second.crop(crop) if crop else im_second
