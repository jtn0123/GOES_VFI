...
class CropDialog(QDialog):
    def __init__(
        ...
        super().__init__(parent)
        self.setWindowTitle(self.tr("Select Crop Region"))
        ...
        try:
            ...
            if self.encoder == "RIFE":
                ...
                if not raw_video_path or not raw_video_path.exists():
                    raise RuntimeError(
                        "RIFE pipeline finished but did not produce a raw video file."
                    )

                # Rename raw output to final requested name
                if raw_video_path.exists() and raw_video_path != self.out_file_path:
                    LOGGER.info(
                        f"Renaming raw output {raw_video_path.name} to {self.out_file_path.name}"
                    )
                    try:
                        ...
                        raise RuntimeError(
                            f"Failed to rename {raw_video_path} to {self.out_file_path}: {e}"
                        ) from e
                elif raw_video_path == self.out_file_path:
                    self.finished.emit(self.out_file_path)
                else:
                    raise RuntimeError(
                        "Raw video file missing after RIFE pipeline completion."
                    )
...
class MainWindow(QWidget):
    ...
    def __init__(self, debug_mode: bool = False) -> None:
        ...
        super().__init__()
        self.debug_mode = debug_mode
        self.setWindowTitle(self.tr("GOES-VFI GUI"))
        ...
        file_sorter_model = FileSorter(
            dry_run=False, duplicate_mode=DuplicateMode.OVERWRITE
        )
        ...
        # Input/Output
        self.in_dir_label = QLabel(self.tr("Input Directory:"))
        self.in_dir_edit = QLineEdit()
        self.in_dir_button = QPushButton(self.tr("Browse..."))

        self.out_file_label = QLabel(self.tr("Output File (MP4):"))
        self.out_file_edit = QLineEdit()
        self.out_file_button = QPushButton(self.tr("Browse..."))

        # Interpolation Settings
        self.fps_label = QLabel(self.tr("Output FPS:"))
        self.fps_spinbox = QSpinBox()
        self.fps_spinbox.setRange(1, 120)
        self.fps_spinbox.setValue(30)

        self.mid_count_label = QLabel(self.tr("Intermediate Frames:"))
        ...
        self.max_workers_label = QLabel(self.tr("Max Workers:"))
        self.max_workers_spinbox = QSpinBox()
        self.max_workers_spinbox.setRange(
            1, os.cpu_count() or 4
        )  # Range from 1 to num CPUs
        ...
        self.rife_uhd_mode_checkbox = QCheckBox(self.tr("UHD Mode (for > 4K)"))
        self.rife_uhd_mode_checkbox.setChecked(False)  # Default to disabled

        self.rife_thread_spec_label = QLabel(self.tr("Thread Spec:"))
        ...
        rife_options_layout.addWidget(
            self.rife_thread_spec_edit, 2, 1, 1, 2
        )  # Span 2 columns
        ...
        self.sanchez_options_groupbox = QGroupBox(self.tr("Sanchez Options"))
        ...
        self.sanchez_false_colour_checkbox = QCheckBox(self.tr("Apply False Colour"))
        self.sanchez_false_colour_checkbox.setChecked(False)  # Default to disabled

        self.sanchez_res_km_label = QLabel(self.tr("Resolution (km):"))
        ...
        self.preview_label_mid = ClickableLabel(
            "Preview 2 (Middle)"
        )  # Added middle label
        ...
        self.preview_label_mid.setAlignment(
        ...
        # Start Button
        self.start_button = QPushButton(self.tr("Start VFI"))
        ...
        # Open in VLC Button
        self.open_vlc_button = QPushButton(self.tr("Open Output in VLC"))
        ...
        preview_splitter.addWidget(
            self.preview_label_mid
        )  # Add middle label to splitter
        ...
        if screen:
            screen_width = int(
                screen.availableGeometry().width() * 0.8
            )  # 80% of screen width
        ...
        vfi_process_layout.addWidget(
            preview_splitter, 1
        )  # Give the splitter a stretch factor
        ...
        self.sanchez_res_km_spinbox.valueChanged.connect(
            lambda: self._trigger_preview_update()
        )

        # Connect signals for preview zooming
        self.preview_label_1.clicked.connect(
            lambda: self._show_zoom(self.preview_label_1)
        )
        self.preview_label_mid.clicked.connect(
            lambda: self._show_zoom(self.preview_label_mid)
        )  # Connect middle label
        self.preview_label_2.clicked.connect(
            lambda: self._show_zoom(self.preview_label_2)
        )
        ...
        self.rife_tile_enable_checkbox.stateChanged.connect(
            self._toggle_tile_size_enabled
        )
        self.rife_thread_spec_edit.textChanged.connect(self._validate_thread_spec)

        # Connect signals for Sanchez options
        self.sanchez_false_colour_checkbox.stateChanged.connect(
            self._toggle_sanchez_res_enabled
        )
        ...
        )  # Update based on initial/loaded encoder
        ...
        self._update_unsharp_controls_state(
            self.unsharp_groupbox.isChecked()
        )  # Update unsharp controls state
        self._update_scd_thresh_state(
            self.ffmpeg_scd_combo.currentText()
        )  # Update scd threshold state
        self._update_quality_controls_state(
            "FFmpeg", self.ffmpeg_quality_preset_combo.currentText()
        )  # Update quality controls state
    ...
    def _trigger_preview_update(
        self, *args: Any
    ) -> None:  # Accept arbitrary args from signals
    ...
    def _pick_out_file(self) -> None:
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Output File", "", "MP4 Files (*.mp4);;All Files (*)"
        )
        if file_path:
    ...
    def _on_crop_clicked(self) -> None:
        ...
        if not self.preview_label_1.file_path:
            QMessageBox.warning(
                self,
                "Crop Error",
                "Load input images first to select a crop region (ensure Preview 1 is loaded).",
            )
            return
        ...
        try:
            ...
            if pixmap_to_crop.isNull():
                QMessageBox.warning(
                    self,
                    "Crop Error",
                    f"Failed to load image for cropping:\n{self.preview_label_1.file_path}",
                )
                return
        except Exception as e:
            LOGGER.error(f"Error loading image for crop dialog: {e}")
            QMessageBox.critical(
                self, "Crop Error", f"Could not load image for cropping:\n{e}"
            )
            return

        dialog = CropDialog(
            pixmap_to_crop, self.current_crop_rect, self
        )  # Pass QPixmap directly
    ...
    def _makeMainTab(self) -> QWidget:
        ...
        layout.addWidget(QLabel(self.tr("This is a placeholder tab.")))
        return tab
    ...
    def _make_ffmpeg_settings_tab(self) -> QWidget:
        ...
        profile_layout.addWidget(QLabel(self.tr("FFmpeg Settings Profile:")))
        self.ffmpeg_profile_combo = QComboBox()
        self.ffmpeg_profile_combo.addItems(list(FFMPEG_PROFILES.keys()))
        self.ffmpeg_profile_combo.addItem(self.tr("Custom"))  # Add Custom option
        profile_layout.addWidget(self.ffmpeg_profile_combo)
        profile_layout.addStretch(1)
        main_layout.addLayout(profile_layout)

        # --- Interpolation Group ---
        interp_group = QGroupBox(self.tr("Interpolation (minterpolate)"))
        ...
        self.ffmpeg_mc_mode_label = QLabel(self.tr("MC Mode:"))
        ...
        self.ffmpeg_me_mode_label = QLabel(self.tr("ME Mode:"))
        ...
        self.ffmpeg_search_param_label = QLabel(self.tr("Search Param:"))
        ...
        self.ffmpeg_scd_label = QLabel(self.tr("Scene Change Detection:"))
        ...
        self.ffmpeg_scd_thresh_label = QLabel(self.tr("SCD Threshold (%):"))
        ...
        unsharp_group = QGroupBox(self.tr("Sharpening (unsharp)"))
        ...
        unsharp_layout.addWidget(QLabel(self.tr("Luma X:")), 0, 0)
        ...
        unsharp_layout.addWidget(QLabel(self.tr("Luma Y:")), 0, 2)
        ...
        unsharp_layout.addWidget(QLabel(self.tr("Luma Amount:")), 0, 4)
        ...
        unsharp_layout.addWidget(QLabel(self.tr("Chroma X:")), 1, 0)
        ...
        unsharp_layout.addWidget(QLabel(self.tr("Chroma Y:")), 1, 2)
        ...
        unsharp_layout.addWidget(QLabel(self.tr("Chroma Amount:")), 1, 4)
        ...
        quality_group = QGroupBox(self.tr("Encoding Quality (libx264)"))
        ...
        self.ffmpeg_crf_label = QLabel(self.tr("CRF:"))
        ...
        self.ffmpeg_bitrate_label = QLabel(self.tr("Bitrate (kbps):"))
        ...
        self.ffmpeg_bufsize_label = QLabel(self.tr("Buffer Size (kbps):"))
    ...
    def _on_profile_selected(self, profile_name: str) -> None:
        ...
        if profile_name not in FFMPEG_PROFILES:
            LOGGER.warning(
                f"Selected profile '{profile_name}' not found in FFMPEG_PROFILES."
            )
            return
        ...
        widgets_to_block = [
            ...
            self.unsharp_cy_spinbox,
            self.unsharp_ca_spinbox,
            self.ffmpeg_quality_preset_combo,
            self.ffmpeg_crf_spinbox,
            self.ffmpeg_bitrate_spinbox,
            self.ffmpeg_pix_fmt_combo,
            self.ffmpeg_filter_preset_combo,
        ...
        blocked_states = {
            widget: widget.signalsBlocked() for widget in widgets_to_block
        }
    ...
    def _update_ffmpeg_controls_state(
        self, enable: bool, update_group: bool = True
    ) -> None:
        ...
        if hasattr(self, "ffmpeg_settings_tab"):
            ...
            if update_group:
                self.ffmpeg_interp_group.setEnabled(
                    enable and self.ffmpeg_interp_group.isChecked()
                )
                self.unsharp_groupbox.setEnabled(
                    enable and self.unsharp_groupbox.isChecked()
                )
                # Update controls within the groups based on their check state
                self._update_unsharp_controls_state(
                    self.unsharp_groupbox.isChecked() if enable else False
                )
                self._update_scd_thresh_state(
                    self.ffmpeg_scd_combo.currentText() if enable else "none"
                )
                self._update_quality_controls_state(
                    "FFmpeg",
                    self.ffmpeg_quality_preset_combo.currentText() if enable else None,
                )

        else:
            LOGGER.warning(
                "Attempted to update FFmpeg controls state, but ffmpeg_settings_tab not found."
            )
        ...
        if layout:
            for i in range(layout.count()):
                ...
                if item is not None:
                    widget = item.widget()
                    if (
                        widget and widget != self.unsharp_groupbox
                    ):  # Don't disable the groupbox itself
    ...
    def _update_quality_controls_state(
        self, encoder_type: str, preset_text: Optional[str] = None
    ) -> None:
        ...
        self.ffmpeg_bufsize_label.setEnabled(
            enable_bitrate and is_ffmpeg
        )  # Bufsize tied to bitrate
    ...
    def _start(self) -> None:
        ...
        scd_threshold = (
            self.ffmpeg_scd_thresh_spinbox.value() if scd_mode == "fdiff" else None
        )
        ...
        if (
            "Custom CRF" not in quality_preset
            and "Custom Bitrate" not in quality_preset
        ):
            ...
            if match:
                ...
                LOGGER.warning(
                    f"Could not extract CRF from preset '{quality_preset}', using spinbox value."
                )
        ...
        if encoder == "RIFE":
            ...
            if not re.fullmatch(r"\d+:\d+:\d+:\d+", rife_thread_spec):
                self._show_error(
                    "Invalid RIFE thread spec format. Use N:N:N:N (e.g., 0:0:0:0).",
                    user_error=True,
                )
                return
        ...
        self.status_label.setText(self.tr("Starting VFI process..."))
        self.progress_bar.setValue(0)
        ...
        self.worker.error.connect(
            lambda msg: self._show_error(msg, stage="Processing")
        )  # Specify stage
        self.worker.start()
    ...
    def _on_progress(self, current: int, total: int, eta: float) -> None:
        ...
        self.status_label.setText(
            f"Processing frame {current}/{total}... ETA: {eta_str}"
        )
    ...
    def _show_error(
        self, msg: str, stage: str = "Error", user_error: bool = False
    ) -> None:
        ...
        if (
            self.is_processing
        ):  # Ensure state is reset if error occurs during processing
            self._set_processing_state(False)
    ...
    def _open_in_vlc(self) -> None:
        ...
        if file_path and Path(file_path).exists():
            ...
            QMessageBox.warning(
                self, "Open Failed", "Output file does not exist or path is not set."
            )
    ...
    def _show_zoom(self, label: ClickableLabel) -> None:
        ...
        try:
            ...
            if img.isNull():
                ...
                QMessageBox.warning(
                    label, "Zoom Error", f"Could not load image:\n{label.file_path}"
                )
                return
            ...
            if screen:
                ...
                LOGGER.warning(
                    "Zoom: Could not get screen geometry, showing unscaled image."
                )
                dialog = ZoomDialog(
                    full_res_processed_pixmap, self
                )  # Show unscaled if no screen info
                dialog.exec()

        except Exception as e:
            LOGGER.error(f"Error showing zoom for {label.file_path}: {e}")
            QMessageBox.critical(
                label, "Zoom Error", f"Could not display zoomed image:\n{e}"
            )
    ...
    def _makeModelLibraryTab(self) -> QWidget:
        ...
        layout.addWidget(QLabel(self.tr("Installed RIFE Models:")))

        self.model_table = QTableWidget()
        self.model_table.setColumnCount(4)  # Name, Type, Path, Capabilities
        self.model_table.setHorizontalHeaderLabels(
            ["Name", "Type", "Path", "Capabilities"]
        )
        ...
        # refresh_button = QPushButton(self.tr("Refresh Models"))
    ...
    def _populate_model_table(self) -> None:
        ...
        )  # Assumes this returns list[str] or similar

        # Base path for models
        project_root = pathlib.Path(
            __file__
        ).parent.parent  # Go up one level from gui.py to project root
        ...
        for i, model_name in enumerate(models):  # Assuming models is a list of names
            ...
            self.model_table.setItem(
                i, 0, QTableWidgetItem(model_name)
            )  # Use model_name as name
            self.model_table.setItem(i, 1, QTableWidgetItem(model_type))  # Set type
            self.model_table.setItem(
                i, 2, QTableWidgetItem(str(model_path))
            )  # Set path
    ...
    def _check_settings_match_profile(self, profile_dict: FfmpegProfile) -> bool:
        ...
        try:
            ...
            for key, profile_value in profile_dict.items():
                ...
                if (
                    key.startswith("unsharp_")
                    and key != "apply_unsharp"
                    and not profile_dict.get("apply_unsharp")
                ):
                ...
                if "Custom CRF" in preset:
                    if key == "bitrate" or key == "bufsize":
                        continue  # Don't compare bitrate/bufsize
                elif "Custom Bitrate" in preset:
                    if key == "crf":
                        continue  # Don't compare CRF
    ...
    def loadSettings(self) -> None:
        ...
        self.in_dir_edit.setText(
            self.settings.value("paths/inputDirectory", "", type=str)
        )
        self.out_file_edit.setText(
            self.settings.value("paths/outputFile", "", type=str)
        )

        # Interpolation
        self.fps_spinbox.setValue(
            self.settings.value("interpolation/fps", 30, type=int)
        )
        self.mid_count_spinbox.setValue(
            self.settings.value("interpolation/midCount", 7, type=int)
        )
        self.max_workers_spinbox.setValue(
            self.settings.value("interpolation/maxWorkers", 4, type=int)
        )
        self.encoder_combo.setCurrentText(
            self.settings.value("interpolation/encoder", "RIFE", type=str)
        )
        self.model_combo.setCurrentText(
            self.settings.value("interpolation/rifeModel", "", type=str)
        )  # Load model name

        # RIFE Options
        self.rife_tile_enable_checkbox.setChecked(
            self.settings.value("rife/tileEnable", True, type=bool)
        )
        self.rife_tile_size_spinbox.setValue(
            self.settings.value("rife/tileSize", 384, type=int)
        )
        self.rife_uhd_mode_checkbox.setChecked(
            self.settings.value("rife/uhdMode", False, type=bool)
        )
        self.rife_thread_spec_edit.setText(
            self.settings.value("rife/threadSpec", "0:0:0:0", type=str)
        )
        self.rife_tta_spatial_checkbox.setChecked(
            self.settings.value("rife/ttaSpatial", False, type=bool)
        )
        self.rife_tta_temporal_checkbox.setChecked(
            self.settings.value("rife/ttaTemporal", False, type=bool)
        )

        # Sanchez Options
        self.sanchez_false_colour_checkbox.setChecked(
            self.settings.value("sanchez/falseColour", False, type=bool)
        )
        self.sanchez_res_km_spinbox.setValue(
            self.settings.value("sanchez/resKm", 500, type=int)
        )

        # FFmpeg Profile and Settings
        # Load profile first, then individual settings which might override profile if saved as "Custom"
        saved_profile_name = self.settings.value(
            "ffmpeg/profileName", "Default", type=str
        )

        # Load individual FFmpeg settings (use profile defaults if key missing)
        default_prof = FFMPEG_PROFILES.get(
            saved_profile_name, DEFAULT_FFMPEG_PROFILE
        )  # Fallback to Default

        self.ffmpeg_interp_group.setChecked(
            self.settings.value(
                "ffmpeg/use_ffmpeg_interp", default_prof["use_ffmpeg_interp"], type=bool
            )
        )
        self.ffmpeg_mi_mode_combo.setCurrentText(
            self.settings.value("ffmpeg/mi_mode", default_prof["mi_mode"], type=str)
        )
        self.ffmpeg_mc_mode_combo.setCurrentText(
            self.settings.value("ffmpeg/mc_mode", default_prof["mc_mode"], type=str)
        )
        self.ffmpeg_me_mode_combo.setCurrentText(
            self.settings.value("ffmpeg/me_mode", default_prof["me_mode"], type=str)
        )
        self.ffmpeg_me_algo_combo.setCurrentText(
            self.settings.value("ffmpeg/me_algo", default_prof["me_algo"], type=str)
        )
        self.ffmpeg_search_param_spinbox.setValue(
            self.settings.value(
                "ffmpeg/search_param", default_prof["search_param"], type=int
            )
        )
        self.ffmpeg_vsbmc_checkbox.setChecked(
            self.settings.value("ffmpeg/vsbmc", default_prof["vsbmc"], type=bool)
        )
        self.ffmpeg_scd_combo.setCurrentText(
            self.settings.value("ffmpeg/scd", default_prof["scd"], type=str)
        )
        self.ffmpeg_scd_thresh_spinbox.setValue(
            self.settings.value(
                "ffmpeg/scd_threshold", default_prof["scd_threshold"], type=float
            )
        )
        self.ffmpeg_mb_size_combo.setCurrentText(
            self.settings.value("ffmpeg/mb_size", default_prof["mb_size"], type=str)
        )

        self.unsharp_groupbox.setChecked(
            self.settings.value(
                "ffmpeg/apply_unsharp", default_prof["apply_unsharp"], type=bool
            )
        )
        self.unsharp_lx_spinbox.setValue(
            self.settings.value(
                "ffmpeg/unsharp_lx", default_prof["unsharp_lx"], type=int
            )
        )
        self.unsharp_ly_spinbox.setValue(
            self.settings.value(
                "ffmpeg/unsharp_ly", default_prof["unsharp_ly"], type=int
            )
        )
        self.unsharp_la_spinbox.setValue(
            self.settings.value(
                "ffmpeg/unsharp_la", default_prof["unsharp_la"], type=float
            )
        )
        self.unsharp_cx_spinbox.setValue(
            self.settings.value(
                "ffmpeg/unsharp_cx", default_prof["unsharp_cx"], type=int
            )
        )
        self.unsharp_cy_spinbox.setValue(
            self.settings.value(
                "ffmpeg/unsharp_cy", default_prof["unsharp_cy"], type=int
            )
        )
        self.unsharp_ca_spinbox.setValue(
            self.settings.value(
                "ffmpeg/unsharp_ca", default_prof["unsharp_ca"], type=float
            )
        )

        # Load quality preset text first, as it determines which other controls are relevant
        quality_preset_text = self.settings.value(
            "ffmpeg/preset_text", default_prof["preset_text"], type=str
        )
        self.ffmpeg_quality_preset_combo.setCurrentText(quality_preset_text)

        self.ffmpeg_crf_spinbox.setValue(
            self.settings.value("ffmpeg/crf", default_prof["crf"], type=int)
        )
        self.ffmpeg_bitrate_spinbox.setValue(
            self.settings.value("ffmpeg/bitrate", default_prof["bitrate"], type=int)
        )
        self.ffmpeg_bufsize_spinbox.setValue(
            self.settings.value("ffmpeg/bufsize", default_prof["bufsize"], type=int)
        )  # Load bufsize
        self.ffmpeg_pix_fmt_combo.setCurrentText(
            self.settings.value("ffmpeg/pix_fmt", default_prof["pix_fmt"], type=str)
        )
        self.ffmpeg_filter_preset_combo.setCurrentText(
            self.settings.value(
                "ffmpeg/filter_preset", default_prof["filter_preset"], type=str
            )
        )
    ...
    def saveSettings(self) -> None:
        ...
        self.settings.setValue(
            "interpolation/maxWorkers", self.max_workers_spinbox.value()
        )
        self.settings.setValue(
            "interpolation/encoder", self.encoder_combo.currentText()
        )
        self.settings.setValue(
            "interpolation/rifeModel", self.model_combo.currentText()
        )  # Save model name

        # RIFE Options
        self.settings.setValue(
            "rife/tileEnable", self.rife_tile_enable_checkbox.isChecked()
        )
        ...
        self.settings.setValue(
            "rife/ttaSpatial", self.rife_tta_spatial_checkbox.isChecked()
        )
        self.settings.setValue(
            "rife/ttaTemporal", self.rife_tta_temporal_checkbox.isChecked()
        )

        # Sanchez Options
        self.settings.setValue(
            "sanchez/falseColour", self.sanchez_false_colour_checkbox.isChecked()
        )
        self.settings.setValue("sanchez/resKm", self.sanchez_res_km_spinbox.value())

        # FFmpeg Profile and Settings
        self.settings.setValue(
            "ffmpeg/profileName", self.ffmpeg_profile_combo.currentText()
        )
        # Save all individual FFmpeg settings regardless of profile
        self.settings.setValue(
            "ffmpeg/use_ffmpeg_interp", self.ffmpeg_interp_group.isChecked()
        )
        self.settings.setValue(
            "ffmpeg/mi_mode", self.ffmpeg_mi_mode_combo.currentText()
        )
        self.settings.setValue(
            "ffmpeg/mc_mode", self.ffmpeg_mc_mode_combo.currentText()
        )
        self.settings.setValue(
            "ffmpeg/me_mode", self.ffmpeg_me_mode_combo.currentText()
        )
        self.settings.setValue(
            "ffmpeg/me_algo", self.ffmpeg_me_algo_combo.currentText()
        )
        self.settings.setValue(
            "ffmpeg/search_param", self.ffmpeg_search_param_spinbox.value()
        )
        ...
        self.settings.setValue(
            "ffmpeg/scd_threshold", self.ffmpeg_scd_thresh_spinbox.value()
        )
        self.settings.setValue(
            "ffmpeg/mb_size", self.ffmpeg_mb_size_combo.currentText()
        )
        self.settings.setValue(
            "ffmpeg/apply_unsharp", self.unsharp_groupbox.isChecked()
        )
        ...
        self.settings.setValue(
            "ffmpeg/preset_text", self.ffmpeg_quality_preset_combo.currentText()
        )
        ...
        self.settings.setValue(
            "ffmpeg/bufsize", self.ffmpeg_bufsize_spinbox.value()
        )  # Save bufsize
        self.settings.setValue(
            "ffmpeg/pix_fmt", self.ffmpeg_pix_fmt_combo.currentText()
        )
        self.settings.setValue(
            "ffmpeg/filter_preset", self.ffmpeg_filter_preset_combo.currentText()
        )
    ...
    def _validate_thread_spec(self, text: str) -> None:
        ...
        if valid_format or not text:  # Allow empty string during typing
            ...
            palette.setColor(
                palette.ColorRole.Base, QColor("#fff0f0")
        ...
        try:
            ...
            if models:
                ...
                last_model_name = self.settings.value(
                    "interpolation/rifeModel", "", type=str
                )
                if last_model_name:
                    ...
                    if index != -1:
                        ...
                        LOGGER.warning(
                            f"Previously selected model '{last_model_name}' not found."
                        )
                        self.model_combo.setCurrentIndex(0)  # Select first available
                else:
                    self.model_combo.setCurrentIndex(
                        0
                    )  # Select first available if none saved
    ...
    def _toggle_sanchez_res_enabled(self, state: Qt.CheckState) -> None:
        ...
        self.sanchez_res_km_label.setEnabled(
            is_checked and is_rife_enabled
        )  # Also enable/disable label
    ...
    def _load_process_scale_preview(
        self, image_path: Path, target_label: ClickableLabel
    ) -> QPixmap | None:
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        if not sanchez_binary.exists():
                            LOGGER.error(
                                f"Sanchez binary not found at: {sanchez_binary}"
                            )
                            raise FileNotFoundError(
                                f"Sanchez binary not found: {sanchez_binary}"
                            )

                        # Check if the binary has execute permissions
                        if not os.access(sanchez_binary, os.X_OK):
                            LOGGER.error(
                                f"Sanchez binary exists but is not executable: {sanchez_binary}"
                            )
                            # Try to make it executable
                            try:
                                os.chmod(
                                    sanchez_binary, 0o755
                                )  # rwx for owner, rx for group and others
                                LOGGER.info(
                                    f"Added execute permission to Sanchez binary: {sanchez_binary}"
                                )
                            except Exception as chmod_error:
                                LOGGER.error(
                                    f"Failed to add execute permission to Sanchez binary: {chmod_error}"
                                )
                                raise RuntimeError(
                                    f"Sanchez binary is not executable: {sanchez_binary}"
                                )
                    ...
                    if not save_result:
                        LOGGER.error(
                            f"Failed to save temporary image for processing: {temp_input_path}"
                        )
                        raise IOError(
                            f"Failed to save temporary image to {temp_input_path}"
                        )

                    # Verify the file was created
                    if (
                        not temp_input_path.exists()
                        or temp_input_path.stat().st_size == 0
                    ):
                        LOGGER.error(
                            f"Temporary file is empty or not created: {temp_input_path}"
                        )
                        raise IOError(
                            f"Temporary file creation failed: {temp_input_path}"
                        )

                    # Get the resolution setting
                    res_km = self.sanchez_res_km_spinbox.value()
                    LOGGER.debug(
                        f"Applying Sanchez processing with resolution {res_km}km"
                    )
                    ...
                    try:
                        ...
                        if not os.path.exists(bin_path):
                            LOGGER.error(
                                f"Sanchez binary not found at runtime: {bin_path}"
                            )
                            raise FileNotFoundError(
                                f"Sanchez binary not found at runtime: {bin_path}"
                            )
                        ...
                        if not os.access(bin_path, os.X_OK):
                            LOGGER.error(
                                f"Sanchez binary not executable at runtime: {bin_path}"
                            )
                            # Try to make it executable
                            try:
                                os.chmod(bin_path, 0o755)
                                LOGGER.info(
                                    f"Added execute permission to Sanchez binary at runtime: {bin_path}"
                                )
                            except Exception as chmod_error:
                                LOGGER.error(
                                    f"Failed to add execute permission: {chmod_error}"
                                )
                                raise RuntimeError(
                                    f"Cannot execute Sanchez binary: {bin_path}"
                                )
                        ...
                        LOGGER.info(
                            f"Using absolute paths: input={full_input_path}, output={full_output_path}"
                        )
                        LOGGER.info(
                            "Using the command that works from our standalone test"
                        )
                        ...
                        LOGGER.info(
                            f"Sanchez command completed with return code: {result.returncode}"
                        )
                        if result.stdout:
                        ...
                        if result.returncode != 0:
                            LOGGER.error(
                                f"Sanchez process failed with code {result.returncode}"
                            )
                            raise RuntimeError(
                                f"Sanchez process failed with code {result.returncode}"
                            )
                    ...
                    if not temp_output_path.exists():
                        LOGGER.error(
                            f"Sanchez output file was not created: {temp_output_path}"
                        )

                        # Try to create a simple test file in the same directory to check permissions
                        try:
                            test_file_path = (
                                Path(temp_output_path.parent)
                                / "test_write_permission.txt"
                            )
                            with open(test_file_path, "w") as f:
                                f.write("Test write permission")
                            LOGGER.info(
                                f"Successfully created test file at {test_file_path}"
                            )
                        ...
                        LOGGER.info(
                            "Attempting fallback with different Sanchez options..."
                        )
                        ...
                        try:
                            ...
                            if help_result.stderr:
                                LOGGER.info(
                                    f"Sanchez help stderr: {help_result.stderr}"
                                )
                            ...
                            LOGGER.info(
                                f"Trying with debug flag: {' '.join(debug_cmd)}"
                            )
                            ...
                            if not fallback_succeeded:
                                ...
                                LOGGER.info(
                                    f"Trying without verb: {' '.join(no_verb_cmd)}"
                                )
                            ...
                            if not fallback_succeeded:
                                ...
                                LOGGER.info(
                                    f"Trying with direct original file: {' '.join(orig_cmd)}"
                                )
                            ...
                            LOGGER.error(
                                f"Error during fallback attempts: {fallback_error}"
                            )

                        # If a fallback succeeded, continue with the existing code path
                        if fallback_succeeded:
                            LOGGER.info(
                                "Using successfully generated output file from fallback"
                            )
                        else:
                            ...
                            raise FileNotFoundError(
                                f"Sanchez output file not found: {temp_output_path}"
                            )

                    # Check output file size
                    if temp_output_path.stat().st_size == 0:
                        LOGGER.error(
                            f"Sanchez output file is empty: {temp_output_path}"
                        )
                        raise IOError(
                            f"Sanchez output file is empty: {temp_output_path}"
                        )
                    ...
                    if not processed_img.isNull():
                        LOGGER.debug(
                            f"Successfully applied Sanchez processing to {image_path.name}"
                        )
                        img = processed_img
                    else:
                        LOGGER.error(
                            f"Sanchez processing resulted in null image for {image_path.name}"
                        )
                        raise IOError(f"Processed image is null: {temp_output_path}")

                except Exception as e:
                    LOGGER.error(
                        f"Error during Sanchez processing for {image_path.name}: {e}"
                    )
                    sanchez_processing_failed = True
            ...
            if self.current_crop_rect:
                ...
                if x < img_w and y < img_h:  # Use < directly
                    ...
                    if crop_w > 0 and crop_h > 0:
                        ...
                        LOGGER.warning(
                            f"Invalid crop rect {self.current_crop_rect} for image size {img_w}x{img_h}"
                        )
                else:
                    LOGGER.warning(
                        f"Crop start point {x},{y} outside image bounds {img_w}x{img_h}"
                    )
            # --- End Crop ---
            ...
            target_label.processed_image = (
                img.copy()
            )  # Store the processed image for zoom
            ...
            if target_size.width() <= 0 or target_size.height() <= 0:
                ...
                LOGGER.warning(
                    f"Target label '{target_label.objectName()}' has invalid size {target_size}, cannot scale preview."
                )
            ...
            if (
                ...
                and self.sanchez_false_colour_checkbox.isChecked()
            ):
    ...
    def _update_previews(self) -> None:
        ...
        def clear_all_previews(message: str = "") -> None:
            for i, label in enumerate(labels):
                ...
                label.setText(
                    default_texts[i] if not message else message if i == 0 else ""
                )
        ...
        if not in_dir_path.is_dir():
            ...
            if self.in_dir_edit.text():
                labels[0].setText(self.tr("Invalid Directory"))
            return
        ...
        try:
            image_files = sorted(
                [
                    ...
                    if f.is_file()
                    and f.suffix.lower() in [".png", ".jpg", ".jpeg", ".tif", ".tiff"]
                ]
            )
        except Exception as e:
        ...
        for label in labels:
            label.setPixmap(QPixmap())
            label.setText(self.tr("Loading..."))  # Indicate loading state
            label.file_path = None
        ...
        for i, label in enumerate(labels):
            if i not in processed_indices:
                ...
                label.setText(
                    self.tr("")
                )  # Clear text (e.g., middle/last if only 1 or 2 images)
                label.file_path = None
    ...
    def _update_start_button_state(self) -> None:
        ...
        out_file_ok = (
            bool(out_file_path_str)
            and pathlib.Path(out_file_path_str).parent.is_dir()
            and out_file_path_str.lower().endswith(".mp4")
        )
        encoder = self.encoder_combo.currentText()
        model_selected = (encoder == "FFmpeg") or (
            encoder == "RIFE" and self.model_combo.currentData() is not None
        )
        ...
        if encoder == "RIFE":
            rife_spec_ok = (
                re.fullmatch(r"\d+:\d+:\d+:\d+", self.rife_thread_spec_edit.text())
                is not None
            )
        ...
        out_file_exists = (
            bool(out_file_path_str) and pathlib.Path(out_file_path_str).exists()
        )  # Check non-empty path first
    ...
    def _set_processing_state(self, processing: bool) -> None:
        ...
        is_rife_encoder = (
            self.encoder_combo.currentText() == "RIFE"
        )  # Check current encoder
        model_available = (
            self.model_combo.count() > 0 and self.model_combo.currentData() is not None
        )

        can_start = (
            pathlib.Path(self.in_dir_edit.text()).is_dir()
            and pathlib.Path(self.out_file_edit.text()).parent.is_dir()
            and self.out_file_edit.text().lower().endswith(".mp4")
            and (is_rife_encoder and model_available)
            or (not is_rife_encoder)
        )
        # Check RIFE thread spec validity if RIFE is selected
        rife_spec_ok = True
        if is_rife_encoder:
        ...
        self.open_vlc_button.setEnabled(
            not processing and pathlib.Path(self.out_file_edit.text()).exists()
        )
        ...
        self.model_combo.setEnabled(
            not processing and is_rife_encoder and self.model_combo.count() > 0
        )
        self.crop_button.setEnabled(
            not processing and pathlib.Path(self.in_dir_edit.text()).is_dir()
        )  # Crop button enabled if not processing and input dir is set
        self.clear_crop_button.setEnabled(
            not processing and self.current_crop_rect is not None
        )  # Clear crop button enabled if not processing and crop is set
        self.main_tabs.setEnabled(
            not processing
        )  # Disable the entire tab widget while processing
    ...
    def _update_crop_buttons_state(self) -> None:
        ...
        self.crop_button.setEnabled(
            in_dir_ok and preview_loaded and not self.is_processing
        )
        self.clear_crop_button.setEnabled(
    ...
    def _update_rife_options_state(self, encoder_type: str) -> None:
        ...
        if is_rife:
            self._toggle_sanchez_res_enabled(
                self.sanchez_false_colour_checkbox.checkState()
            )
        else:
        ...
        if hasattr(self, "ffmpeg_settings_tab"):
            ...
            LOGGER.warning(
                "ffmpeg_settings_tab not found during _update_rife_options_state"
            )
