...
class FFmpegSettingsTab(QWidget):
    ...
    def _create_widgets(self) -> None:
        ...
        self.ffmpeg_settings_group = QGroupBox(self.tr("Interpolation (minterpolate filter)"))
        ...
        self.ffmpeg_mb_size_combo.addItems([self.tr("(default)"), self.tr("8"), self.tr("16"), self.tr("32"), self.tr("64")])  # Common block sizes
        ...
        self.ffmpeg_pix_fmt_combo.addItems([self.tr("yuv444p"), self.tr("yuv420p"), self.tr("yuv422p"), self.tr("rgb24")])  # Common formats
    ...
    def _compare_float_values(
        ...
        if not isinstance(current_value, (float, int)) or not isinstance(profile_value, (float, int)):
            LOGGER.warning(
                ...
                key, current_value, profile_value
            )
    ...
    def _check_settings_match_profile(self, profile_dict: FfmpegProfile) -> bool:
        ...
        for key in ffmpeg_profile_keys_to_compare:
            ...
            if key not in current_settings:
                LOGGER.warning(
                    "Key %r in profile but not returned by get_current_settings().",
                    key
                )
                return False
            ...
            if key in ["scd_threshold", "unsharp_la", "unsharp_ca"]:
                if not self._compare_float_values(key, current_value, profile_value,
                                               current_settings, profile_dict):
                    return False
