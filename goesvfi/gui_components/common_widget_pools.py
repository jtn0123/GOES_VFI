...
def initialize_common_pools() -> None:
    ...
    register_widget_pool(
        "status_labels", QLabel, create_status_label,
        max_size=20, cleanup_func=cleanup_label
    )

    register_widget_pool(
        "info_labels", QLabel, create_info_label,
        max_size=15, cleanup_func=cleanup_label
    )

    register_widget_pool(
        "preview_labels", QLabel, create_preview_label,
        max_size=10, cleanup_func=cleanup_label
    )

    # Button pools
    register_widget_pool(
        "action_buttons", QPushButton, create_action_button,
        max_size=15, cleanup_func=cleanup_button
    )

    register_widget_pool(
        "secondary_buttons", QPushButton, create_secondary_button,
        max_size=10, cleanup_func=cleanup_button
    )

    # Progress bar pool
    register_widget_pool(
        "progress_bars", QProgressBar, create_progress_bar,
        max_size=8, cleanup_func=cleanup_progress_bar
    )

    # Group box pool
    register_widget_pool(
        "group_boxes", QGroupBox, create_group_box,
        max_size=12, cleanup_func=cleanup_group_box
    )

    # Layout pools (these are less commonly pooled but useful for complex dynamic UIs)
    register_widget_pool(
        "horizontal_layouts", QHBoxLayout, create_horizontal_layout,
        max_size=8, cleanup_func=cleanup_layout
    )

    register_widget_pool(
        "vertical_layouts", QVBoxLayout, create_vertical_layout,
        max_size=8, cleanup_func=cleanup_layout
    )
